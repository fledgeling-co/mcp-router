...
| **I6** | **Make Mac approval fast, without moving the boundary** | mac | M6 ✓ | — | Opus | **REFUSED by review — fixing** `wf_04d82125-374` | `ai/i6` | **Gates green and the review refused it anyway**, which is the shape this fleet distrusts: rebase 0, lint 0 / 460 files, **1460 tests / 178 suites**, build-mac 0. The review **cleared the boundary** — nothing installs without a human at this Mac along any indirect path (notification actions, defaults, timers, undo expiry, test doubles in release paths), and nothing asserts a phone-originated arrival happens today. It refused on everything else |
