...
| **OWED** | **Re-verify `mac-shell.sh` on an idle machine** | orchestrator | M14 measured exit 0 / 39 assertions; the orchestrator's re-run failed at `axkit setframe` under load 137, outside M14's diff. **Do this when the fleet lane is empty, and do NOT re-run under load until it goes green** — that is the `D-p` mistake |
---
