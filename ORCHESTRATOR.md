...
| G3 | `make test` is not deterministically green | harness | — | — | Opus | **Needs More Work** (2nd) — gap-fix 2 | `ai/g3` `b646364` | **Verified 2026-08-21 at `metamorphic`; verdict Needs More Work (gap-fix 2 queued, `G3-gapfix-2.md`).** **The blocking finding is closed and the property is established by the verifier's own re-run**: assigned mutation exit 2 after **14.151 s** at `PoolReapingTests.swift:98:29` naming *timed out after 10.0s waiting for: `own` to be reaped under the arming it just made*; control with P6's wrap removed exit **142 with zero `Test run with` lines**; `make test` exit 0 on **8 runs**, lint 0/494, parity 358/358, `acceptance-r6` clean. Slower than the runner's numbers at 0-11% idle with two siblings live — contention, not divergence. **Blocking: the scanner built to keep the property holding is defeated in both directions**, by five shapes its ten controls do not cover, found with 22 constructed call sites across three files. Misses: a trailing `// TODO: wrap in awaitEvent("...")` satisfies the check for the wrapper because `isBounded`'s first line tests the **raw** line — drop the paren and it still reds, which pins it; likewise `/* awaitEvent( */` and a URL ending `/awaitEvent(`; and a line with `//` inside a string literal is not seen as a call at all, since `isCall` truncates at the first `//`. **The panel had already found the mirror image of the first** and it was fixed in the upward walk, one line below the shortcut that still carries it. False fires: a correctly wrapped call inside `#if DEBUG` at column 0 (the walk sets `depth = 0` and can never step out), a call inside a `/* */` block, and a tab-indented call. The file's own doc says *a false red is what this gate exists to not be* — falsified as delivered. **`D-g3-j`'s fourth correction did not land and its row claims it did**: `D-g3-b` still cites `PoolLifecycleTests.swift:114` where the sleep is at `:116`, correct at `4c0f920` and shifted by the gap-fix commit itself, so the two rows stay on different bases — the exact defect `D-g3-j` was opened for. **Acceptance criterion 3 is vacuous, and it was the orchestrator's**: relaxing `:87` so execution reaches `:98` under the same mutation gives P6 passing in **2.291 s** and the run green, because mutation B leaves the reap deadline on the requested 25 ms window and so could not take ten minutes whichever side the bound is on. Sixth entry in G4. **Grok's correction verified right** and taken correctly — `AuthorizationURLBox` at `OAuthFlowStarter.swift:82-119` is `withTaskCancellationHandler` around `withCheckedThrowingContinuation`, a cancellation-aware wait a group does bound. **Gemini's CRITICAL was half right, not refuted, and is reopened as `D-g3-q`**: gutting both accessors reds only 2 of the 5 bounded call sites, 4 of 4 runs, so three have no demonstrated mutation power. **Two fixed-sleep flakes measured red outside the surveyed suites** — `CallbackListenerTests:108` 2 of 3 and `OAuthWireTests:263` once — which is `D-g3-c`'s prediction observed rather than surveyed. Follow-ups `D-g3-l`...`D-g3-v`. |
