...
| M23 | The mock-to-SwiftUI conversion contract | `M23-mock-to-swiftui-contract.md` | — | — | **Needs More Work** (4th) — gap-fix 4 queued (`M23-gapfix-4.md`); verified at `effect-witness`. **Not a fourth instance of the mechanism** — every property the item was bounced for now holds and was witnessed, including BL-1's partition against two routes of the verifier's own and BL-2's enumeration measured at exactly four reads inside the allowlist. One blocker, and it is an arm rather than a product defect: case 51 stays green when the marker it guards is moved before the write, because neither of its invocations drives the only configuration where the orderings differ. The missing arm is a third invocation of a fixture case 33 already builds. Follow-ups `D-m23-ac`…`D-m23-ai` |
