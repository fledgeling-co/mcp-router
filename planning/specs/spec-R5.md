...
|---|---|---|
| B77 | `POST /servers/:name/auth` answers **400** `{error:"stdio servers do not authorize; their credentials are env vars"}` for a stdio upstream, **200** `{server, authorizationUrl}` in that key order otherwise, and **502** `{error:<the thrown message>}` when `beginAuth` throws. The stdio check runs **before** any flow is begun, so a stdio request binds no port. `isStdio` is `u.transport === "stdio"` ("export const isStdio = (u: UpstreamConfig): u is StdioUpstream => u.transport === 'stdio';", `config.ts:55` at `93ced44`) — a discriminant on the config union, never “has a command” or “has no url” | fixture test against `auth-start.json` + a branch test each, plus a spy asserting no listener binds on the stdio path |
...
| B79 | The completion side-effect runs **only** on the flow resolving: `clearPending(name)` then re-index, in that order. It is attached **before** the 200 is written (`void flow.completed`, `control.ts:399-405` at `93ced44`), so a Swift port must not assume the response has already been sent. A rejection from **either** the flow **or** the re-index logs `authorization for "<name>" did not complete: <message>` at **warn** — one message, two sources — and is never surfaced as a request failure | ordered spy trace on the success path, **plus** a separate test rejecting from the re-index rather than the flow, both asserting the same warn and an unchanged request outcome |
| B80 | Every server name used as a **lookup or map key** by the auth store and both routes is keyed on **UTF-16 code units** (`JSString`), never Swift `String`, whose equality is canonical. The reference's mechanisms are `Map.get` ("const u = deps.upstreams.get(name);", `control.ts:301` at `93ced44`), object-key lookup (`manifest.servers[name]`) and `===` ("if (current?.flow.server === serverName) current = undefined;", `auth.ts:242` at `93ced44`) — there is no `!==` name comparison, and the earlier draft citing one was wrong. **This divergence is latent, not live**: `ServerParser.isAdoptableName` refuses any name outside `[A-Za-z0-9_-]+` (verified at "/// `/^[A-Za-z0-9_-]+$/` — written out rather than regex-matched so it is obvious that an", `Config/ServerParser.swift:106-112` at `93ced44`), so two spellings of one name cannot both reach a port and no cross-record read is reachable today. The `JSString` typing stands anyway — it is what stops the gate's removal or relaxation from silently becoming a divergence — but R5 claims **no live bug fixed here** | red-green test at the **key level only**: a `JSString` map holding U+00E9 and U+0065 U+0301 keeps two entries and resolves neither from the other. **Not** asserted through the filesystem — default macOS APFS volumes compare filenames normalisation-insensitively, so a disk round-trip conflates the two for *both* routers and would prove nothing |
...
| # | Clause | Evidence type |
|---|---|---|
| B83 | Both routes sit **inside** the `/^\/servers\/([^/]+)(\/[a-z]+)?$/` block, behind `deps.upstreams.get(name)`: an unconfigured name answers **404** `{error:"no server named \"<name>\""}` and never reaches the sub-route ("const serverMatch = /^\/servers\/([^/]+)(\/[a-z]+)?$/.exec(p);", `control.ts:297-305` at `93ced44`) | fixture-shaped test per route with an unknown name asserting 404 and that no manifest read or flow begin occurred |
| B84 | `beginAuth` has **two** pre-flow failures, both surfacing as B77's 502, with different semantics: a `listen` error on the fixed port (EADDRINUSE) rejects **without** any cleanup — timer armed, transport unclosed (`await new Promise<void>((resolve, reject) => {`, `auth.ts:245-251` at `93ced44`) — while the **20 000 ms** URL race (separate from `timeoutMs`) rejects `the server never produced an authorization URL` **after** `cleanup()` (`auth.ts:257-265) | test per path asserting the 502 body and, for each, whether a timer/transport survives |
| B85 | Starting a second flow calls `current?.close()`, which is `cleanup` — and `cleanup` **never calls `settle`** (`auth.ts:181, 238-243, 276`). The superseded flow's `completed` therefore **never settles**: neither its side-effect nor its warn ever runs. Only `cancel()` settles-with-error (`cancel: () => {`, `auth.ts:271-274` at `93ced44`). **Swift correction:** a `CheckedContinuation` dropped un-resumed is *not* the equivalent — it trips `SWIFT TASK CONTINUATION MISUSE` and suspends its observer forever, where JavaScript's `completed.catch(() => undefined)` ("completed.catch(() => undefined);", `auth.ts:198` at `16ba14f`) makes the abandoned promise inert. The observer is therefore released with a distinguished `AuthAbandoned`, which every caller ignores: no side effect, no warn, no stranded task — observationally identical to the reference | red-green test: supersede a flow, then assert over a bounded wait that the observer settled as **abandoned** rather than resolved or rejected, that `clearPending`, re-index and the warn never fired, and that the run emits no continuation-misuse warning. A port that cancels the prior task with an error fails this |
| B86 | On the missing-code path the **page** renders `the provider returned no code` while the **rejection** carries `no authorization code returned` ("res.end(PAGE('Authorization failed', error ?? 'the provider returned no code'));", `auth.ts:211-212` at `93ced44`) — two different strings, and B79's `<message>` is the latter | test asserting both strings from one exercise of the path |
| B87 | `clientMetadata` is the dynamic-registration request body and is byte-visible: `client_name` = `mcp-router (<server>)`, `client_uri` = `https://mcp-router.fledgeling.app`, `redirect_uris` = `[<AUTH_REDIRECT_URI>]`, `grant_types` = `["authorization_code","refresh_token"]`, `response_types` = `["code"]`, `token_endpoint_auth_method` = `"none"`, **in that key order** (`get clientMetadata(): OAuthClientMetadata {`, `auth.ts:87-98` at `93ced44`) | serialized-bytes test against the literal expected JSON |
| B88 | `/approve` reads the manifest **fresh from disk** via `loadManifest(cfg.manifestPath)` and writes with `saveManifest` (`control.ts:348,361`), where `/changes` uses the **cached** `deps.manifest.current()` ("const entry = deps.manifest.current().servers[name];", `control.ts:337` at `93ced44`). A corrupt manifest degrades to `{version:1,servers:{}}` with a warn (`export function loadManifest(path: string): Manifest {`, `manifest.ts:115-122` at `93ced44`), so approve against one answers 409, never a throw | a stale-cache test proving the read is from disk, plus a corrupt-manifest test asserting 409 |
| B89 | The written entry follows JS spread order: keys already on `entry` keep their existing positions and only genuinely new keys append; `pending: undefined` is **dropped** by serialization, not emitted as null (`manifest.servers[name] = {`, `control.ts:354-360` at `93ced44`). `saveManifest` writes `JSON.stringify(manifest, null, 2)` through temp+rename (`export function saveManifest(path: string, manifest: Manifest): void {`, `manifest.ts:134-140` at `93ced44`) | byte test over an entry whose original key order differs from the literal's, asserting the file bytes |
| B90 | `AUTH_CALLBACK_PORT` is `Number(process.env.MCP_ROUTER_AUTH_PORT ?? 8880)` and `AUTH_REDIRECT_URI` is derived from it **once at module load**, not per call ("export const AUTH_CALLBACK_PORT = Number(process.env.MCP_ROUTER_AUTH_PORT ?? 8880);", `auth.ts:21-22` at `93ced44`). It is byte-visible inside `authorizationUrl` (`…%3A8880%2Fcallback` in `auth-start.json`) | test asserting the override is honoured and that a later environment mutation does **not** change the derived URI |
| B91 | The auth record file is `JSON.stringify(rec, null, 2)` — 2-space pretty ("writeFileSync(recordPath(server), JSON.stringify(rec, null, 2), { mode: 0o600 });", `auth.ts:49` at `93ced44`) — and every writer merges `{...readRecord(server), …}`, so key order is read-order-then-append | byte test over a record written by two successive partial writes |
...
| B94 | Two `log.info` lines are required verbatim: `authorized upstream "<name>"`, emitted **between** the 200 page write and `settle()` (`authorized upstream "${serverName}"`, `auth.ts:220` at `93ced44`), and `approved "<name>"'s new tool surface (<n> tools)`, emitted **before** the 200 (`approved "${name}"'s new tool surface (${entry.pending.tools.length} tools)`, `control.ts:362` at `93ced44`) | log-capture test asserting text and ordering |
...
| B96 | `cleanup`'s final step is guarded: `if (current?.flow.server === serverName) current = undefined` ("if (current?.flow.server === serverName) current = undefined;", `auth.ts:242` at `93ced44`). Clearing unconditionally diverges — a superseded flow's late callback would null the flow that replaced it | red-green test: supersede A with B, then drive A's callback, then assert `currentFlow()` still reports B |
| B97 | `authorizedAt(server)` returns the record's `authorizedAt` verbatim or undefined (`export function authorizedAt(server: string): string`, `auth.ts:56-58` at `93ced44`), and `codeVerifier()` **throws** `no PKCE code verifier saved for "<server>"` when absent (`auth.ts:126) — a reachable path with a fixed message | test per function incl. the throw's message |
| B98 | *(folded into B77 — `isStdio` definition)* | — |
| B99 | Both 400 paths and the 500 path pass the literal `Authorization failed` as `title`, which `PAGE` renders into **both** `<title>` and `<h1>` (`const PAGE = (title: string, detail: string): string =>`, `auth.ts:156-161` at `93ced44`). The timeout's rejection message is `authorization timed out` ("settle(new Error('authorization timed out'));", `auth.ts:233` at `93ced44`) | byte test per page asserting the full emitted string |
| B100 | B61's warning is verbatim `auth record for "<server>" unreadable (<err.message>); treating as unauthorized` (`auth record for "${server}" unreadable (${(err as Error).message}); treating as`, `auth.ts:40` at `93ced44`) — pinned to the character, as B79's is | log-capture test |
...
name outside `[A-Za-z0-9_-]+` ("/// `/^[A-Za-z0-9_-]+$/` — written out rather than regex-matched so it is obvious that an", `Config/ServerParser.swift:106-112` at `93ced44`), and additionally refuses `__`,
