...
## Why `test-ios-glass` is not in `make all`

Two of five cases are red and both reds are findings rather than flakes. Adding a known-red lane to
the whole-repo gate either blocks every unrelated commit or invites someone to soften the two
assertions — and those assertions are the only reason the findings are visible. The target exists,
is documented, and joins `all` when DEF-X2-b and DEF-X2-c close.
