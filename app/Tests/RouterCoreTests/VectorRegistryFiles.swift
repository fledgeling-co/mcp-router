...
extension VectorRegistry {
    ...
    static let coreFiles: [RegisteredVectorFile] = [
        // R5 — the callback pages. Generated by IMPORTING the reference's own exported `PAGE`
        // from `dist/auth.js`, so these are reference-derived rather than transcribed from what
        // the port happens to emit. R4 diffs exactly these bytes.
        //
