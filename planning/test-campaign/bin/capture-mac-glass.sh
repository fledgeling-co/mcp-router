...
# Nine boards, each selected then read back. SURF-026 joined on 2026-08-27 (G16): the
# Insights board shipped with M22 and was never enumerated, so it was never selected here. SURF-001 is the shell — chrome that
...
do
  ...
  # predicate wrongly accepting SURF-011's dump. Filed as DEF-058.
  #
  ...
  if [ "$SELECTED" = "0" ]; then
    ...
    echo "  $("$AXKIT" title "$PID") instead. Filing that picture under $sid is DEF-058."
    continue
  fi
