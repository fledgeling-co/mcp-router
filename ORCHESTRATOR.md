...
| D-g3-q | Gutting the pool's two awaits leaves four of the five call sites green | G3 | **Re-measured in gap-fix 2 and it is wider than first recorded; deferred with a tested reason.** Both accessors replaced with an immediate `return`: only `PoolReapingTests.swift:101` reds, 4 of 4 runs at 0% idle — `PoolTests.swift:144` stayed green here where the verifier saw it red 4 of 4, so that site's power is load-dependent (it needs the 30 ms window not to have elapsed while the machine is busy). **Why the three `awaitSessionEnded` sites cannot be fixed at the call site**: a probe printing which branch the accessor takes reports `PROBE-EARLY-RETURN` at 3 of 3 and `PROBE-AWAITS-WATCHER` at 0, so at every one of those sites the handle is already gone and the accessor awaits nothing whatever the caller does. That is `D-g3-g`'s mechanism, and `D-g3-g` is the remedy; it stays deferred by this pass's scope, so this does too. The bound it places on the gate is now explicit: `PoolAwaitBoundTests` proves no site is unbounded, not that every site observes something |
