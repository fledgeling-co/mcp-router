...
| **MERGE HAZARD** | **An untracked draft in the main tree refuses the merge that would replace it** | orchestrator | D1's merge aborted on `planning/specs/spec-D1.md`: the dead run left an 8.9k draft untracked in main and the branch commits a 20k finished one. **Fix the drafts, not the merge** — the draft was preserved to `/tmp/spec-D1-dead-run-draft.md`, checked to be a strict subset (no `D-` id lost, constraints all carried), then removed. **D2 will hit this identically**: `spec-D2.md` and `plan-D2.md` are untracked in main right now |
