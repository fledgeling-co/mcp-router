...
| **I6** | **Make Mac approval fast, without moving the boundary** | mac | M6 ✓ | — | Opus | **Closing out** `wf_01aefa66-4eb` | `ai/i6` | 16 commits, tree clean, **compiles and green**: build-mac 0, test 0 at **1463 tests / 178 suites**. The refusing review is committed at `f503f63` as the work list. `0abceb2` **re-aimed the mutation filters** — `i6-mutations.sh` now names `InboxBandTests` 7 times and `InboxAnnouncementTests` 10, where it named the band suite 15 times and the announcement suite never. `15d9e7f` (rescue) implements the two hardest findings: **two notification categories** rather than one stamped on every banner, and **the shell moved to the app delegate** so the notification delegate attaches at launch. **Re-aiming a filter is a claim, not a proof** — the close-out owes nine mutations each shown going red for the right reason, plus the remaining findings and a second review |
...
| **D-i6-a** | **THE ORCHESTRATOR'S RESCUE COMMIT BROKE THE MUTATION GATE** | open, being fixed | Mine, not a runner's. `7fe67ab` is I6's uncommitted tail, committed verbatim after an API death and **explicitly labelled unreviewed**. That tail **split `InboxAnnouncementTests` out of `InboxBandTests`**, and **fifteen filters in `i6-mutations.sh` still name `InboxBandTests`** — which now holds 11 clauses, while the **8 announcement clauses those mutations test moved to the new suite**. The reviewer **proved it rather than asserting it**: applied A11, got `InboxBandTests` 11 passed exit 0 and `InboxAnnouncementTests` red, so **the gate would have printed GREEN A11**. Reverted cleanly. **Nine of twenty-six mutations have measured nothing since that commit.** Verified independently by the orchestrator. **The risk was named in the rescue commit's own message and it landed anyway — a rescue is a rescue, never a result** |
| **D-i6-b** | The many-item notification ships a button the spec forbade | open, being fixed | One category is registered for **every** banner (`ArrivalNotifierFactory.swift:54-76`, stamped at :93), so macOS draws **Decline** on the many-item banner that `spec-I6.md:234-235` says must not have one. The assertion at `InboxAnnouncementTests.swift:55` **tests the value, not what macOS draws**, so it is green against the wrong behaviour. Pressing it does not decline — the delegate opens the Inbox for every action |
| **D-i6-c** | The notification delegate is installed too late to receive a launch response | open, being fixed | Set in a view's `onAppear` (`MCPRouterApp.swift:33-37`) rather than at launch. `UNUserNotificationCenter.delegate` must be set before the app finishes launching or **the response that LAUNCHED the app is discarded**. Quit the app, leave a banner, press Decline: the app opens and nothing is declined, **which is worse than losing Review because the user believes they declined** |
| **D-i6-d** | Four smaller claims the spec makes that the code does not keep | open | `InboxCopy.Arrival.notificationsOff` is never rendered and `notificationsAuthorized` is written but never read; a **disabled partial row renders a full-width Review button** while the spec says it *carries no review affordance* — the reviewer's phrase is the right one, **the affordance is a lie, not a hole**; withdrawal is derived on the next `load()` so it is up to one 2s poll late rather than *the moment*; and the spec's keyboard table describes handlers `MenuBarPopover` does not have, which **M8's spec also claimed and also did not implement** |
