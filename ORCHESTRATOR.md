...
| R7 | The router's thesis is unmet for every harness but Claude Code | router | R3 ✓ | — | Opus | **Ready to verify** — gap-fix delivered | `ai/r7` `c4ecff1` | **Gap-fix delivered 2026-08-21; ready to re-verify.** All three blocking findings closed, each proved by a mutation that went red and returned on restore. **F1** — `HarnessRoute.detect` read `url` alone, so `.directHTTP` was unreachable for Gemini, which reported `not-wired` and was then offered a plan to wire it. `HarnessDialect` now carries the spellings **per client**; `ServerParser` and `UpstreamHash` are untouched, and the harness entry's **raw** JSON is normalised at the seam because the digest reads `raw.member("url")` rather than the parsed value. Reverting Gemini's key list turns 24 of 38 unit tests red and the lane to 8 failing checks, including `no + add line for a harness that is already wired: expected 0, got 1` — the original defect verbatim. **F2** — `unreadable` is on the wire with the parser's own sentence; the lane pins both directions, the whole empty row, and that the screen carries the *same* sentence, so a hard-coded string fails. Dropping the member gives 3 failing checks. **F3** — both rules are file-scoped over comment-blanked code, a third refuses any write inside the seam, and the vocabulary is argument labels so `FileHandle.init(forWritingTo:)` matches too. **The verifier's realistic-applier plant now exits 1**; the selftest holds 13 cases and six mutations of the gate's own rules each turn it red. **The out-of-family review is the process finding this item was sent back on, and all three lanes delivered** — codex 4,578 B (`model: gpt-5.6-sol`, `reasoning effort: high` asserted in the log), grok 4,750 B, agy 5,371 B, raw under `planning/evidence/R7/`. All three independently named the same top defect, and it was one the gap-fix had introduced: the `httpUrl` widening was global rather than Gemini's. Taken in full, with thirteen further findings taken and three overruled with reasons recorded. **The gate's claim is now narrower than it was, on purpose**: `grep` is not a call graph, an applier split across two neutrally-named files outside the seam walks through, and the selftest asserts that miss as `P10` so it shows up in a run — `D-r7-m`, with the closed-world fix and its cost named. Gates: swiftformat 0/507 · swiftlint --strict 0/500 · `make test` **twice at 1621**, both 0, and `PoolReapingTests` (`G3`) did not flake in four runs this session · parity **358/358 at floor** · lock-selftest 12 · normalise-selftest 14 · the R7 lane `pass` at 27 checks over five passes. `parity-manifest-selftest` exit 2 and `make lint`/`make all` blocked at the `tools` guard in a worktree with no `node_modules` — recorded as blocked, not as passes; the six lint steps were run individually. Six new follow-ups `D-r7-l`…`D-r7-q`; `D-r7-j` closed by the digest that replaced the lane's one-token boundary check. The eleven inherited follow-ups stay deferred, children R7-C1…C4 unchanged. |
...
| D-r7-p | An entry declaring two different endpoints under two spellings is resolved by precedence, not by evidence | new item | `{"url": "https://a/mcp", "httpUrl": "https://b/mcp"}` in a Gemini file: route detection accepts **either** as a match for this router, which is the right answer for a yes/no question, while the identity comparison takes `url` — matching what `ServerParser` would have done with the same bytes. Nothing establishes which key `agy` itself prefers, and the honest alternative is to report the entry `unparsed` rather than pick. Not seen on any machine; filed because the precedence is currently an assumption wearing the shape of a rule |
