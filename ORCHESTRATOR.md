...
| G3 | `make test` is not deterministically green | harness | — | — | Opus | **Needs More Work** (3rd) — gap-fix 3 | `ai/g3` `e8c20e0` | **Verified 2026-08-21 at `metamorphic`; verdict Needs More Work (3rd; gap-fix 3 to be written).** **Everything re-measured holds**: `make test` 0 twice at 1587/199, lint 0 over 497 files, parity 358/358, `acceptance-r6` clean, assigned mutation exit 2 at **10.836 s** against the runner's 10.589 s *under load 127*, `git diff app/` empty on the delivered tree and again after the verifier's own mutations were restored. The verifier built `Delexer` and `AwaitBoundScan` standalone and walked the four trees itself: exactly 5 call sites, all bounded, no unreadable file. **`PrimitiveBodyTests.swift:140` was a real miss, not self-inflicted** — `#""""#` is present at `9420fcf` and `b646364` and was last touched by an unrelated R3 commit. **Criterion 3's deletion is honest rather than convenient**: mutation B leaves the reap deadline on the requested 25 ms window so nothing can wait long whichever side the bound sits on, the only observable is duration, and the failure text *is* the bound's own 10.0 s timeout against 601 s unbounded. **B1 — the runner's correction of the previous verifier does not reproduce.** Gutting both accessors reds `PoolTests.swift:144` **and** `PoolReapingTests.swift:101` in **4 of 4**, at 15.5% idle falling to 0.6% and load 127 — heavier contention than the runner's, so the load-dependence explanation fails on its own terms. The register now asserts one site fewer than the verifier saw and derives a narrower deferral from it; that derivation does not stand. **B2 — the completeness claim is right about where the approximation is and wrong about which way it fails.** The brief says the residual consequences each fail toward a red on correct source rather than toward a miss. Two counterexamples, each pinned by a one-token control: a **statement label** before a control keyword (`check: if awaitEvent(x) {` reads the enclosed call BOUND; delete the label and the identical source reads UNBOUND, because `firstWord(of:)` returns the label and the `bodyKeywords` guard never fires — legal on `if`, `while`, `for`, `switch`, `do`); and a **labelled string-literal final argument** (`awaitReap(name: "own")` produces no call site at all, because the literal delexes to spaces and the walk-back lands on the label's colon). Both are misses, in the layer the brief said fails only toward reds. Plus a false fire: the word `Task` anywhere in the opener statement, **including inside a string interpolation**, reddens a correct wrap. **B3 — the population is not closed as claimed.** The scanner implements three models, not two: families A (19 controls) and B (12) rest on closed, citable production lists and that argument is sound; family C's 22 are `verdict`/`statement`/`firstWord`/`continuesStatement` plus four hand-written keyword lists, whose population is exactly *shapes somebody might write* — the open set the argument set out to escape. The 34-of-34 matrix proves mutation adequacy of the code as written, which is a different claim from covering the grammar the code should implement. **B4 — five register ids occur twice in one table** (`D-g3-g`...`D-g3-k`), three with materially different rows, introduced by the runner's own merge `e4cb050` and shipped at `cbc6a81`. Lanes: gemini delivered 7 candidates on a break-it prompt at 26 KB, one of them a mechanism nobody had named; fable produced nothing after ~10 minutes at 7% idle and 26 KB and is reported as not delivered rather than as a lane defect; codex down to 27 Aug; grok not attempted. Follow-ups `D-g3-aa`...`D-g3-ag`. |
