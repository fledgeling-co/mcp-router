...
| **P6** | **The gate states a target the owner did not choose** | router | — | — | Opus | **INCOMPLETE — rescued, not ready** | `ai/p6` | Zero commits of its own in ~29 minutes; its whole working diff was rescued as `5424df4`. The approach reads sound — `excluded` as a **PARTITION of the blocked rows rather than a subtraction from them**, with the report block explicitly scoring nothing — but **that is precisely what the item was dispatched to PROVE and did not reach**. No mutation, no before/after gate output, no review. Not mergeable until a runner shows the arithmetic diff is zero |
...
| **D-p5-b** | **THE LIFELINE GATEWAY IS DESTROYING RUNS — four killed** | **OPEN, BLOCKING, owner's call** | Diagnosed rather than guessed. PID **68985** (`claude-lifeline/dist/gateway`, up 2h36m) **holds 127.0.0.1:8787 and does not answer** — `curl` gets HTTP **000** after a 5s timeout, while its upstream **8858 answers 200 in 0.7ms** and 8857 has no listener at all. `lifeline doctor` agrees: *gateway unreachable — the operation was aborted due to timeout*. **A bound socket that does not serve**, which is the same class as a healthz that lies. **Mechanism**: launching claude re-chains agents through 8787, so direct probes succeed while every workflow runner dies. **Killed P5, I5, I6 and P6.** NOT TOUCHED: it is the owner's multi-account proxy and another session may hold it — this fleet has already destroyed another session's browser by killing a pid it did not own. **No further runners will be launched until it is resolved** |
...
| **I6** | **Make Mac approval fast, without moving the boundary** | mac | M6 ✓ | — | Opus | **INCOMPLETE — rescued, not ready** | `ai/i6` | Five commits of its own plus a rescue commit `5e62567`. Spec, the Kit-side band, the popover UI, the notification delegate and 32 test clauses all landed. **Phase 3 was partway and Phase 4 — gates and the out-of-family review — never ran**, so nothing here is gated and it must not be merged as it stands. Unaffected by I5's finding: the band is entirely Mac-side |
