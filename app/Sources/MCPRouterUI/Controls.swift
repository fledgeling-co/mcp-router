...
public struct ProminentButtonStyle: ButtonStyle {
    ...
    ///
    /// The design of record was read first and **does not settle it by drawing one.** Not one of
    /// the mock's primary buttons is disabled — and because the earlier wording gave a bare `29`,
    /// here is the normaliser behind it, which is the thing a reader needs to re-derive it:
    ///
    /// - **29** — `class` attribute values in `design/mcp-router-console.html` that *begin* with
    ///   the literal `btn primary`. This is the number the first draft of this comment reported,
    ///   and it excludes the six `btn sm primary` variants.
    /// - **33** — `<button>` elements whose class attribute carries both words in any order.
    /// - **35** — any element whose class attribute carries both words; the two extra are
    ///   `<span class="btn sm primary push">` at :2878 and `<span class="btn sm primary">` at :3159.
    ///
    /// **The conclusion holds under all three**: zero of the 35 carries a `disabled` attribute or a
    /// `disabled` class, and the file has no `.btn.primary:disabled` rule at all. Every site is
    /// listed in `planning/evidence/M18-gapfix-2/btn-primary-sites.md`, generated by the script
    /// beside it, and `MockPrimaryDisabledTests` re-derives all three counts from the mock on every
    /// run rather than trusting the literals above.
    ///
    /// Worse than not drawing one, its CSS would not dim one either: `.btn:disabled` at :679 and
    /// `.btn.primary` at :680 have equal specificity, `.primary` is declared second and therefore
    /// wins, so a disabled primary in the mock would keep its accent fill, its accent border and
    /// its `--on-accent` label and differ from a live one only by losing its shadow. That is the
    /// same defect this method exists to close, and it is reported as a fault in the mock rather
    /// than copied into the app (`M31`) — the treatment below is what the mock's own `:disabled`
    /// rule *says*, applied without the cascade accident that defeats it.
    ///
...
public struct StandardButtonStyle: ButtonStyle {
    ...
    /// The control's fill, given its role and whether it is pressed. **`nil` means unfilled.**
    ///
    /// A value rather than an expression inside `makeBody`, for `ProminentButtonStyle.palette`'s
    /// reason: the decision is then assertable without a render, and `ButtonPaletteTests` reads it.
    ///
    /// **The destructive control is unfilled, because the design of record draws it unfilled.**
    /// `design/mcp-router-console.html:682` is
    /// `.btn.destructive{color:var(--fail-ink);background:none;box-shadow:none;}` — it keeps
    /// `.btn`'s `--line-strong` bezel (:672, which it does not reset) and drops the background.
    /// This style painted `--raised` behind every role, which in the light appearance is `#FFFFFF`
    /// on a `--panel` sheet ground: a second *filled* control on a surface the brief allows one on
    /// — *"a destructive alternative is a quiet red text button, never a second filled one."* M18
    /// moved the label to the role and left the fill; this is the other half. (M18 verdict,
    /// Finding 2.)
    ///
    /// Pressed goes to `--f1`, which is the mock's own `.btn.destructive:hover` fill at :683 rather
    ...
    /// destructive is declared second, so its `background:none` wins there as well. Unlike the
    /// primary case `M31` records, that is not a defect to route around — the label tier still
    /// moves from `--fail-ink` to `--t4`, so the two states stay distinguishable without the fill
    /// carrying any of the work.
    public func fill(role: ButtonRole?, isPressed: Bool) -> ColorToken? {
        if role == .destructive { return isPressed ? .f1 : nil }
        return isPressed ? .raised2 : .raised
    }
    ...
    public func makeBody(configuration: Configuration) -> some View {
        ...
        Label(scale: scale, tier: labelColour, surface: fill, configuration: configuration)
    }

    private struct Label: View {
        let scale: ControlScale
        let tier: (Bool, ButtonRole?) -> ColorToken
        let surface: (ButtonRole?, Bool) -> ColorToken?
        let configuration: Configuration
        @Environment(\.isEnabled) private var isEnabled
        ...
        var body: some View {
            configuration.label
                ...
                .background(
                    RoundedRectangle(cornerRadius: scale.cornerRadius, style: .continuous)
                        // `.clear` where the style resolves to no fill: the shape stays, so the
                        // bezel, the corner radius and the hit area are unchanged and only the
                        // paint goes. The lint permits `Color.clear` by name — transparency is a
                        // layout decision rather than a palette one.
                        .fill(surface(configuration.role, configuration.isPressed)?.color ?? .clear)
                )
                .overlay(
                    RoundedRectangle(cornerRadius: scale.cornerRadius, style: .continuous)
