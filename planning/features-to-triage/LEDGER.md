...
| M23 | The mock-to-SwiftUI conversion contract | `M23-mock-to-swiftui-contract.md` | — | — | **Needs More Work** (8th) — gap-fix 8 queued; verified at `effect-witness`. The engine change is confirmed the right call, bounded to one hunk, and a 19-shape differential proves nothing else moved. **One blocker: the `145` correction landed in one of three places in its own file** — the pass raised to fix *one fact under two wordings* committed exactly that. One grep closes it |
