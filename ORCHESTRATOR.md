...
| **P1** | **Make the two auth routes reachable** | router | R3 ✓ R5 ✓ | — | Opus | **Wave 1** | `ai/p1` | `D-j` + `D-r2r-c`. Both routes implemented and unreachable: dispatch never routes to them, so they answer 405 where the reference answers 409/400. **Blocks 2 parity rows.** Retiring the differential's stale known-defect assertions is part of the same change, or the gate goes red on a defect that has just been fixed |
| **M13** | **The scroll-edge separator, A34** | mac | M1 ✓ M6 ✓ M11 ✓ | — | Opus | **Wave 1** | `ai/m13` | **The only genuinely red product check on main.** Revealed, not caused, by M11. **The diagnosis is the first job**: broken separator and mis-sampled row are indistinguishable from the failure message, and if the check is wrong it is the check that changes |
| **V1** | **Re-run the out-of-family review on the router items** | review | R3 ✓ R2-W ✓ | — | Opus | **Wave 1** | `ai/v1` | Owner's note overrides the lane: **grok-4.6 at high, not codex**, which is account-limited. Lane probe-verified by the orchestrator before scheduling (exit 0, binary at `~/.grok/bin/grok` 1.0.3). Scope is **R3** (whose critic never ran at all) and **R2-W** (in-family). Read-only, so it shares no files with any other runner |
| P2 | The `import` verb and the config rewrite | router | R2-R ✓ R2-W ✓ | — | Opus | Wave 2 | — | `D-k` + `D-w2`. **Blocks 3 parity rows.** The rewrite is a cross-process write; use R2-W's sidecar flock rather than inventing a second scheme |
...
| P4 | Derive the manifest rows; fix the normaliser | harness | R4 ✓ | — | Opus | Wave 2 | — | `D-n` + `D-o`. **42 of 82 rows are hand-written, so deleting one raises the coverage figure** — the gate's own worst failure mode. And the normaliser's character class omits `-`, so the verdict depends on the directory name it is read from |
...
| **R4-C** | **The installer cutover** | router | P1 P2 P3 P4 · **82/82** | — | Opus — never downgrade | **Wave 4 — last** | — | The owner chose `finish-first`, so the switch is licensed **only once parity reads 82 of 82**. Currently 71 from the repo root. Flips `install.sh` to the Swift binary and retires `src/*.ts` |
...
| **M13** | **The scroll-edge separator, A34** | M1, M6 | **A live red gate on `main`**: `mac-shell.sh` exits 1 at A34 — *"the top row is not one colour once scrolled (`#2F2F2F` covers 0.707) — that is content, not a separator"*. `main` never reached it, having stopped at A22 since M3. **Revealed, not caused, by M11**: its diff over `ScrollEdge.swift`, `ShellWindow.swift` and `Boards/` is empty, verified by the orchestrator. Mechanism is the one the script's own comment predicted — A34 is driven on Servers as "the first destination still using the shell's scroll view", and M6 landed the last board, so its rows now scroll under the sampled row. Settling whether the separator is broken or merely mis-sampled means choosing a new measurement site across boards M11 did not touch |
