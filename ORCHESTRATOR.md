...
| D-g3-a | Two fixed sleeps left in `PoolTests.swift`'s P2a, both vacuous-on-loss | new item | `:92` sleeps 20ms before opening the transport gate and `:98` sleeps 50ms before asserting the superseded session was closed. Neither can report a failure that is not there — losing the first lets the start install and shutdown force-reaps it, so the assertion holds either way, and the second is dead weight because both `pool.shutdown()` and the lease attempt are already awaited above it. Left because the failure mode is **proving less**, not going red, which is a different item from G3 |
