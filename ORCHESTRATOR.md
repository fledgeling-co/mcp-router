...
- One skill is vendored and four are not. `vendor/test-campaign/` holds **0.9.2** pinned to
  `28ecd67` of fledgeling-plugins, and the four campaign gates run from it — `vendor/README.md`
  carries the pin, the checksum and the commands. `mockup-fidelity`, `mac-craft`, `design-craft`
  and `ux-craft` exist only inside the `.claude/plugins/fledgeling-plugins` submodule, which no
  worktree may populate (DISPATCH — WORKTREE SUBMODULE), so they depend on the machine. This line
  used to say all of them were vendored; the submodule pin carries `test-campaign` **0.5.0**,
  which lacks two of the four gate scripts outright.
...
Its citation named **no tree**, and that is what makes it unrecoverable: read at the tree this
paragraph was written on, `against it, with the downgrade logged.`, `ORCHESTRATOR.md:219-232` at
`cb14f5a`, is another item's text entirely. `main` moved to `1f5ad55` while the runner worked and
the text it meant is now at **`:350` — 131 lines off, landing on another item's verdict line.** Its
own words: *that is the defect I was fixing, arriving on schedule.*
...
### THE STATUS FIELD NEVER ADVANCED, AND THE FLEET SCHEDULES FROM IT

Found 2026-08-23 by M16's verifier as a **programme-wide** observation, verified here and fixed in
both files.

`M17`, `M18`, `M19`, `M20` and `M22` all read **`To Do`** in their status column while four of them
had been built, gap-fixed and verified. This orchestrator had been appending rich narrative to each
row for a full day and **never touching the field a reader scans.**

**Same shape as everything else in this register**: the field that reads as the answer was wrong
while the detail beside it was right. Worse here, because `ledger-reconcile.py`'s check I states the
consequence exactly — *the fleet schedules from ORCHESTRATOR, so a row stale there dispatches a
runner to rebuild shipped work.*

**And fixing one file is how the gate caught it.** Updating `LEDGER.md`'s statuses turned check I red
on six items, because `ORCHESTRATOR.md` still said `To Do`. A half-fix made the disagreement visible;
neither file alone could.

**So: advance the status field in the same edit as the narrative, in both files.** A verdict is not
recorded until the field says so.
...
`harbourmaster` **0.1.0**'s SKILL.md at line 85 — *roughly the cores the job will want; a single
test is 1, a `-j`-parallel build is 4-8*. Version rather than tree, because that skill is not in
this repo: it lives in the plugin cache, and the line moved to 94 at 0.5.0 and 96 at 0.6.0. Measured here, `make lint` is **0.94 cores for 74 seconds**, which is a
...
|---|---|---|
| `ai/r19` | `parity-manifest-check.sh` — 96 rows against the pin `rows: 94`, `surface.tsv:3` at `38bc724` | `main` contributes two `div-m22-*` rows, the branch two `overlap` rows |
...
The union of `ai/r19` with today's `main` puts **96 rows** against the pin `rows: 94`, `planning/parity/surface.tsv:3` at `38bc724`, so **`parity-manifest-check.sh` exits 1 on the merged tree while both parents exit
...
- `npm install && npm run build` in the worktree first, or symlink `node_modules` and `dist`
  ...
  and "All run in this worktree, with `node_modules` and `dist` symlinked to the main checkout", `planning/evidence/P7-acceptance.md:160` at `87f6f2e`.
...
| M18 | `.worktrees/M18` | `ai/m18` | 8 commits, `6b620b3` | **cut 08:55** | held for recovery. Its **gap-fix never began** — the only dirt is the two brief files. `GAPFIX-BRIEF.md` is in the worktree and still the work to do: `swift build` exits 1 at `.foregroundStyle(ColorToken.onAccent.color)`, `Controls.swift:111` at `c8bbea8` |
...
| M17 | `.worktrees/M17` | `ai/m17` | done | merged | — |
...
`R21` edits `try? ManifestIO.save(manifest, toPath: manifestPath, fileSystem: fileSystem)`, `AuthRoutes.swift:120` at `1d95734`, which is one of R19's eight manifest write sites, and R19 is
...
| F5 | Skill version usage history and telemetry | feature | — | — | Opus | **To Do** | — | — |
| F6 | Marketplace auto-update checker | feature | — | — | Opus | **To Do** | — | — |
| F7 | Session analyser and skill recommender | feature | — | — | Opus | **To Do** | — | — |
| F8 | Cross-harness skill profile sync | feature | — | — | Opus | **To Do** | — | — |
...
| P9 | `make parity-regen` cannot run, so the vectors are unverifiable against the reference | router | — | — | Opus | **Merged** | — | `c735ab1` |
| P10 | A hand-pinned row count went stale; make parity-selftest red on main | parity | — | — | Opus | **Merged** | — | `9259d48` |
| P11 | Nothing prevents a sixth vector that re-types its reference | parity | — | — | Opus | **Merged** | — | `415b1da` |
...
| M31 | The design of record cannot draw a disabled primary | design | — | `design/mcp-router-console.html` | — | **Merged** | — | `8c417a9` |
...
| M35 | `mac-shell.sh` hand-names two facts the app has since changed | harness | — | — | Opus | **Merged** | — | `423d74b` |
| M36 | The phone control ladder dims a disabled primary to a token `DESIGN.md` §3 does not ratify | phone | — | — | Opus | **Merged** | — | `0fda91a` |
| M37 | Upstream server disable and pause action | mac | — | — | Opus | **To Do** | — | — |
| M38 | Staged tool diff visual review | mac | — | — | Opus | **To Do** | — | — |
| M39 | Pairing QR presentation and expiry countdown | phone | — | — | Opus | **To Do** | — | — |
...
| X7 | The campaign's published artifacts under-report what it knows | harness | — | — | Opus | Untriaged (**upstream**: fledgeling-plugins, not this repo) | — | **The blocker changed shape on 2026-08-22 and the item did not move.** G5 vendored test-campaign 0.9.2 to `vendor/test-campaign/`, so the scripts are editable from this repository for the first time — but an edit that lands only here is a fork of the instrument, and a script behaving one way in this repo and another way everywhere else stops being evidence. Closing X7 still means a change pushed to fledgeling-plugins, after which `vendor/README.md`'s pin moves to the new commit. G5 touched neither X7 nor X8. |
| X8 | Two campaign detectors report findings they cannot support | harness | — | — | Opus | Untriaged (**upstream**: fledgeling-plugins, not this repo) | — | Same shape as X7: editable here since G5, closable only upstream. |
| X9 | Upstream: a probe reports a present file as missing | evidence | — | — | — | **Untriaged** (upstream) | — | — |
...
| G5 | Vendor the `test-campaign` version the gates actually run | harness | `ai/g5` | — | Opus | **Merged** `34b9cf8`** — 2026-08-23, **gap-fix 5 applied** | — | **Delivered.** `vendor/test-campaign/` is 0.9.2 at `28ecd6753386ff6d480a98d6646a5b73c62dc299`, byte-identical to the cache copy and to `git archive` at that commit; `vendor/README.md` carries the pin, the tree checksum and the four gate commands. Reproduced from a fresh clone of `ai/g5` with the submodule uninitialised and `~/.claude/plugins` denied under `sandbox-exec` plus an empty `HOME` — `campaign.py check` exit 0, **69 pass · 3 fail · 4 n/a of 76, armed 69/69**; `strict-check.py` **62 of 76 (82%)** against a ratchet of 58; `capture-lineage.py --gate` **16 of 16 judged**; `vacuity-check.py` **findings=1 (REQ-016)**. Identical to what `0.9.4` gives on the same data. The cache was denied rather than renamed, because four agents share it. **The trap this removes, measured:** the submodule's 0.5.0 prints **58 of 76 (76%)** on the same registry — the exact figure `strict-ratchet.json` records — while having no notion of the `effect-witness` rung, so it drops all four such cases and reads as agreement. **The trap is not the submodule pin's alone, corrected 2026-08-22: `0.9.1` reads 58 too**, having no `effect-witness` rung either, so every version through 0.9.1 prints the recorded figure and reads as agreement. The consequence is that **the vendored 0.9.2 does not reproduce the campaign's recorded strict figure** — it reads 62 where "69 of 76 passing and 69 of 69 armed; `strict-check.py` holds at 58 of 76 (76%).", `RUN-2026-08-20.md:589` at `1d4ce95` records 58, because that recorded reading came from a sitting headed 0.9.1. It is still the right pin: a pin that reproduced 58 would reproduce the blindness that produced it, and 0.9.2 is 0.9.1 plus the DEF-048 fix the campaign itself requested. What should move is the recorded figure, which is the campaign owner's to raise. **The strict +4 is the instrument, not the registry** — 0.9.1 on today's `cases.json` reads 58 and 0.9.2 reads 62; the `capture-lineage` +1 *is* the registry, that script being byte-identical across the two. Filed 2026-08-22 from **M28/DEF-057, answered by the owner**: vendor `test-campaign` only. Measured — vendored `plugin.json` says `0.5.0`, the cache the gates actually ran says `0.9.2`, and `plugins/test-campaign` is **8.9 MB** against `.claude/plugins/fledgeling-plugins`'s **546 MB**. 0.5.0 has five scripts and none the campaign depends on (`vacuity-check.py`, `capture-lineage.py`, `effect-witness`, `blindVocabulary` all absent), so a fresh clone following the documented `--init --recursive` reproduces none of the campaign's numbers while `LEDGER.md` claims it can. **The docket predates the second cost**: populating the submodule in a worktree breaks every runner sent into it, which cost three launches on 2026-08-22 and is now a dispatch hazard row above — so the vendoring claim describes something this fleet deliberately avoids. Rejected and recorded: bumping the whole submodule keeps 546 MB and the hazard; dropping the claim costs reproducibility outright. Acceptance requires the cache fallback be **disproved** by renaming it away, because a path that silently falls back is this item's own defect class. **X7 and X8 do not close** — vendoring makes them editable here for the first time, and an edit that never reaches upstream is a fork rather than a fix. Progress at `planning/progress/G5.md`, gap-fix 1 at `planning/progress/G5-gapfix.md`, gap-fix 2 at `planning/progress/G5-gapfix-2.md`, gap-fix 3 at `planning/progress/G5-gapfix-3.md`, gap-fix 4 at `planning/progress/G5-gapfix-4.md`, gap-fix 5 in `planning/progress/G5.md`. **Gap-fix 4** dropped the `installed <version>` label at twelve sites in six documents — a reading belongs to a named version, and which version is installed is read with a command rather than quoted — and committed `planning/installed-claim-sweep.py`, which fails on any such binding in prose and carries a `--selftest` of 105 cases. **Gap-fix 5** closed the verdict's seven-item work order, all of it record and instrument: the guard's word boundary could not see the word inside markdown emphasis, so the one site written in that form escaped every class including the one that only counts — the shipped fix separates on an underscore run only when the run carries no letter or digit on its far side, which keeps identifiers like `installed_plugins` out where the obvious widening admits 34 of them, and each half of it is proved by a mutation that reddens it alone. `planning/G5-citation-frames.py` gives the citation-frame presence control the reader and the three predicates it never had, which is what reconciles its `10/10` with the same note's *nine of the ten*. Two swiftformat `skipped` denominators and two backtick counts were quoted for commits they are not properties of. The vendored tree, its checksum and every gate number are unchanged and were re-derived. |
| G6 | Evidence kept in `/tmp` is not evidence | harness | — | — | — | **Merged** | — | `6ce5ee1` |
| G7 | A citation that does not resolve where it is read | harness | — | — | — | **Merged** | — | `4f54935` |
...
| G9 | Two gates `cd` into a worktree that no longer exists | harness | — | — | — | **Merged** | — | `.worktrees/R2` is gone; both scripts exit 90 forever, and `spec-R2.md` cites one as runnable. Mitigated by `\|\| `171bcce` |
| G10 | `make acceptance` dies at its first lane | harness | — | — | — | **Merged** | — | `9259d48` |
...
| R28 | The router owns skills, plugins and marketplaces | router | — | — | Opus | **Merged** | — | `784dde9` — 17 tests in 2 suites; acceptance lane 31 ok / 0 failed, disposed `af8417a` |
| R29 | Push a reload to live sessions | router | — | — | Opus | **Merged** | — | `b80c638` — `notifications/tools/list_changed` over the standalone GET /mcp stream refreshes a live session in 87ms end-to-end, no model, no slash command. Socket half only **asks** (receiver enqueues with slash commands disabled); `notifySessions` ships off. TypeScript router only — Swift answers 404 on `/sessions`, porting is R4's parity surface |
| R30 | Ingest what Claude acquires | router | — | — | Opus | **Merged** | — | `516be61` — `mcp-router ingest`, copy→verify→remove, reversible, nothing deleted. **Never run against the real `~/.claude`** per the owner's standing "prepare it, don't run it"; proved on a fixture tree. 35-assertion lane |
| R31 | Invalidate the caches a change outruns | router | — | — | Opus | **Merged** | `ai/r31` | `567b5a1` — content component **beside** the identity key, not replacing it. Measured: 15 of 21 stdio upstreams can change code with command/args/env byte-identical. `GET /caches` plans by default; a wholesale clear answers 409 carrying the byte figure. `~/.npm/_npx` is never cleared wholesale |
| R32 | Claude Desktop has no live reload | router | — | — | Opus | **Merged** | — | `034251b` — Desktop's schema requires `command`; there is no `url` and no `type`, so the HTTP router **cannot** be registered today and the verb refuses rather than writing a false entry. Reload path is a Developer-menu item, recorded `locatedNotExercised` |
...
and no fuller log exists. G3's `expect(await !pool.isLive("a"))`, `PoolReapingTests.swift:61` at `b7153e1` is the strongest candidate: it is the
...
knowing the machine's load** — under that saturation `expect(await !pool.isLive("a"))`, `PoolReapingTests.swift:61` at `b7153e1` would have failed
...
into any work on that suite: `expect(result.tools != nil, "the success scenario reported no tool count")`, `ShellTests.swift:133` at `9a40ca8` currently reads
...
Copied to `/tmp/mcp-router-setaside/` (gone) before removal rather than deleted outright.
...
| M12 | Staleness and an as-of time inside a destructive dialog — **Merged** `a040b17` again 2026-08-23** on `ai/m12` after gap-fix 1, not merged. Verified `Needs More Work` at `3367f72`: the product was sound — thirteen clauses held and six mutations went red under the verifier's own patch strings — and **five record claims were false**, the blocker being A15c's own C7 row, which claimed to prove *the stamp is absolute* and greped `reading taken $shape` against a sentence hardcoding `at `. All five closed; **one line of `app/Sources` moved and it is a doc comment**. The row is armed by planting now: `'now ago'` renders and the gate exits 1, reverted byte-identical, `25 passed, 0 failed` twice. `make lint` and `make build-mac` both ran **as composed targets** here, which the verifier's worktree could not do. Both dialogs now state an **absolute** as-of time and carry the board's own stale marker; the relative form the plan called for was killed by the out-of-family plan review, because a modal's body does not re-run and "taken 3m ago" would have frozen at whatever it said when the sheet opened — the defect rebuilt one line below the fix | M7 | M7's Phase D findings 4 and 8, both VALID. A `.stale` reading is shown in the present tense with no marker, and calls accruing between load and POST are discarded uncounted — so the figure is a lower bound presented as a count |
...
| R11 | Skills write endpoint (remove/disable) with preconditions and undo | R3 | **Filed as R7 and renumbered 2026-08-21 — that id was already a top-level ledger item.** Cleanup lists absent skills and can offer no action on them, because the control API is read-only for skills. M7's A16 asserts that gap rather than hiding it, and `DisabledAction(label: "Remove", reason: CheckCopy.skillRemoveDisabled)`, `CleanupSheets.swift:204` at `eb784e4` still draws `DisabledAction(label: "Remove", reason: CheckCopy.skillRemoveDisabled)` — so the gap is live and this item is genuinely open |
...
| D-w2 | `?? (NSHomeDirectory() as NSString).appendingPathComponent(".claude.json")`, `ImportVerb.swift:22` at `bb056e7` uses `NSHomeDirectory()` where the reference honours `$HOME` | R4-C | Out of scope for R2-W and currently unreached, because `cli-import` passes `--from`. It diverges the moment anything calls it without one |
...
| **D-r2r-b** ↑ | **Confirmed and WORSE than recorded** | new item | `printf " %-11s %2d of %2d proven", g, state[g"/proven"] + state[g"/by-suite"], seen[g]`, `parity-gate.sh:280` at `6a9bf62` claims the control lane compares on the wire; **that is false for the Swift half.** D1 promoted it rather than half-doing it. R2-R-sized |
...
| **DISPATCH — WORKTREE SUBMODULE** | **`git submodule update --init` in a worktree breaks every runner dispatched into it** | orchestrator | Measured 2026-08-22. Populating `.claude/plugins/fledgeling-plugins` puts **546 MB** of plugin skills where Claude Code loads them, and the runner dies on prompt length. The main checkout survives it only because `.claude/settings.local.json` curates which plugins load, and that file is untracked so no worktree has it. **Every existing worktree leaves the submodule uninitialised on purpose** — `R17` and `R7` both read 0 B. `git submodule deinit -f` reverses it. **G5 does not remove this.** The 546 MB submodule is still declared in `.gitmodules`, still pinned at `baaf79ab`, and still kills any runner in a worktree that populates it; G5 only removes the reason to populate it for the campaign gates, which now read `vendor/test-campaign/`. The four other skills still live only in there. **And the false-confirmation trap is wider than this pin:** the pin's 0.5.0 and **0.9.1 both read 58 of 76** on today's registry, neither having an `effect-witness` rung, so populating the submodule is not the only route to a reading that agrees with `strict-ratchet.json` while measuring less. **0.9.1 names a version, not the cache's current state** — the plugin cache held 0.9.1 when the campaign's recorded sitting ran. **What it holds now is not a fact this repository can hold.** It is a shared directory four agents write to, and the last version quoted into this row was false eighty minutes after it was written. Read it instead of quoting it: `jq -r '.plugins["test-campaign@fledgeling-plugins"][0].version' ~/.claude/plugins/installed_plugins.json`. What is fixed is the reading **per named version**, each taken against a `/tmp` copy of the registry at gap-fix 2 and re-taken at gap-fix 3: 0.5.0 and 0.9.1 read **58 of 76**, and 0.9.2, 0.9.3, 0.9.4, 0.9.6 and 0.9.8 all read **62 of 76 (82%)** against the ratchet's 58. The vendored 0.9.2 is the one reading that cannot move, because it is checksummed at `b9acf616…`. The trap is a property of every version through 0.9.1, and it is behind the cache rather than in it. The vendored 0.9.2 reads 62 and therefore does not reproduce the campaign's recorded figure — the recorded one is pre-0.9.2. See the G5 row. |
...
| D-r10-b | `import` still prints the false green `index` just lost | R7 | `Out.print(" ok \(name) (\(outcome.tools) tools)\n")`, `ImportVerb.swift:109` at `c96b20f` prints `ok <name> (N tools)` and reads neither `cacheFailure` nor `heldChanges`, so it can adopt a server and exit 0 with no manifest row. Fenced out of R10 expressly; verified unchanged |
...
| D-m23-bd | Gate B is in every brief's standing gate list and its recipe exists nowhere | M23-gapfix-7 | Named once at `(116 breadth + 16 copy), gate B exit 3 with a ledger written, selftest exit 0 with all`, `M23-gapfix-2.md:121` at `2455774` and carried forward as *B 3 with a ledger written* through four briefs with no surface, command or fixture recorded. The verifier constructed an equivalent and states plainly that is not the same as reproducing it. A gate in a standing list that nobody can run is a claim, not a check |
| D-m23-be | `make test` red once at `OAuthWireTests.swift:263` as of `64195dd` | D-g3-c | *A waiter that is cancelled is resumed rather than stranded*, exit 2 with one issue at 1615/202, green on the re-run. A cancellation-timing test on a machine concurrently running another session's Swift builds — G3's class, not M23-adjacent |
...
| D-g3-s | Three fixed-sleep flakes observed red outside the three surveyed suites | D-g3-c | `@Test("termination 4 — a failing token exchange serves 500 with the thrown message")`, `CallbackListenerTests.swift:108` at `9420fcf` (150 ms at `:101`) red 2 of 3; `OAuthWireTests.swift:263` at `64195dd` (3 s at `:261`) **red three times now** — once as first recorded, once during G4's first verification at load 900-945, and once during R17's gap-fix 4 at load 625-1015, green on the retry each time. **Recorded as three events rather than as a rate**, deliberately: `D-r17-d` spent three passes stating rates the next measurement refuted, and this row is not going to repeat it. All three reds are on a loaded box and none has been reproduced on a quiet one, which is a statement about what has been observed rather than about what the test does; and **`@Test("a listener binds once — reuse is refused rather than quietly racing")`, `CallbackLifecycleTests.swift:238` at `5610cd2` red once in gap-fix 2** — *the callback listener was cancelled before it bound*, in a file carrying three fixed sleeps including a 50 ms one at `:217`. **Two more in gap-fix 3**, at 0.0% idle and 1-minute load 421: `@Test("a listener binds once — reuse is refused rather than quietly racing")`, `CallbackLifecycleTests.swift:238` at `5610cd2` again, and `ControlStreamTests.swift:72` at `64195dd` — *(arrival → …) < (lastSent → …)* comparing two timestamps that round to the same second. Five measured instances in four files now, two of them red on 2 of 4 `make test` runs in one session. All at 0-11% idle with two sibling runners live, and none in a file this item touched. `D-g3-c` predicted around sixty unclassified; three are now measured rather than surveyed, in three different files |
...
| D-r7-t | A dialect regression crashes the test bundle instead of failing it | G1 | `expect(found.duplicates[0].harnessName == "Mobbin")`, `HarnessDialectTests.swift:163` at `5610cd2` indexes `found.duplicates[0]` unguarded, so the mutation that empties it aborts the suite at signal 5 and ~1600 tests never run. The gate still reddens, but the diagnosis names an index rather than the assertion |
...
| VER2-R17-3 | The same read-window divergence is readable from source on `index` and `import`, and is filed as merely unmeasured | R19 | Swift routes every upstream through `ManifestIndexer.index` to `record`, re-loading at `var manifest = ManifestIO.load(path: manifestPath, fileSystem: fileSystem).manifest`, `ServicePorts.swift:381` at `afc087f` and saving at `:391` **per entry**; node's `cmdIndex` loads once at "const manifest = loadManifest(manifestPath);", `src/index.ts:177` at `afc087f` and saves once at `:186`, and `cmdImport` at `:101` and `:146`. The same disagreement the declaration covers for `watch`, on the writer R19's own reproduction used. Established by reading rather than measuring |
...
| D-g5-a | The absent half of G5's sweep was never repository surface, and its corpus was four hand-listed files | new item | Registered 2026-08-22 at G5's second verification. Leaving the 27-string checker at `/tmp/g5gf/sweep.py` (gone) uncommitted is right for its **present** assertions — a checker for one commit's prose is not repository surface. Its two **ABSENT** assertions are a different job, and they are `planning/claim-sweep.py`'s: that tool is on `main` and **not on this branch at all**, so the withdrawn claims were checked over four hand-listed files rather than a corpus. The verifier ran the corpus check that tool would have run, over **1366 tracked files**: `10da833` survives only in `G5-gapfix.md` and in `G5.md`'s explanation of the amend, and the inverted attribution sentence only where it is quoted as withdrawn. **Nothing leaked, and that is luck confirmed afterwards rather than coverage.** Registered rather than fixed: bringing the tool onto this branch is not a documents-only change. |
| D-g5-b | G5's sweep has a per-file fixed point, and its note implies a general one | new item | Registered 2026-08-22 at G5's second verification. `G5-gapfix.md` states that a phrase reproduced unwrapped in the document it guards stops being a control. Wrap control W1 *is* quoted unwrapped, at `G5-gapfix.md:84`, and that is safe **only** because the sweep keys each control to one file and W1 guards `G5.md`. **A corpus-wide sweep would collapse on it**, matching its own quotation. The property is per-file rather than general and the doc should say so; left as written because the sweep it describes is correct as built. |
| D-g5-c | `strict-ratchet.json` holds `"checked": 58, "total": 70` and should read 62 | campaign owner | Registered 2026-08-22, already recorded in `planning/progress/G5.md`, the G5 row above and the `LEDGER.md` G5 row; given an id here so it can be pointed at. The four `effect-witness` cases are armed and passing, and every instrument from 0.9.2 on reads **62 of 76** — 0.9.2, 0.9.3, 0.9.4, 0.9.6 and 0.9.8, each measured by name at gap-fix 2 and re-measured at gap-fix 3. Which of those the cache currently serves is deliberately not recorded here; it moves. The ratchet records a pre-0.9.2 figure. **`planning/test-campaign/` belongs to another session, so G5 does not touch it.** |
| D-g5-d | **Four documents in ten places** bound a version number to the word *installed*, and the tree behind that label carries two manifests that disagree | label fixed at G5 gap-fix 4; manifests upstream | Found 2026-08-22 at gap-fix 2, **count corrected at gap-fix 3**, **label fixed at gap-fix 4**. The row previously said *two* documents and named `G5-gapfix.md`'s four-version table and the `LEDGER.md` G5 row. Four carry it, one being **the G5 row above in this same file**. The ten sites, **cited at `a9603e5`** — the frame they were found in: `claims it can. **The docket predates the second cost**: populating the submodule in a`, `ORCHESTRATOR.md:314` at `a9603e5`, `reproduces none of its numbers. **A second cost the docket predates**: populating that`, `LEDGER.md:67` at `a9603e5` and `:295`, `Fresh clone, cache denied`, `G5.md:76` at `a9603e5`, `:94`, `:123`, `:143`, `:159` and `:163`, and `G5-gapfix.md:22`. The frame is not decoration. Six of the ten are in `G5.md`, gap-fix 3's own edit added eleven lines above them, and all six stop resolving at `43b44a2` while reading as ordinary line numbers — a bare citation resolves in the revision it was written against and then fails silently. **Nine of the ten bound a number to *installed*. The tenth, `G5.md:163` at `a9603e5`, names `0.9.4` without the word and was already correct**, which is the distinction the fix turns on. Two things were wrong with the label. **It named an instant, not a state** — `installed_plugins.json` read `0.9.4` on 2026-08-21, `0.9.8` at 2026-08-22T07:38:26Z and `0.9.9` at 2026-08-22T09:54:03Z, so re-numbering the ten places would only have restarted the decay. Gap-fix 4 dropped the word instead, at **twelve sites in six files** — the nine above, this row's own heading, and two more the count never reached: "Vendored `plugin.json` says `0.5.0`; the cache copy every gate this campaign reports", `M28-decision-docket.md:244` at `6a3dbb8` and `What every gate this campaign reports actually ran, from the machine's plugin cache`, `G5-vendor-the-campaign-version-the-gates-run.md:17` at `6a3dbb8`, both binding `0.9.2` rather than `0.9.4`, which is why a sweep keyed on `0.9.4` had not found them. Read the cache rather than quoting it, per the dispatch hazard row above; `planning/installed-claim-sweep.py` now exits 1 on any `installed <version>` binding in prose, keyed on a version-shaped token so re-numbering a caught site to the next release trips the identical row. **And it is checkable two ways with two answers**: in the `0.9.4/` tree, `.claude-plugin/plugin.json` reads `0.9.4` while the top-level `plugin.json` reads `0.9.3`, an upstream packaging slip rather than this repository's, and the `0.9.3/` and `0.9.4/` skills trees are byte-identical but for one `.pyc`. **The readings themselves stand**: the `0.9.2`, `0.9.3`, `0.9.4`, `0.9.6` and `0.9.8` trees all read **62 of 76 (82%)** on this registry, measured at gap-fix 2 and re-measured at gap-fix 3, so only the label is wrong. The disagreement **recurs at 0.9.8** — top-level `0.9.7` against `.claude-plugin` `0.9.8` — so it is an upstream pattern rather than one slip. Gap-fix 2 was scoped to three named corrections and gap-fix 3 corrected this row's **count** and left the ten labels alone, on the reasoning that a moving reference is withdrawn rather than re-numbered. That reasoning licenses withdrawal and does not license leaving the number asserted, which is what gap-fix 4 acted on: the readings are records of a past measurement and survive intact once the adjective goes, because a reading belongs to a named version. **The label half is closed. The disagreeing manifests are not** — they belong upstream with X7 and X8, and are what keeps this row open. |
| D-g5-e | A guard keyed on `\b` cannot see a word its own corpus writes in underscore emphasis | new item | Registered 2026-08-23 at G5 gap-fix 5, and **generic rather than G5-specific**. `_` is a word character to Python's `re`, so `\binstalled\b` does not match inside `_installed_`; the site at "Corrected at gap-fix 4.** The last row bound `0.9.4` to the word _installed_. The reading", `planning/progress/G5-gapfix.md:24` at `c0a9837` escaped **every** class of `planning/installed-claim-sweep.py`, `role` included, so a sentence written in that emphasis was not merely unblocked but uncounted, and the guard that missed it was committed in the same pass that wrote it. G5 fixed its own guard and proved both halves of the boundary by mutation. **The property is `\b`'s, not this guard's**, so any other sweep in this fleet that greps for a word has it — and the fleet greps for words often. `G7` covers citations that do not resolve where they are read; this is its sibling, an instrument that cannot read its own corpus's emphasis style, and it wants a survey of the existing guards rather than a fix here. The narrow reading matters too: the obvious widening to `(?<![A-Za-z0-9])word(?![A-Za-z0-9])` then reads the word inside identifiers — measured on this corpus at 34 further sites, all of them `installed_plugins`, `INSTALLED_COUNT`, `board_registry_installed` and kin — which is the same defect pointed the other way. |
| D-r17-a | `VER2-R17-3` — the manifest read-window divergence is readable from source on the `index` and `import` verbs, and is filed as merely unmeasured | R19 | Swift routes every upstream through `ManifestIndexer.index` → `record`, which re-loads at `var manifest = ManifestIO.load(path: manifestPath, fileSystem: fileSystem).manifest`, `ServicePorts.swift:381` at `afc087f` and saves at `:391` **per entry**; node's `cmdIndex` loads once at "const manifest = loadManifest(manifestPath);", `src/index.ts:177` at `afc087f` and saves once at `:186`, and `cmdImport` loads at `:101` and saves at `:146`. That is the same read-window disagreement `surface.tsv`'s `cli-watch` note declares for `watch`, on the writer R19's own reproduction drove — so it is a second instance of a declared divergence rather than a new one. **Established by reading the source, not by measuring it**, and said that way deliberately: `parity-cli.sh` runs the two binaries sequentially and cannot reach the property. Raised by R17's second verification, registered rather than fixed |
...
**`make test` is green on main.** The two failures recorded earlier today — `OAuthWireTests.swift:263` at `64195dd`
and `let child = try #require(pid, "the stub should have recorded its pid before the timeout")`, `RealProcessTests.swift:93` at `850bb7a` — do not reproduce on an idle machine. Both are wall-clock-bound
...
- 2026-08-25 — **A session of gate repairs, briefs and audits. No item was triaged, planned, branched, verified
  ...
  **Wave 3 launched (`wf_1fc79b9d-147`), four agents on 10 free berths at 0.38 load/core**, inner waves capped at 3
  ...
  - **`runner:G6`**, carrying the two corrections this orchestrator measured and the brief lacks — `/tmp` is cited in
    **durable specs** (`spec-P4.md` :43/:85/:300, "effort, read-only, wire-verified — `/tmp/gate-F1-cri", `planning/specs/spec-F1.md:166` at `03c34c3`), not merely in transient logs, and the
  ...
  **`G7` → Developer Review.** A citation gate with **seven classes partitioning the set**, wired into `make lint`,
  ...
  anchor sentence at `M28 docket and the X7 brief all name, and it reads what 0.9.4 reads on this data. What`, `G5.md:159` at `b1c974c` is no longer in the file at all, so renumbering could not have repaired it even in
  principle.
  ...
  **Untriaged → To Do**, each verdict appended to its ledger row (append-never-assign, per-cell and whole-file length
  ...
  (`swift test` exits 1 naming `MCPRouterApp.swift` line 234 column 45 **of the mutated tree** — the fault
  adds the line, so no committed revision carries it; the file's 14 revisions top out at 232 lines — and 0 at
  1941 tests without it). Grounding
...
- 2026-08-15 — **M7 merged `85d8331` — seven of eight panes.** The `<<<<<<<` SourceKit reported at
  `// M8's Settings pane.`, `ShellTestSupport.swift:161` at `43633e0` was a **stale index artefact, not a conflict**: no markers in either
...
- 2026-08-14 — **The parity gate's verdict depends on the name of the directory it is run from
  ...
  number was wrong about the code and neither runner was careless: `(r'"project":"[A-Za-z0-9]+"', '"project":"<project>"')`, `parity-fixture.sh:121` at `cd3be8d`
...
- 2026-08-14 — **Twelve unregistered work items found and registered — the R2-R failure repeating,
  ...
  through `D-n` lived only inside `spec-R2.md`'s and `plan-R2R.md`'s own tables. "`R2-R` is registered nowhere.** `spec-R2.md`'s deferred-children table registers `R2-W`,", `spec-R4.md:68` at `dea2493`
  ...
  runs at `bash "$REPO_ROOT/scripts/acceptance/parity-manifest-check.sh"`, `parity-gate.sh:49` at `3da1e23` and derives the **control** and **fixture** rows from source, so a
...
## 2026-08-26 · The verify wave: five for five, NEEDS MORE WORK

Five branches sat READY-TO-VERIFY for three iterations while the goal guard blocked on an
unchanged `[branches, worklist]` failing set. The change of approach the guard asked for was to
stop dispatching runners and start dispatching **verifiers**. Three fresh-context agents, grouped
by subject, graded all five. **None passed**, and not one verdict is a matter of taste — each names
a defect the verifier reproduced on its own run.

| Item | Branch | Verdict | What blocks it |
|---|---|---|---|
| G8 | `ai/g8` +5 | NEEDS MORE WORK | Regresses `reader-accounting.py` (exit 1) on its own "return [f for f in out.stdout.split("\n")", `planning/sweep-control-gate.py:125` at `956bb4f` — a gate broken in the target it wired itself into |
| G9 | `ai/g9` +6 | NEEDS MORE WORK | `runnable-path-gate.py` is invoked by nothing, and unlike the hook install it is **not declared** |
| G10 | `ai/g10` +7 | NEEDS MORE WORK | `9259d48` |
| M31 | `ai/m31` +4 | NEEDS MORE WORK | The sweep's `failures` counter is dead code, so `failures=0` cannot fire |
| M33 | `ai/m33` +4 | NEEDS MORE WORK | Adds its own row to the citation ratchet (10 files → 11) while certifying that gate as fully inherited |

**What the wave proves about the pipeline, beyond the five items.** Every one of these branches
reported itself green. Four of the five carry genuinely strong work — G9's 40/44 measurement
reproduces to the file, M33's poison arming was called the strongest the verifier had seen, G10
proved twice over that a red lane cannot hide in a green total, M31's contrast control both
reproduces DESIGN.md's four published columns *and* discriminates against the 8-bit method. The
gap in all five is the same shape and it is not competence: **the account of the work outruns the
work at exactly one seam per item.** A gate that regresses a neighbouring gate. A gate nothing
runs. A table stale against its own commit. A counter never incremented. A record certifying a
number it did not re-read.

Three of those five are the item's own subject turned back on it. G8 ships an instrument that
does not report what it discards, inside the family "instruments that don't report what they
discard". G9's brief says of the R2 scripts *"Nothing went red, because nothing invoked them"* and
its remedy is a gate nothing invokes. M33 exists to stop a lane reporting success over something
it never read, and its acceptance record certifies a gate it did not re-read. **The pattern is
worth naming because it is not caught by grading harder — it is caught by grading in fresh
context, by someone who did not build the thing.** A builder cannot see the seam between what it
did and what it believes it did; that is what the seam is made of.

Two corrections the verifiers made to the orchestrator's own record, both standing:

- **G8's flagging of three orchestrator-authored goal scripts as UNDISPOSED absence sweeps is
  CORRECT.** Strongest case: `planning/goals/lint-except-citation-ratchet.sh` decides whether
  `make lint` failed *only* on the citation ratchet using `grep -cE 'error:|Violation|FAIL|did not
  pass lint'` with **no presence control** — so a lint failure whose message matches none of those
  four patterns exits 0 while `ratchet: BARE` appears anywhere in the output, and a real failure
  passes unnoticed. Written by the party that spent this session demanding controls. The finding
  stays standing; closing it is the orchestrator's debt, not a runner's.
- **The earlier adjacent verdict that "no normalisation yields 40" was wrong**, and wrong because
  it was drawn without checking out the tree. 40 reproduces exactly as tracked files containing
  `/Users/lukerhodes` over `git ls-files` at `b757a1ac`; 44 at `afd823a`. A number needed a tree.

Also recorded: G10's verifier caught **its own** sampler converting to sRGB and reading #252528
where the truth is #1C1C1E — the exact Display-P3 trap `shells.sh` documents. It said so and used
the lane's raw sampler. And M31/M33's verifier **voided its own first out-of-family call**, which
it had handed the premise "gate failures were strictly inherited" before checking whether that was
true; both recorded verdicts come from re-runs on corrected evidence. On M33 the lanes split —
gemini agreed with NEEDS MORE WORK, grok-4.6 dissented for DONE on the ground that a miscounted
write-up is not the unread-build-description defect. The verifier took gemini's side on the
measurable half (the gate goes 10 files → 11 because of this branch) rather than on the sentence.

**Dispatched in response:** three gap-fix lanes (G8+G9+M33 together, since all three are narrow;
G10 and M31 alone, since both are substantive) plus the two items that had never started — M30
