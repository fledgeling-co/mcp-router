...
| I3 | iPhone Triage, Queue, Library | ios | I1 ✓ I2 ✓ | `?only=phone&tab=triage` | Opus | **In progress** `wf_0e36d4b8-7af` | `ai/i3` | Completes the phone. Two binding product decisions carried into the brief: the phone **queues and never installs** (the Mac Inbox decides), and the **Tinder-style swipe deck is rejected by the owner** — as are Whering's numbered stepper and Apple Mail/Spark swipe-to-reveal. Inherits three known phone gaps knowingly rather than silently: no Dynamic Type scaling anywhere (`TypeToken.font` is fixed and shared with every Mac surface, `DESIGN.md` §2 fixes the eight sizes deliberately), `--attnWash`/`--attnLine` still absent from `ColorToken`, and I1's Dynamic Type test overriding a UIKit trait that measurably does not reach the SwiftUI view |
