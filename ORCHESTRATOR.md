...
| D-m23-s | `layer_copy` measures a pairing the gate has never vouched for | M23 | `D-m23-l`'s confusion one layer over: breadth files an unvouched pair `unclassified` while copy compares the same two strings and reports a mismatch as a measured difference. Deliberately not closed in this pass — filtering copy's population moves the finding count acceptance criterion 6 pins at 16, so it belongs to the pass that re-pins it. `gpt-5.6-sol`, against the shipped diff |
