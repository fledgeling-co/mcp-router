...
| **M14** | **A shipped menu tells the user the app is not built** | M1, M6 | `MenuCommand.swift:320-321` returns `.surfaceAbsent` **unconditionally** for `.pairPhone` and `.exportLibrary`, under a comment reading "Still owned by items that have not shipped". **M6 shipped pairing.** So hovering "Pair iPhone…" reads *"This part of the app isn't built yet."* while `ShellCommandRouterTests` already routes that same command to `.openPairing`. It is also the check now failing in `mac-shell.sh`, which A34 had been masking. `.exportLibrary` is genuinely unshipped, so the fix is **not** simply deleting the string |
