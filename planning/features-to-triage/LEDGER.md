...
| R14 | A client's Authenticate action succeeds, and says which upstreams still need authorising | `R14-client-auth-succeeds-and-reports-upstreams.md` | — | — | **Ready to verify** — built with R15 (`ai/r14` `76cbb53`, 7 commits, both implementations). All gates green including a new 94-check authorization-server parity lane. The panel found four real defects, one an auth bypass the runner introduced — a consent ticket redeemable as an authorization code, reproduced red then green. The report reads tool count rather than `auth.authorized`, and `mobbin` correctly carries no command. `PARITY_CUTOVER_TARGET` (82 against a census of 92) is the owner's to move |
| R15 | The Host check guards `/mcp` and nothing else | `R15-host-check-is-scoped-to-the-mcp-transport.md` | — | — | **Ready to verify** — delivered inside `ai/r14` `76cbb53`. The check moves ahead of the dispatch ladder in both implementations and all eleven routes inherit it; `/mcp`'s 403 stays byte-identical |
