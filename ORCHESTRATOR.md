...
| **P5** | **Close the last three closeable parity rows** | router | D1 ✓ P1 ✓ | — | Opus | **Relaunched** `wf_0cb11f0b-277` | `ai/p5` | **Its wave-6 runner died to infrastructure with the work uncommitted** — `502 lifeline gateway: connect ECONNREFUSED 127.0.0.1:8857` — and none of it had been reported. The orchestrator committed it VERBATIM as `e5ff86a` purely to make it survivable: **unreviewed, ungated, and not a claim that any of it is right.** It marks `cli-auth` and `install-launchd-watch` proven, which are **claims by a dead runner, not measurements**. The relaunch's first job is to verify or revert each. `control-auth-post-http` (D-p1-a) was never reached and is still open. **Brief now says COMMIT AS YOU GO**, because one API error nearly cost the whole run |
...
| **I4** | **Let the phone install directly** | ios | I3 ✓ D-m6-a | — | Opus | **STOPPED — classifier** | `ai/i4` | Owner's own choice, **against the shipped queue-only default and with the threat-model consequence shown and accepted**. Absorbs `D-m6-a`: the pairing round trip has never been proven end to end, and direct install cannot be built on an unproven transport. **Its wave-6 runner DIED to infrastructure, not to the work** — `API Error: Server error mid-response` after ~13 minutes, 144KB of transcript, **nothing committed and the worktree clean**, so no reasoning survived and there was nothing to reconcile. The relaunch brief now says COMMIT AS YOU GO for exactly that reason. **Two further launches were then BLOCKED BY THE SAFETY CLASSIFIER**, both returning `null` at 0 tokens and 0 tool uses. The first was labelled transient, which justified one retry; the second block did not repeat that label and flagged the retry itself as routing around a denial. **The item is STOPPED pending the owner's word — it is not blocked on engineering.** Note the brief is ordinary security-feature work and a near-identical one ran 13 minutes unblocked in wave 6, so the trigger is not obviously the content |
...
| D-g1-e | **Eight parity rows have never been shown able to fail** | new item | The row count is honest and the reddening is not proven for those eight. Directly weakens the 83/83 target |
...
| D-p3-b | `fixture-registry-search` is now `accepted-uncomparable` | R4-C | Not a defect. Recorded so the cutover's 83/83 target is read correctly: this row can never be byte-compared, and pretending otherwise later would be a regression |
