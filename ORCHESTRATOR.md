...
| G3 | `make test` is not deterministically green | harness | — | — | Opus | **Ready to verify** — gap-fix complete | `ai/g3` | **Gap-fix delivered 2026-08-21, ready to verify (`ai/g3` `906e795`, 4 commits on `d544968`).** Property established: **a regression in this class produces a named red inside the CI bound, not a timeout.** The verifier's one blocking finding is closed: `awaitReap` and `awaitSessionEnded` await an unstructured `Task`, so the effective bound on each was the pool's own armed window and P6 configures that at 600,000 ms. Both are now called through **`awaitEvent`** in `PoolTestSupport`, which runs the event under `waitUntil`'s ten-second breaker and reports expiry as a failure naming the condition at the caller's source location. The wait is **abandoned** rather than cancelled — `await task.value` on a `Task<_, Never>` has no cancellation check, so a group around it as it stands awaits the loser and the run still takes ten minutes; an observer records the landing and the poll reads that record. **The assigned mutation reddens in a 10.461 s run where the verifier measured 601.184 s**, at `PoolReapingTests.swift:98` naming *timed out after 10.0s waiting for: `own` to be reaped under the arming it just made* — the `awaitEvent` line, not the `isLive("inherits")` line the 601-second run reported. **The route nobody had named** — arming records the default while the reap uses the requested window — reddens in a 3.916 s run at `:87` on `(armed.idleMilliseconds → 600000) == 25`, because the `#require` throws before the await is reached: the bound is on the right side. Control: the same mutation with P6's wrap removed, killed at 60 s, exits 142 with no summary line and no report from P6. The bound is asserted rather than documented — `PoolAwaitBoundTests` requires every call site in `app/Sources` and `app/Tests` to sit lexically inside an `awaitEvent`, held by **ten controls, five red and five green**. **The panel found the fix carrying its own defect for the fourth time this session.** All three lanes took apart the first scan (counting wraps per function: two wraps and one bare call passes); grok found a `func` whose own signature contains `awaitEvent(` reading as a wrap, plus same-line and nested-block false reds; codex found `// awaitEvent(` posing as an opener and basename self-exclusion. All taken. **Grok also corrected the fix's own reasoning** — a task group is not ruled out, only that shape of one; make the wait cancellation-aware and it works, and `AuthorizationURLBox` in `OAuthFlowStarter.swift` is already that construction, written for the same hang at 91 s against a 20 s budget. The claim is corrected in source and brief; the construction is `D-g3-k`. Gemini's CRITICAL (early return reads as a false green) and its MAJOR (the leaked observer mutates state in later tests) were **refuted by the other two lanes and by measurement**. The one hole no lane found was found by planting a control. `D-g3-j` closed — five test-only members not three, `ReapArming` is the `Sendable` type and `ReapTimer` never was, 960 ms as 3×120 + 2×150 + 2×20 + 3×60 + 80, `D-g3-a` on post-fix `:96`/`:102`. `D-g3-a`…`D-g3-f` stay deferred; `D-g3-g`/`D-g3-h`/`D-g3-i` registered and not taken — `D-g3-i` left undone with its reason stated: registered, out of scope, and *a fix I cannot watch bite is not evidence*. Gates: `make test` **0** and **0** at 1584 tests in 198 suites (4.092s, 4.023s); `make lint` **0**, 0 violations in 494 files; `make parity` **0** at 358/358 floor 358; `make acceptance-r6` **0** at `examined=6 failures=0`; three pool suites alone 0.305 s. Machine 41–51% idle, siblings measuring in the same repo. Prior verdict (2026-08-21, `metamorphic`): the item does what it was filed for — the `release()`/`armedReap()` race is structurally closed and would not reopen in 80,797 rounds at a 1 ms window under 48 spinners, `D-g3-a`/`D-g3-b` empirically confirmed, five mutations red and restored. Original finding: found independently by the R6 and M23 verifiers on the same night, in different worktrees. `PoolReapingTests.swift:61` — *P6, a per-server idle window overrides the default* — slept 150ms expecting a 25ms idle reap, failing under whole-suite load while passing in isolation four times running. **A gate that is green on the second run is not a gate.** The session's unattributed load-548 red stays unattributed; nothing here touched it. The runner flagged that **`ORCHESTRATOR.md` would conflict on merge** — it edited the worktree's copy surgically rather than pulling `main` mid-flight; resolved by union at merge. |
...
| M23 | The mock-to-SwiftUI conversion contract | mac | — | `design/mcp-router-console.html` | Opus | **Needs More Work** (3rd) — gap-fix 3 | `ai/m23` `8622e91` | **Verified 2026-08-21 at `effect-witness`; verdict Needs More Work (3rd; gap-fix 3 queued, `M23-gapfix-3.md`).** Every claim reproduced — gate 1 at 132 findings, write-token gate 3 with a ledger written, selftest 0 at 47 cases with all three exits observed, lint 0, `make test` 0 twice at 1603/200, reconciler 0 — and an independent run of the shipped engine rebuilds the committed `servers.ledger.md` **byte for byte**. Census closes at 149 rows and 132 findings; `D-m23-l` moved exactly two rows, both nameable. Ten mutation arms red on their own case only. **Three blockers, all one instance of the mechanism that produced the previous two verdicts: the property is wider than the site the finding named.** BL-1 — `main()`'s console loop (1038-1057) runs outside every `measuring()` and `write_report` is at 1058, *after* it, so an exception there gives exit 1, no report and the stale ledger while the gate prints `ledger written to ...` — the exact sentence pass 2 was closing. Settled at `effect-witness` with a sentinel at the report path and a filesystem read-back; fires on non-UTF-8 stdout and on a lone surrogate; bare `LC_ALL=C` does not fire it (PEP 538) and `| head` cannot (stdout caps near 36 KB under the 64 KB pipe buffer, confirmed by inflating a dump by 1600 nodes). BL-2 — the injectivity check went into `layer_breadth` only; `layer_copy` reads the same structure untested, so breadth prints `unclassified x 2` and copy states a measured difference against that same control three lines below. BL-3 — `layer_structure` counts nodes **enumerated** rather than axis declarations **corroborated**, so a surface with every `axis` key removed prints the identical line and the layer-wide guard cannot fire; the verifier ranks this downgradable and says a next pass could reasonably fix the comment instead. **`D-m23-s`'s deferral reason is refuted by measurement** — filtering copy to vouched pairings gives 19 paired strings and 16 findings with the gate total still 132, so the population moves and the pinned number does not; the stated obstacle does not exist, and the orchestrator recorded that reason as established without testing it. **What held:** B3's class is closed by construction (zero of 4,174 Default_Ignorable codepoints survive `readable()`), and every one of the sixteen new checks has a demonstrated firing input, which distinguishes the correct nothing-moved outcome from the vacuous one that looks identical. Follow-ups `D-m23-x`...`D-m23-ab`. |
...
| D-g3-b | `PoolLifecycleTests.swift:114`'s 30ms before the follower shutdown | new item | P9 wants the second `shutdown()` to arrive with teardown still in flight. Lose the window and the follower arrives after teardown finished, and shutdown is idempotent, so it passes **vacuously**. Same class as `D-g3-a`: a test that stops meaning what it says rather than one that fails. The fix wants an observable for "teardown is in flight", which the pool does not currently expose |
...
| D-g3-j | G3's brief misstated its own diff on four counts | G3-gapfix | **Closed by the gap-fix.** Five test-only members described as three read-only accessors; `Sendable` said to be added to `ReapTimer` and fifteen lines later said to be removed, while `struct ReapTimer {` is unchanged at `PoolEntry.swift:74` at both `e32b185` and HEAD — `ReapArming` is the `Sendable` type and `ReapTimer` never was; 960 ms of removed sleeps written as 950; `D-g3-a` citing pre-fix line numbers where `D-g3-b` cites post-fix. All four corrected in the brief and in the row above |
