...
| M23 | The mock-to-SwiftUI conversion contract | `M23-mock-to-swiftui-contract.md` | — | — | **Ready to verify** — 7th gap-fix complete (`ai/m23` `8a755fa`). **BL-1 closed: R5 is reachable with a report path** — `main()` flushes stdout after `gate()` returns, so a dead pipe put `BrokenPipeError` in the ledger where the first write had named the true cause. Reproduced 3/3 unmutated; case 68 red on the shipped engine (67 ok, case 68 alone) and green after. **The engine is no longer byte-identical and that is deliberate**: the acceptance is a property of its behaviour, and a lane established no selftest-only mechanism can reach it. Six lines in `main()`, and the lane broke the first shape twice before this one. `D-m23-ba` widened: AST-derived spelling count, eleven more escapes, a fragment check for run-time assembly, stdlib-only imports — nine evasions red, residual stated. Three of our numbers corrected: nine arms invisible rather than five, 145 python3 processes of which 52 are the engine, and gate B's recipe recorded rather than listed. Gates A 1 at 132, B 3, selftest 0 twice at 68, lint 0, `make test` 0 twice at 1615/202, reconciler 0 |
