...
| **D2** | **Deferred register: Mac surfaces and design authority** | mac | M13 ✓ G1 ✓ | — | Opus | **DIED — WIP RESCUED, relaunch** | `ai/d2` @ `9bdbffb` | Killed by `503 no-eligible-account / over_reserve` (8 of 9 accounts at or over reserve) after **24 minutes and ~592k tokens**, so unlike G1's first death this was NOT cheap and NOT empty. **Work preserved rather than discarded**: spec-D2 (18.9k) and plan-D2 (9.4k) landed in the main tree, and 8 modified files were committed as an explicitly unverified WIP rescue — `ShellWindow` (D-m13-a), `SettingsBoard` (D-m13-b), `MenuCommand` (D-m14-*), `m8-settings-menubar.sh` (D-g1-b), `DESIGN.md` (M10/D-f) and `spec-M1.md`. **Nothing in it is gated, reviewed or mutation-proven and none of it may merge as-is.** Liveness was verified before touching the worktree: its only open handles belonged to a **32-hour-old `sourcekit-lsp` daemon**, which is the false-liveness signal that has fooled this fleet before |
| **D1** | **Deferred register: router side** | router | P1 ✓ P2 ✓ P3 ✓ P4 ✓ | — | Opus | **DIED — relaunch clean** | — | Same 503. **Nothing survived**: 0 commits, 0 uncommitted changes, worktree at main's HEAD — a clean re-run, not a resume. It did write `spec-D1.md` (8.9k) into the main tree before dying, so its grounding is not lost. Still owns `D-g1-g` (concurrent parity runs corrupt each other via fixed ports), which is arguably the highest-value item left on the router side because every parity number this fleet reports carries an asterisk until it is fixed |
...
| **D-g1-g** | **Two parity runs on one machine corrupt each other silently** | new item | Every lane binds a **fixed port** the harness claims never to share. Reproduced deterministically by holding `:8982` by hand. **Named, not called flaky.** This explains gate runs that read 69 and 77, and it means any parity number measured while another parity run was live is suspect. Wants a lockfile or ephemeral ports |
...
| D-g1-b | m8 A9: a disabled control carries no reason | M8 | A **real product finding**, not a harness one: 'Forget the stored token' is disabled and explains nothing. Pre-existing — the assertion is on main, and G1 took that script from 19/2 to 20/1 |
