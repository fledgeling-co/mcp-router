...
| R7 | The router's thesis is unmet for every harness but Claude Code | `R7-harness-reconciliation.md` | — | — | **Needs More Work** — gap-fix re-queued (`ai/r7` `b3d2410`; rung `metamorphic`; 3 blocking, see `R7-gapfix.md`; 11 follow-ups `D-r7-a`…`D-r7-k`; every bundle gate reproduced, but the bundle's claim that no out-of-family lane could deliver is refuted — all three delivered and all three named the same top defect) |
...
| G3 | `make test` is not deterministically green | `G3-make-test-is-not-deterministic.md` | — | — | **Ready to verify — gap-fix complete** (`ai/g3`; the fix contains no duration — it asserts what the arming chose and awaits the event, and the gap-fix bounds both awaits through `awaitEvent` so a regression names an assertion inside the CI bound instead of timing the run out: the assigned mutation reddens in a 10.461 s run where the verifier measured 601.184 s, and a standing constraint held by ten controls keeps every call site inside the bound. `make test` 0 twice at 1584 tests; lint 0; parity 358/358; `acceptance-r6` `examined=6 failures=0`. 11 of 14 fixed sleeps removed; `D-g3-j` closed, 10 follow-ups `D-g3-a`…`D-g3-i` and `D-g3-k` open) |
...
| M23 | The mock-to-SwiftUI conversion contract | `M23-mock-to-swiftui-contract.md` | — | — | **Ready to verify** — gap-fix complete (`ai/m23` `f74ca96`; all three blockers closed with red-green mutation arms; 6 new follow-ups `D-m23-f`…`D-m23-k`; original verdict `ai/m23` `5d388fb`; `metamorphic`; 3 blocking, see `M23-gapfix.md`) |
