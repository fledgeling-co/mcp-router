...
| R17 | A failed index leaves a recorded error for one server and nothing at all for another | `R17-a-failed-index-can-leave-no-trace.md` | — | — | **Ready to verify** (`ai/r17` `1004d10`, 5 commits). **One bug, not two**: `buildManifest` wrote the row for both servers and the watcher deleted it on both implementations; the divergence is the route, since `namecheap` is staged in `~/.claude.json` and `lifeline` is not. Its reason survived only in `watch-state.json`, which nothing reads. Gates green with sources marked; `parity-gate.sh`'s exit 1 has two causes, neither R17's and neither the unwired lane predicted. Review lanes preserved as evidence; fable's F1 filed as **R18** and reproduced against the built router |
