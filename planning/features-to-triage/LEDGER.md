...
| G3 | `make test` is not deterministically green | `G3-make-test-is-not-deterministic.md` | — | — | **Ready to verify** — gap-fix 2 complete (`ai/g3` `cbc6a81`). The scanner was rebuilt rather than patched: a delexer implementing Swift's real comment and literal grammar, and a scan walking brace balance rather than indentation. Completeness established rather than asserted — 53 controls in both directions, 34 single-mechanism mutations, 34 of 34 red, every control red under at least one — and what is still approximated is named. Three review rounds asked to break it found twelve more defects, and the readability guard added along the way caught a real file being silently blanked. Criterion 3 deleted with its reason; `D-g3-q` deferred with a tested reason that corrects the verifier's site count. Gates green, assigned mutation reds in 10.589 s |
...
| R14 | A client's Authenticate action succeeds, and says which upstreams still need authorising | `R14-client-auth-succeeds-and-reports-upstreams.md` | — | — | Untriaged — raised by the owner 2026-08-21 from a live `Dynamic Client Registration rejected (HTTP 404)`. The action can never succeed today and a better error is not available: OAuth offers no compliant *no authorization needed* answer to a flow the user initiated. Serve a local authorization server that authenticates nobody and report the real upstream state, in four states rather than one — 13 upstreams, 8 serving, and `mobbin` holds valid tokens while serving nothing. Two out-of-family lanes reviewed and corrected the draft's security model in four places |
| R15 | The Host check guards `/mcp` and nothing else | `R15-host-check-is-scoped-to-the-mcp-transport.md` | — | — | Untriaged — security. DNS-rebinding protection sits in the MCP transport, not the dispatcher, so `/health`, `/status` and the control block answer 200 to a foreign Host while `/mcp` answers 403. Measured on the running router; both implementations scope it the same way. Discloses usage history, project paths and every server's command line to a rebound page. `envKeys` is names only — no credential is served. Precondition for R14 |
