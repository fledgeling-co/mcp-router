...
| Plan | `planning/plans/plan-M21.md` — **Plan committed 2026-08-22.** Its out-of-family review **never ran during planning** — both lanes returned zero bytes, `agy` on a permission prompt and its retry on a timeout — and was run afterwards instead, returning **AMEND** from both lanes with eight questions each. Those verdicts postdate the planning session and **nothing is dispositioned**; the plan says so explicitly. Three findings were verified against the repo rather than taken on trust, including that `token-register.json` carries **50** `M21-*` rows and not the 45 the plan's opening census states. |
