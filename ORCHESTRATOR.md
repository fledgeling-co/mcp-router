...
| **P5** | **Close the last three closeable parity rows** | router | D1 ✓ P1 ✓ | — | Opus | **Running** | `ai/p5` | Takes parity 78 → 81 and leaves R4-C owning only the rollback proof. `D-p1-a` is the HTTP half of `POST /servers/:name/auth`. `D-p1-d` is the cli auth row, whose blocker note P1 already corrected once (`D-j` is fixed and no longer blocks it). `install-launchd-watch` is the hard one and its instructions are explicit in the row: P1's six runs show the two terms varying INDEPENDENTLY (yes,yes,no ×2; yes,no,yes ×3), so one explanation never covered the signature and **a green streak after a partial fix would be re-running until green** |
...
| **D3** | **Deferred register: phone copy and the harness limit** | ios | I3 ✓ | — | Opus | **Running** | `ai/d3` | 4 children. Two are **spec-level** (`D-i3-g`, `D-i3-h`) and want the owner's word on the copy before the code changes; the brief tells it to bring back **the exact strings it would ship** rather than a question, so the owner can accept or edit in one pass |
