...
| M23 | The mock-to-SwiftUI conversion contract | mac | — | `design/mcp-router-console.html` | Opus | **Ready to verify** — 8th gap-fix | `ai/m23` `7213ce2` | **Delivered 2026-08-21, ready to verify (8th gap-fix; `ai/m23` `7213ce2`, 3 files).** `grep -n 145` before: 481, 1221, 1224, 1434 — after: each site reads *145 python3 processes* against a stated 52 engine runs, and **both rewrites do more than fix the number**, dropping a chronology claim and naming the population that can reach the route at all. **`D-m23-bk` went further than asked and answered honestly**: there is no inline `-c` script in that suite and never was, checked across **all 11 revisions `git log --follow` names**, each blob read and non-empty so an absent path cannot read as a zero — and **what 53 was counted from is not recoverable, which the row now says** rather than inventing a reason. The one mechanism that produces it is case 46's `python3 -` stdin heredoc whose `sys.argv[0]` is `-`, measured directly, and which **`ast.parse`s the engine rather than executing it, making it an engine reader rather than a run**. `D-m23-bl` corrected to 52 with that as its reason; `D-m23-bm` measured off `2bfaa7f` as one hunk, 25 added and 2 removed — 15 comment, 10 code of which 2 are pre-existing `flush()` calls re-indented, leaving **8 new physical code lines and four statement nodes by AST** — in a commit holding a 206-line selftest change alongside. **Both lanes independently broke the runner's first wording on four distinct counts**: it bounded a trace quantity with a source grep, called one heredoc the whole 53-vs-52 gap, said `argv[0]` where POSIX and Python disagree, and counted `try`/`except` as five statements where the AST gives four — all fixed before the commit. Gates: selftest **0 twice at 68 cases**, lint 0 over 521 files, gate A **1** at 132 findings with the ledger and all four dumps sha256-identical either side **and to the pre-edit baseline**, gate B **3** with the obituary written, reconciler 0 across A-K, `make test` **0 twice at 1619/204 on attempts 4 and 5** — three earlier attempts failed on wall-clock budgets at **0.0% idle with 227 runnable on 16 cores**, and **alone at the same load `OAuthWireTests.swift:263` passes 3/3**, which discriminates contention from the test. Engine and gate script byte-identical to `cf7e5fc`. **Two residuals flagged rather than fixed**: `planning/evidence/M23-gapfix-7.md` still says *six-line change* at two lines, a dated report outside the stated scope; and the canonical block's *145 is every python3 process the suite starts* is **bounded by its own instrument** — a `sitecustomize.py` sees only processes that loaded `site` with it on the path — which fable raised and the runner left as the verified wording rather than reopening it. |
...
| R17 | A failed index leaves a recorded error for one server and nothing at all for another | router | — | — | Opus | Untriaged | — | **Raised 2026-08-21 from a live investigation across two sessions.** 14 configured, 13 in the manifest, 12 serving. `lifeline` and `namecheap` both fail with `MCP error -32000: Connection closed`, observed for both during `index --force` — and `lifeline` gets a manifest row with the error recorded and reports it on `/servers`, while **`namecheap` gets no manifest row and reports `error: None`, `tools: 0`, `state: idle`**. Nothing anywhere says it failed or was attempted. Same class as R14's silent upstreams and R16's silent skipped adoption, one layer in: **the router knows the index failed and keeps no record of it for this server.** It also degrades R14: that report derives state from tool count plus the auth record, both empty and blameless here, so it will file `namecheap` as a not-an-auth-problem with no reason to show — the reason existed and was discarded rather than never known. **Establish why the two differ before writing the fix** — both fail identically, so the divergence is in the indexing path; `namecheap` is also a global `~/.claude.json` entry and so arrived by adoption, which may take a different branch. That question decides whether this is one bug or two. Related: `namecheap` being both a router upstream and a direct Claude Code entry is R7's duplicate class and is probably why it still works for the user at all. |
...
| D-m23-ax | The `run.report_path` cleanup | M23 | Found live by the Google lane reviewing read-only from `/tmp`; registered rather than taken in an arming pass |
| D-m23-ay | `set -euo pipefail` kills the gate at its bare preflight | M23 | Found live by the fable lane. The gate's preflight runs unguarded, so strict mode would terminate it before the decision block it exists to reach |
| D-m23-az | `D-m23-at` is mis-stated: one of five call sites is unexecuted, not three | M23-gapfix-7 | A `sitecustomize.py` trace over 145 engine runs recorded eight emissions and showed R4 runs once taking the WARNING branch and R5 runs once with `path` None, so only R1 was executed by nothing at all. The row was written from a verifier's report; correct it at merge |
...
| D-m23-bc | *145 engine runs* counts every `python3` process, and `D-m23-az` repeats it | M23-gapfix-7 | 145 processes total, of which 52 are the engine plus one `python3 -c`; the engine ran 53 times, 10 with `--report`. Every emission figure the conclusion rests on is right; only the population is over-counted 2.7x |
