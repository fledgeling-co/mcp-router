...
| M23 | The mock-to-SwiftUI conversion contract | mac | — | `design/mcp-router-console.html` | Opus | **Needs More Work** (4th) — gap-fix 4 | `ai/m23` `668c0f8` | **Verified 2026-08-21 at `effect-witness`; verdict Needs More Work (4th; gap-fix 4 queued, `M23-gapfix-4.md`) — but this is not a fourth instance of the mechanism.** The method change worked and the verifier measured every property rather than reading it. **BL-1's partition holds within `main()`**: `run = Run()` is the only statement before the `try` and assigns three constants, the handler and its final flush are both wrapped, both named routes reproduce at exit 3, and so do two of the verifier's own (stdout closed with `>&-`; both descriptors closed). **BL-2's enumeration is complete today and was measured**: exactly four `.pairs` reads, all four inside the allowlist, none outside a function body, and none of the seven dynamic-access spellings present; reverting `layer_copy` to `ctx.pairs` gives exactly 20 paired strings, 16 findings, 132 total — the runner's numbers independently reproduced. **BL-3 checks out across all eight layers**; the axis-stripped dump exits 3 naming `structure`. **The panel refusal was demonstrated rather than accepted** — building `claims` from `pairs.items()` turns case 34's fixture from exit 1 with the collision finding into exit 0 `present 5 · clean`, the duplicate-id defect reopening exactly as the runner argued. Settled at `effect-witness`: the MEASURE product built, `MeasureDump` opening real windows and writing four dumps totalling 73 nodes, the engine run as a separate OS process, and for `D-m23-y` the ledger path made a directory so no write could occur — the gate printed `mock-fidelity-gate: no ledger was written by this run (exit 3)` and the filesystem was read back from outside the process. Gate A exit 1 at 132 findings from freshly rendered dumps with the ledger byte-identical; selftest 59 cases exit 0 twice; lint 0; `make test` 1603/200; reconciler 0 across A-J; 4,174 Default_Ignorable codepoints re-derived with 0 surviving. **The single blocker is an arm, not a product defect.** Case 51 is named *the engine claims a written report only when it wrote one*, and moving `emit(REPORT_MARKER + report_path)` before `write_report` leaves both its assertions green and the other 58 too. It cannot discriminate because its first invocation points `--report` at a **writable** path, where the write succeeds whichever side the marker sits on, and its second passes no `--report`, so the marker is unreachable either way; the only configuration where the orderings differ is a `--report` path whose write **fails**, and no case drives it. Case 33 already builds a `chmod 500` directory, so the missing arm is a third invocation of an existing fixture. The verifier's own words: *no product property fails on it — the behaviour is correct and I witnessed it — but a check that stays green under the mutation it was written to catch is the thing this item exists to prevent.* Second instance in this pass of the defect the runner reported against itself. **The codex lane is down until 27 August on a usage limit**; the Google family substituted and earned it, naming two evasion families the verifier had not found plus one factual error the verifier refuted by measurement (it claimed `async def` evades the AST walk; the walk checks `AsyncFunctionDef` and an async reader goes red) and one hallucinated decorator. Follow-ups `D-m23-ac`...`D-m23-ai`. |
