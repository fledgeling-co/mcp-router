...
| M15 | Settings becomes its own window | `M15-settings-window.md` | `spec-M15.md` | — | To Do — triaged 2026-08-22. Settings is a sidebar destination today and no `Settings` scene exists; no URL handling exists in the app at all so the brief's deep links are the mock's own. Seven panes confirmed against the mock. Removing `.settings` from the destination enum is the risky edit — four readers depend on it. Eight assumptions, no essential question |
...
| M21 | The token layer, the split accent, and `DESIGN.md` | `M21-token-layer-and-design-md.md` | `spec-M21.md` | — | **To Do** — design of record settled 2026-08-22 as `design/mcp-router-console.html`. Needs More Info — triaged 2026-08-22. **The programme's one owner decision: which document is the design authority.** DEF-016's closure records the owner keeping `prototype.html`; `PRD.md` §9.1 says the console mock supersedes `DESIGN.md`. Neither record names the other artifact, so every check reads clean over a live fork. M23 already measured the size: 89 token rows, 25 matched and 64 pending. `DesignTokenParityTests`' exact two-way name-set equality means even the split accent cannot land until `DESIGN.md` §2 carries the rows. Three options, (b) marked reversible, (c) added by the review. Pairs with `DEF-042` on M28's docket |
...
| R17 | A failed index leaves a recorded error for one server and nothing at all for another | `R17-a-failed-index-can-leave-no-trace.md` | — | — | Untriaged — `lifeline` and `namecheap` fail identically at index time; one records the error in the manifest and on `/servers`, the other records nothing anywhere and reads as an upstream that simply has no tools. Same class as R14 and R16, one layer in. Establish why the paths differ before fixing |
