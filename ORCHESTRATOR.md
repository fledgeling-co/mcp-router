...
| G3 | `make test` is not deterministically green | harness | — | — | Opus | **Ready to verify** — gap-fix 3 | `ai/g3` `94ddd73` | **Delivered 2026-08-21, ready to verify (gap-fix 3; `ai/g3` `94ddd73`, 4 commits on `e8c20e0`).** All three shapes fixed, each with a one-token control showing the flip: the statement label read BOUND and now reads UNBOUND (deleting the label was the control); the labelled string-literal argument produced **no call site at all** and now produces one (identifier-for-literal was the control); and `Task` inside a string interpolation reddened a correct wrap and no longer does (`Clock`-for-`Task` was the control). Each fix went one level up rather than patching the symptom: `firstWord` now steps past a prefix that introduces a statement without being one, **which closed a `case`/`default` clause nobody had opened**; blanking comments and literals alike to whitespace had made *a value indistinguishable from absence*, so literal bytes and non-ASCII code bytes now become `ScanByte.elided`, **which closed a non-ASCII argument name too**; and the escape test reads the owner of the brace rather than the statement around it. **The directional claim is corrected and now rests on a count rather than an argument**: of the twelve shapes measured against the delivered scanner, **eight fail toward a miss and four toward a red**, so the corrected wording is *the layer fails both ways and the direction is not predictable from it* — written at both sites in the brief, the suite's doc comment, the register and the LEDGER. **Family C is stated, not closed**, which is what the brief allowed: A closed at 19 on a citable production list, B closed at 12 on brace nesting, **C open at 38**, with the mutation matrix stated separately because *every mechanism is load-bearing* is a different claim from *the grammar is covered*. 69 controls, each red under at least one of twelve mutations. **Two of the panel's findings were regressions the fix had just introduced** — reading the escape from the brace's owner lost `_Concurrency.Task { }` (chain root is the module) and `Task.detached(operation: { })` (the brace is an argument, not a trailing closure), both of which the old whole-opener word search had covered; the owner is now every component of the chain read from both positions a brace can hold, which also catches a shape neither lane named. Gates: `make test` 0 twice at 1587/199, lint 0 over 500 files, parity 358/358, `acceptance-r6` clean, **`ledger-reconcile.py` 0 with check K over 172 register rows**, assigned mutation exit 2 at 13.189 s naming its own condition, sources restored with `git diff app/Sources/` empty, exactly 5 call sites all bounded over 499 files. **Two of four `make test` runs went red** in `CallbackLifecycleTests.swift:238` and `ControlStreamTests.swift:72` — files this item does not touch, `D-g3-c`'s class, now `D-g3-s`'s fourth and fifth instances. **`make test` is still not deterministically green on this machine, which is this item's own title**; what G3 fixed is the pool suite, and the residue is `D-g3-c`'s ~60 unclassified sleeps elsewhere. Load average 421 at 0.0% idle throughout, so no timing here is a comparison. Lanes: gemini 8 findings at 33 KB (first launch produced nothing — headless auto-denied a tool permission, re-run with `--disable-slash-commands`), fable 8 at **8.6 KB where 33 KB produced nothing last pass**, codex down to 27 Aug, grok 481 bytes of narration over ten minutes and reported down once without retry. |
...
| M23 | The mock-to-SwiftUI conversion contract | mac | — | `design/mcp-router-console.html` | Opus | **Ready to verify** — 6th gap-fix | `ai/m23` `e387951` | **Delivered 2026-08-21, ready to verify (6th gap-fix; `ai/m23` `e387951`).** **The method changed rather than the targets, and the enumeration is the deliverable.** `REPORT_MARKER` has exactly two emission sites — `gate()`'s post-write site and the tail of `write_unmeasured_report` — and the second is reached through five callers, which is six route classes. **Completeness is argued from the code**: the byte string the gate script greps for exists once in the engine, as a module constant; that constant is read at exactly two places, both `emit(REPORT_MARKER + …)`; and nothing reaches a module global under another name, since the engine contains no `getattr`, no `vars` and no `__dict__`. **The verifier's trace reproduces exactly** — traced with a `sitecustomize.py` on `PYTHONPATH` rather than by editing the engine: **145 engine runs, 10 with `--report`, eight emissions, five at S1 and three at S2** through R2 once and R3 twice. **R1 was executed by nothing at all**, which is wider than the brief's four and is `D-m23-az`: `D-m23-at` says three of the five call sites are executed by no case, and only one is — R4 runs once and takes the WARNING branch, R5 runs once with `path` None. All four instances closed, each watched red: `normpath` reds **case 51 alone** at 66 ok (the arm is `./rel-ledger.md`, a fixed point of neither `normpath` nor `abspath`); the S2 `emit` moved into R3's call site reds **cases 32, 60 and 61** at 64 ok; a bare `print` at S1 reds **cases 44 and 60** at 65 ok, which **corrects `D-m23-ar`** — case 44's two invocations DO drive stdout-dead/stderr-open and `emit`'s fallback fires on both, because `emit` flushes explicitly, so the route was driven and unasserted rather than undriven, and case 44 was discarding it with `2>/dev/null` (`D-m23-au`, also closed). **The harness is adopted and the false declaration is gone.** The selftest said covering the gate script's console decision needed the MEASURE build and four rendered dumps, "three minutes and not hermetic"; it needs a symlinked gate script, the `swift` stub `build()` already writes and a twelve-line `MeasureDump`, and takes about a second. **Six cases now drive the marker's only consumer** — all three branches, both emission sites, both non-zero verdicts, and `${PIPESTATUS[0]}` — where before this pass every line of that block had zero coverage and any edit confined to it was green by construction. **`D-m23-aw` is closed by making the enumeration itself checkable**: case 60 re-derives it from the engine's syntax tree every run — one spelling, two emission sites and their owners, every read going out through `emit`, `emit` still naming stderr, five callers and their owners, the `Run` wiring R5 depends on, and the consumer still grepping the marker at the path it asked for. Eleven mutation arms red on their own cases and restored, engine and gate script sha256-identical after each; **five of the eleven were invisible to the shipped suite**. Gates: A **1** at 132 findings with the ledger byte-identical, B **3** with a ledger written, selftest **0 twice at 67 cases**, lint 0, `make test` 0 twice at 1615/202, reconciler 0 across A–K. **Both lanes ran read-only from `/tmp` against a copy and both found live breaks**: `gemini-3.7-flash-high` (6,031 B) the `run.report_path` cleanup and the `[ $status -ne 3 ]` guard, `claude-fable-5` (8,348 B) `set -euo pipefail` and the grep replaced by `[ -f "$LEDGER" ]` — all four now red, `D-m23-ax` and `D-m23-ay`. Gemini's first run returned 0 B: headless mode auto-denied a tool permission, so the packet went inline. Evidence at `planning/evidence/M23-gapfix-6.md`. **Delivered 2026-08-21, ready to verify (5th gap-fix; `ai/m23` `a523c41`, 3 commits).** Engine byte-identical to `17e90e8` — scope held exactly. **BL-1's arm**: one grep on case 43, and under the brief's mutation the selftest gives exit 1, 58 ok, **one FAIL, case 43 alone**, with the marker appearing **zero times** while the ledger on disk carried this run's real table. Restored, `git diff --exit-code` 0, sha256 matching. **Case 48 made load-bearing rather than removed, and the choice was measured**: deleting the old fifth conjunct and running the census mutation left the case red — the literal caught it first — so the identity was strictly decorative and removing it would have lost the case's stated claim rather than fixed it. The identity now **leads** the chain with the fixture's shape following as floors, which forces the same values so the literals give up nothing; under the census mutation the case is red **at the identity**, and deleting the identity turns it **green reporting `3 + 1 = 3`** — that is the measurement saying it carries load. **The two corrected rows were measured rather than taken on the brief's word**: the ordering revert gives 57 ok / 2 FAIL at cases 43 and 44, with 44's unbuffered run leaving the obituary on disk, while case 50 stays green **correctly** because it passes no `--report`. **Two more holes of BL-1's class, found by lanes asked to break rather than review, both closed with the case count unmoved at 59**: `os.path.abspath(report_path)` on the marker left **all 59 green** while the **live gate** printed `NO ledger was written by this run` over the table it had just written — every fixture `--report` was already absolute, where `abspath` is the identity — closed by a fourth invocation of case 51 with a relative path; and dropping the marker from `write_unmeasured_report` left all 59 green while the gate denied an obituary it wrote, closed on case 49. Two registered open **and measured**: `D-m23-ar` (a bare `print` for the marker is invisible, and the route that matters — stdout closed with stderr open — is driven by no case) and `D-m23-as`. Gates: A **1** at 132 findings with the ledger byte-identical and `git status` clean, B **3** with a ledger written, selftest **0 twice at 59 cases**, lint 0, `make test` 0 twice at 1615/202, reconciler 0 across A-K run three times. **Process finding, and the reason for the new standing rule: the fable lane edited the artifact under test.** Launched with the worktree as its cwd and asked to break the artifact, it *applied* `os.path.abspath` to `mock_fidelity.py` in the working tree at 17:45:17 — between gate B finishing and gate A finishing — so **gate A's first run measured a mutated engine** and its `NO ledger was written` was the lane's edit rather than the gate's behaviour. Caught by `git status`, lane killed, engine restored, both gates re-run clean. The finding was genuine and is the strongest evidence in this pass, witnessed on the live gate rather than a fixture — but it arrived as an unannounced mutation of the tree under test. Grok was run from `/tmp` afterwards for exactly that reason. Gemini 3,066 B and found both holes plus `D-m23-as`; grok killed by its own 300 s alarm after 262 bytes; codex down to 27 Aug. **Delivered 2026-08-21, ready to verify (4th gap-fix; `ai/m23` `0bad4a6`).** **Case 51 armed**, and the blocking finding reproduced first: with the marker moved before `write_report` the selftest reported 59 cases, exit 0, 0 FAIL. The arm is a third invocation whose `--report` points into a fresh `chmod 500` directory — the one configuration where the orderings differ — asserting exit 3, no `report written to`, and the `could not be replaced` warning. With the mutation still applied: **exit 1, 58 ok, one FAIL, case 51 alone**; engine restored from a `cp` backup, exit 0, 59 cases. It uses **its own** `chmod 500` directory rather than case 33's, which is chmod'ed back to 700 as that case ends — sharing the path would make the result depend on run order. **`D-m23-ag`'s premise was measurably unreachable as written**, which is a correction to the verifier's own follow-up row: while stdout is block-buffered the print loop never reaches a `write(2)`, so nothing raises inside `gate()` and the report is written whichever side of the loop it sits on — measured with a ledger assertion added, where only case 43 reddened and case 44 stayed green. Both lanes reached that independently and **the Google lane named the way through: take the buffer away.** Case 44 now runs the same dead pipe twice, buffered and with `PYTHONUNBUFFERED=1`; under the ordering revert the buffered ledger holds the real table and **the unbuffered one holds the obituary** — red, and red again under a dropped `run.report_written` guard, the mutation grok proposed unprompted. **Case 48 rewritten to a partition identity**: every text node naming a ladder role is either compared or excluded as a wrap count, so `comparisons + excluded == census` is an identity of the layer rather than a fact about the fixture — red at 3+1=3 under the census mutation and at 2+0=3 with the exclusion counter dropped. **The sweep is mechanical and its instrument was validated rather than trusted**: five passes over an instrumented copy (line coverage per case, want cross-satisfaction, quantity blinding, assertion-set discrimination, per-want redundancy), preserved at `/tmp/m23gf4/sweep/`, then **re-run against the pre-arm suite** where pass E flags case 48's want pair and pass A shows the report-write-failure branch reached by no case at all — so both known instances are visible to it. Findings: 573 executable statements with **72 never executed by any case**; 55 of 59 cases survive a mutant shifting every printed integer, so only four read a printed quantity; 16 of 39 `expect` cases accept only their own run. **What it cannot see, stated plainly**: the mutation each case is named for lives in prose, so no pass distinguishes armed from merely-accepting; pass A cannot see a reordering at all and so could not have caught case 51 directly; and **the 9 hand-written blocks embed assertions in shell conditionals the parser cannot reach — case 51 and case 44 are both hand-written blocks**, so 'no fourth instance found' is bounded by exactly the two cases this pass was about. Gates, none moved: A exit 1 at 132 findings with the ledger byte-identical, B exit 3 with a ledger written, selftest 0 at 59 cases, lint 0, `make test` 0 twice at 1603/200, reconciler 0 across A-J. **Gate-order finding**: B legitimately overwrites the ledger with its INCONCLUSIVE table, so the batch must run A last; re-run rather than checked out. Lanes: codex down until 27 Aug; Google failed once with `Find command timed out` at 0 bytes and delivered 2,791 B from an empty working directory, and **refused the unanswerable probe** — *stating a specific line number would be an estimate/hallucination* — so the control worked this time, the probe being a coverage figure that exists only in this session's instrument. grok returned 882 B at exit 0 on an 820-byte packet, same mechanism, correctly naming case 43 as the route that does redden. Nothing overruled. Machine 0.0% idle, load 442 to 582 with siblings live. Chain carried from `ai/m23`, which `main`'s per-pass compression dropped: **Delivered 2026-08-21, ready to verify (3rd gap-fix; `ai/m23` `17e90e8`).** **The method changed rather than the targets, which is what the brief asked for.** BL-1's enumeration is **a partition, not a list** — the runner's own words: *that is the move that failed at `Context(...)` and again at the console loop* — dividing the process into inside `gate()`, after `gate()` returns, and `main()`'s handler, each covered by a construct so a frame nobody has thought of is still inside one. Widest member: **outside `main()` entirely**, where a `print` to a dead pipe raises during the interpreter's shutdown flush and exits **120**, measured 3/3. Closed with one `except Exception` around `gate()`, an explicit flush of both streams inside it, and the report write moved before the console loop; `KeyboardInterrupt` stays uncaught and the property is worded to exclude it rather than pretending it cannot happen. **BL-2's enumeration is now checked rather than argued** — a selftest case walks the engine's AST and fails if any function outside `{__init__, load, derive_pairings, layer_breadth}` reads `.pairs`. Both tests moved into `Context.derive_pairings`. The pass-2 deferral reason was wrong exactly as the verifier measured: copy went 20 to 19 paired strings, findings stayed 16, gate total 132. **BL-3 widened rather than narrowed**, with the reason stated — `dumpNodes` is a real ratchet and collapsing it to a corroboration count would drop a floor of 73 to 26; two of eight layers failed the property (`structure`, fixed to carry both quantities, and `tokens`, where `rows` was asserted by the marker and must now satisfy `matched + pending == rows`). **The panel found two of the three enumerations still incomplete** — ten findings taken, six overruled with reasons, including gemini's proposal to build `claims` from `pairs` rather than the inventory, which would reopen the duplicate-id defect selftest case 34 arms. Taken: the raw INCONCLUSIVE prints lost the domain reason on an unencodable console; the second standard stream (gemini and grok independently); the mtime is not an ownership token, so the engine emits its own written-marker (codex and gemini); `layer_type_metrics` counts eligibility not comparisons at 24/2/22 (codex and grok independently, both with the numbers); and `D-m23-m` closed rather than deferred a third time, since a path naming two nodes names no control. Gates: A **1** (132 findings, all 149 breadth rows byte-identical, ledger diff 3 summary lines), B **3** with a ledger written, selftest **0 over 59 cases** (was 47), lint 0, `make test` 0 twice at 1603/200, reconciler 0. Thirteen arms red and restored. **Three process findings surfaced rather than buried.** The negative control worked in all three lanes; **the unanswerable probe did not** — half was derivable from a context line in the diff and the rest from the repository, which two lanes can read, and grok said so unprompted, so the next pass needs a probe unanswerable *from the repo*. **One arm passed on its first attempt** because the case asserted the note *contained* a phrase that stays true when both numbers agree — this item's own defect arriving inside its own selftest, recorded rather than quietly re-run. This branch predates `main`'s `9e01000`, so the register closures need re-applying at merge: `D-m23-y` is closed, `D-m23-aa` turns out to be gemini's orphan-row finding and stays open, `D-m23-x` is this property inside the selftest and is deferred per the brief. **Verified 2026-08-21 at `metamorphic`; verdict Needs More Work.** The central claim survived attack — the third exit state is armed, not decorative: `required:false` on `structure` gives 3, a removed dump gives 3, a census cut 39→3 gives 3, and reverting `return 3` to `return 1` inside the engine turns the selftest red. The declared 64-of-89 `pending` residue is honest bookkeeping, not a gap: every row carries a citation, a test proves each citation resolves to a real file containing the quoted line, and drifting `--tl-zoom` `#28C840`→`#FF00FF` turns the suite red naming both values. **Three findings blocked, all in the rows that would let a future surface reach exit 0** — `planning/features-to-triage/M23-gapfix.md`. **All three closed 2026-08-21 on the same branch**, each armed by a mutation: restoring `mock_text == app_text` takes the selftest to exit 0, so did never comparing the control kind, so did restoring the blanket `inside_a_pair` exemption, and removing the `Inconclusive` wrapper returned the uncaught `ValueError` and exit 1. `present` went 10 to 3 and `unclassified` 1 to 6 on the one filled ledger; the fourth skeleton row is a finding for the first time; `MCP_ROUTER_WRITE_TOKEN_REGISTER=1` now exits 3 with the tokens layer INCONCLUSIVE and a ledger written. **Both out-of-family lanes reached the same two holes in the first draft** — a build role spelling the mock kind vouching for itself, and a quota counting pairing success while claiming to count the mock — and both were closed before commit (`planning/evidence/M23-gapfix-review.md`). Gates: gate 1/132 · selftest 0/26 · lint 0 · `make test` 0 twice, 1580 tests in 198 suites. Eleven follow-ups `D-m23-a`…`D-m23-k`. Four bundle claims refuted. 14 gates run. **Verified again 2026-08-21 at `metamorphic`; verdict Needs More Work, the second on this item.** All three first-pass criteria reproduce exactly and every arm fires — *"this is not 'the fixes did not land', it is 'the properties they assert do not yet hold, by routes of the same class'"*. Each fix closed the ROUTE its finding named rather than the property: `present` stopped agreeing on two empty strings and still agreed on two zero-width spaces; the container exemption became a quota on the enumerated role and a surplus child wearing any other role still reached exit 0; three subprocess failures became `Inconclusive` and a `KeyError` one frame up still exited 1 on a stale ledger. **Four blocking, one of them new and worse than anything in the first pass** — a required layer that scanned nothing reads `clean` and the gate exits 0 — see `planning/features-to-triage/M23-gapfix-2.md`. Three independent instruments converged on the set without seeing each other. `D-m23-l` is the coordinator's own correction and is in scope. **Delivered 2026-08-21, ready to verify (2nd gap-fix; `ai/m23` `775a0d7`, engine at `4b7d719`).** All five blockers closed as properties with a route nobody named, which is the standard the second bounce set. **B2** — `measuring()` converts any `Exception` to `Inconclusive` with the traceback quoted, wrapping `Context(...)`, `ctx.load()`, every layer and the report write, while `KeyboardInterrupt`/`SystemExit` stay outside because an operator pressing ^C is not an unmeasurable surface; the two named routes plus two of the runner's own and two more from the panel all go 1 to 3, and the arm `except ()` returns all four to exit 1 with the stale ledger. **B1** — the `lintFiles` floor, plus two holes under it all three lanes reached independently: **a floor of `0` is not a floor** (`0 < 0` is false, and the comparison is the thing being defeated) so floors are validated as positive ints once at `Context.__init__`; and the property is layer-wide, so any required layer that ran, raised nothing and measured nothing is inconclusive — writing one floor per layer closes a list rather than a class. **B3** — eleven invisible categories plus `BLANK_CODEPOINTS`; U+200B was named, U+FEFF/U+00AD are the runner's, U+3164 HANGUL FILLER (`Lo`) and U+034F (`Mn`) came from the panel, and adding `Mn Me Mc` closes U+034F as a class without hiding content since a mark draws on the base in front of it. **B4** — claimants counted rather than filtered: `[o for o in claimants if o != my_id]` looks like the same test and is not, because two inventory entries sharing an id lose both copies and each earns `present`. **D-m23-l** corrected: unvouched now reads `unclassified`, `divergent` 18 to 16 and `unclassified` 6 to 8, exactly the two rows. **The review was fixed at both ends.** Provenance checked before the prompt was written (`grep -n timeout` returns only `= 900` and its two uses — no plant this time), and graded by a probe whose middle part is **unanswerable** plus a **negative control** (`layer_geometry` shipped unlabelled beside `layer_literals`, only the second in the diff). All three families said (b) could not be answered, all three named `layer_literals`, two volunteered the other was untouched, and two filed a real `layer_geometry` defect as pre-existing and out of diff — the reading the control exists to distinguish. That is the control whose absence the verifier named when it refused the `timeout=1` inference. Eleven further defects returned, nine closed and two deferred; **three panel findings were wrong** and are named with how that was established (`paired_nodes` is read at five sites; the argc check already exists; grok's two-spellings route cannot resolve because `nodes.get(node_path)` is an exact-key lookup). Gates: `mock-fidelity-gate.sh servers` exit 1 with 132 findings (116 breadth + 16 copy); `MCP_ROUTER_WRITE_TOKEN_REGISTER=1` exit 3 with a ledger written; selftest exit 0 over **47 cases** with all three exits observed; lint 0; `make test` 0 twice at 1603 tests in 200 suites; reconciler 0. Nine mutation arms red on their own case and restored byte-identical. **None of the sixteen new checks moves a number on the real surface**, which is what should happen. `D-m23-s` deferred with its reason stated — filtering copy's population moves the finding count acceptance 6 pins at 16, so it belongs to the pass that re-pins it. New follow-ups `D-m23-s`...`D-m23-w`; `D-m23-p` rewritten from the whole `Lo` hole to the residue actually left. Panel record at `planning/evidence/M23-gapfix-2-review.md`; 14 gates run across the two verification passes. **Verified a third time 2026-08-21 at `effect-witness`; Needs More Work, the third on this item — and the same mechanism each time: the fix landed where the finding pointed and the property was wider than that.** **Delivered 2026-08-21 as three ENUMERATIONS rather than three patches** (`planning/features-to-triage/M23-gapfix-3.md`, evidence `planning/evidence/M23-gapfix-3.md`). **BL-1** — the boundary is the process, not `main()`: the enumeration is a partition into inside-`gate()`, after-`gate()`-returns and the handler itself, each covered by a construct rather than a list. `main()` wraps the whole of `gate()` in one `except Exception`; the report write moved BEFORE the console print loop, which is what was killing the run at exit 1 with a stale ledger; stdout AND stderr are flushed inside the boundary, because a `print` to a dead pipe raises during the interpreter's shutdown flush and exits **120** from outside every boundary in the file. **BL-2 + `D-m23-s`** — the pass-2 deferral reason did not survive measurement (filtering copy to vouched pairings gives 19 paired strings and 16 findings, gate total still 132), so both tests moved into `Context.derive_pairings` and every reader consumes one derived set; a selftest case walks the engine's AST and fails if any function outside `{__init__, load, derive_pairings, layer_breadth}` reads `.pairs`, or if the engine grows `getattr`/`vars`/`__dict__`. **BL-3** — `observations` audited across all eight layers against three consumers; `structure` counted nodes it enumerated rather than axes it corroborated and now carries both quantities, and `tokens` reported a number the marker asserts and now has to satisfy `matched + pending == rows`. **The panel found two of the three enumerations still incomplete and ten findings were taken** — `gpt-5.6-sol` (high), `gemini-3.7-flash-high`, `grok-4.6` (xhigh), the last on a 29 KB packet after a 64 KB one failed that lane last session. Two families independently reached `layer_type_metrics` (24 nodes reported, 2 excluded, 22 compared) and two independently reached the second standard stream. `D-m23-m` is closed rather than deferred a third time — `index_nodes` refuses a tree where a path names two nodes, because the claimant test rests on a path naming one control. Six findings were overruled with the reason stated, one of them because the proposed remedy would reopen the defect selftest case 34 arms. The negative control worked in all three lanes; the unanswerable probe did **not** — one half was derivable from a context line in the diff and the other from the repository, which two lanes have access to, and `grok-4.6` said so unprompted. Thirteen mutation arms red on their own case and restored byte-identical — twelve through the selftest and one on the live surface for `D-m23-y`, which the selftest cannot reach without the MEASURE build. One arm passed on its first attempt and is recorded: the type-metrics case asserted the note CONTAINED a phrase that stays true when both numbers agree, which is this item's own defect arriving inside its selftest. Gates: gate 1/132 (116 breadth + 16 copy, all 149 breadth rows byte-identical); gate B 3 with a ledger written; selftest 0 over **59 cases**; lint 0; `make test` 0 twice at 1603/200; reconciler 0. **This branch predates `main`'s `9e01000`, so `D-m23-x`…`D-m23-ab` are not in its register and the closures need re-applying at merge.** **Verified a fourth time 2026-08-21 at `effect-witness`; Needs More Work — and the mechanism changed: every property the item was bounced for was MEASURED to hold, and what failed was a check written to keep one of them correct.** **Delivered 2026-08-21 as an arming pass (`ai/m23` `f74b9d7`), engine untouched.** Case 51 could not discriminate the mechanism it names — its two invocations use a writable `--report` path, where the marker prints whichever side of `write_report` the `emit` sits on, and no `--report` at all, where it is unreachable; moving the `emit` before the write left **all 59 cases green**, reproduced here. A third invocation drives case 33's `chmod 500` fixture under a clean tree — the only configuration in which the two orderings differ — and is now the sole case reaching the engine's report-write-failure branch. `D-m23-ag`: the broken-pipe route cannot see the write-before-print ordering while stdout is buffered, because the print loop never reaches a `write(2)` and nothing raises inside `gate()`; **`gemini-3.7-flash-high` and `grok-4.6` reached that independently from the code and the Google lane named the way through** — run the same dead pipe again with `PYTHONUNBUFFERED=1`, which is red under the ordering revert and red under a dropped `run.report_written` guard. `D-m23-ah`: case 48's second want now asserts the partition `comparisons + excluded == census` rather than the exclusion count, red at 3 + 1 = 3 and at 2 + 0 = 3. **The other 56 were swept mechanically rather than read**: per-case engine line coverage, want satisfaction across every recorded run, a quantity-blinding mutant, an assertion-set discrimination matrix and a per-want redundancy pass — validated by re-running the whole sweep against the pre-arm suite, where it flags case 48's want pair and shows the report-failure branch reached by no case. **No fourth instance was found**, and what the sweep cannot see is stated: the mutation each case is named for lives in prose, and the 9 hand-written blocks embed their assertions in shell conditionals the passes cannot parse. Weaknesses recorded, not fixed: 72 engine statements no case executes; case 4's lone `INCONCLUSIVE` want accepts 17 other runs. Gates unmoved: gate A **1** at **132** findings with the ledger byte-identical to HEAD, gate B **3** with a ledger written, selftest **0 over 59 cases**, lint 0, `make test` 0 twice at **1603/200**, reconciler 0 across A–J. |
