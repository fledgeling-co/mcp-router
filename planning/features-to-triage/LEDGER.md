...
| M23 | The mock-to-SwiftUI conversion contract | `M23-mock-to-swiftui-contract.md` | — | — | **Ready to verify** — 6th gap-fix complete (`ai/m23` `2442033`). **The enumeration is done and it is a check, not prose**: the marker string exists once, is read at exactly two places, and no dynamic access can reach it — with case 60 re-deriving all three from the syntax tree every run, which closes `D-m23-aw`. Six route classes tabulated, the trace reproduced without editing the engine, and a route found that nothing executed. All four instances watched red; eleven arms, five invisible to the shipped suite. The hermetic harness is adopted and the false declaration removed — that block had **zero** coverage before, so any edit inside it was green by construction, which is what both lanes exploited. Selftest 59 to 67 cases; no other number moved |
