...
| R7 | The router's thesis is unmet for every harness but Claude Code | `R7-harness-reconciliation.md` | `spec-R7.md` | `plan-R7.md` | **Ready to verify** — gap-fix delivered (`ai/r7` `c4ecff1`; the three blocking findings of `R7-gapfix.md` closed, each armed by a mutation; `HarnessDialect` reads `httpUrl` for Gemini and only for Gemini; `unreadable` is on the wire; the write gate is file-scoped with a seam rule and a 13-case selftest, and its claim is narrowed to what it can carry. All three out-of-family lanes delivered and all three named the same top defect. 1621 tests twice, parity 358/358, lane 27 checks. Six new follow-ups `D-r7-l`…`D-r7-q`) |
