...
| G3 | `make test` is not deterministically green | `G3-make-test-is-not-deterministic.md` | — | — | **Needs More Work** — gap-fix queued (`G3-gapfix.md`); verified at `metamorphic`. The sleeps are gone and the `release()`/`armedReap()` race is structurally closed (0 in 80,797 rounds at 1 ms under 48 spinners; a 40 ms gap reddens both sites). Blocking: `awaitReap`/`awaitSessionEnded` are unbounded, so the assigned mutation takes `make test` to **601 s** and names the wrong assertion — a 150 ms false red traded for a ten-minute nameless one. Remedy already in the PR: bound both awaits like `waitUntil`. Follow-ups `D-g3-g`…`D-g3-j` |
