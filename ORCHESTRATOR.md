...
| **R4-C** | **The installer cutover** | router | R2-R ✓ R4 ✓ · R2-W · D-j · D-k | — | Opus — never downgrade | **Blocked — needs 82 of 83; the user decision is MADE** | — | **Registered by the orchestrator 2026-08-14.** Flip `install.sh` to the Swift binary and retire `src/*.ts`. The commit is already specified in `spec-R4.md`. Two gates on it, and the second is not technical: the parity gate must reach **83 of 83** (it is at 69), and **the user decides**, because this changes the router their own live Claude Code sessions depend on | · **UPDATED after P5**: parity is **79**, and **three rows** stand between here and the target — `D-p1-a` (a whole OAuth client), `D-p1-e` (withdrawn, and its term needs rebuilding rather than re-running) and `install-rollback`, which is R4-C's own work and the reason the cutover is a one-way door until proven
...
| R10 | `index` prints two counts that disagree, and neither is checked | router | R9 ✓ | — | Opus | **Ready for AI** | — | Half of DEF-049. `index --force` prints `ok fixture (1 tools)` then `0 tools cached` and compares neither. The exit-code contract is M28's, not this item's. |
...
| **R7** | **The router's thesis is unmet for every harness but Claude Code** | router | R3 ✓ | — | Opus | **Ready to verify** | `ai/r7` | **The brief's premise is false for the harness it was written about, and it was established by measuring the shipped binaries rather than by asking a panel — `planning/deep-research/` is empty.** `agy` 1.1.17 embeds the Go MCP SDK's `StreamableClientTransport` and carries a config field `json:"httpUrl"`, so the `mcp-remote` shim in `~/.gemini/settings.json` is a stale wiring choice, not a workaround for a missing transport. Codex 0.146.0, cursor-agent 2026.08.11 and grok 1.0.5 all measured HTTP-capable too; only Claude Code's is end-to-end and every other row says so in the type (`HTTPCapability.measured` vs `.documented` vs `.unknown`). **Three rows contradict the brief six days on:** Claude Code, the harness it exempted, now carries a duplicate (`namecheap`); grok was never audited and is wired via HTTP with one duplicate; Gemini carries **12**, not ten — eleven by name and `Ref` by config identity (`30da6798334b2466` on both sides), so name-only matching under-counts and identity-only matching misses grok's `mobbin`. Ships `RouterCore/Discovery` (route by **endpoint, never by name**; two duplicate bases each recording which found it; capability as a third axis that changes the *remedy*, not the state) plus `mcp-router harnesses`. **Gemini CLI and grok were added to `MCPClient`** — Gemini, the brief's entire subject, was absent from it. **Two defects found by running it against the real machine:** `MiniTOML` refused `~/.grok/config.toml` on an unrelated `[[marketplace.sources]]` and again on a multi-line `args`, and the unread file then produced a **confident wrong plan** offering to add a router entry to a harness already wired via HTTP. **No harness config is written and no seam could:** `ReconciliationPlan` renders a diff and has no applier, guarded by `scripts/lint/no-harness-config-writes.sh` in `make lint`, shown able to go red. Gates: swiftlint --strict 0 · swiftformat clean · **1551 tests** · parity vectors **358/358 at floor** · `r7-harness-reconciliation.sh` 3→0→armed-0, shown red at 7 checks under a planted mutation. `make lint`/`make all` blocked at the `tools` guard in a fresh worktree (no `node_modules`); the five lint steps were run individually and all pass. Out-of-family `gpt-5.6-sol` high ran twice — once on the design fork, taken in full, once on the diff. Children **R7-C1…C4** deferred. |
