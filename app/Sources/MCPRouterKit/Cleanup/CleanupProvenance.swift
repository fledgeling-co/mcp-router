...
public extension CleanupPresentation {
    ...
    static func asOfLabel(_ date: Date, now: Date, calendar: Calendar = .current) -> String {
    ...
    static func resetFigureProvenance(
        observedAt: Date,
        isStale: Bool,
        calls: Int?,
        now: Date = Date()
    ...
    static func removeFigureProvenance(
        observedAt: Date,
        isStale: Bool,
        now: Date = Date()
    ) -> Provenance {
    ...
    internal static func staleReadingClause(stamp: String) -> String {
