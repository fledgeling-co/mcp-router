...
| M23 | The mock-to-SwiftUI conversion contract | `M23-mock-to-swiftui-contract.md` | — | — | **Needs More Work** (6th) — gap-fix 6 queued (`M23-gapfix-6.md`); verified at `effect-witness`. The restoration verifies independently and case 48's floors carry load beyond what was claimed. **The mechanism is named: five passes armed the marker route by route against a route set nobody had enumerated** — a trace records eight emissions across two sites, one with five callers. Four instances, three one assertion away. The deepest finding is that the suite's stated reason for not covering the gate script is **measurably false**: the verifier built the hermetic harness it called impossible and reached the decision in about a second. Nothing keeps a declared blind spot true, and a wrong bound reads exactly like a right one |
