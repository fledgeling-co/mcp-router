...
| G4 | Assertions that do not read the quantity they are named for | `G4-assertions-that-do-not-read-their-own-quantity.md` | — | — | Ready for AI — triaged 2026-08-22. **The open question is answered and the answer is no**: the Google lane (`agy`, `gemini-3.7-flash-high`) refused name→quantity derivation outright — *heuristic or NLP mapping from test identifiers to arbitrary in-scope constants yields ambiguous bindings and false positives on auxiliary constants* — so **option B is refused on this repo's own detector-defects doctrine**, and A starts at zero coverage while C holds no line. The substitute declines the framing rather than the question and needs no quantity at all: **raw-input accounting as a structural invariant** (every reader returns `(matched, dropped)`; `dropped` empty unless a skip is declared — `ledger-reconcile.py`'s printed `examined N` line is the convention that found instances 1, 4 and 5, and the work is turning it into a contract and a gate) plus **a null-run gate** (an assertion that passes on an empty, inverted or poisoned fixture is *provably* vacuous, which is a property of the assertion rather than an inference about its name). Reaches instances 1, 4, 5, 6 — every silent-drop and partial-match case — with no annotation and no false-finding class. **Stated rather than implied: instances 2, 3, 7 and the `egress` one read a real quantity that is the wrong one and survive both mechanisms**, so half this brief's own table is out of scope for its own fix and the item must not report as closing the class. Stays separate from `G1`, which owns assertions that are too weak where this owns readers that cannot account for their input. |
...
| R17 | A failed index leaves a recorded error for one server and nothing at all for another | `R17-a-failed-index-can-leave-no-trace.md` | — | — | **Needs More Work** (2nd) — gap-fix 2 queued, **two clauses and no code**. *No behaviour changed* was proven by compiling both revisions to byte-identical output, and the route account survived the R19-only world being built against it and is under-stated. Blocked on a withdrawn claim standing five lines from its withdrawal, and an uncovered list wrong in both directions — six sites not four, half of them with no Swift twin. The verifier also found two committed conflict blocks in `ORCHESTRATOR.md` that all ten checks passed over; **check L** now reads lines rather than rows |
...
| M28 | Five findings that need a decision rather than a runner | `M28-decision-docket.md` | — | — | Needs input — **1 of 5**, re-measured 2026-08-22. DEF-042 resolved by the console mock, which **re-sourced** the numbers rather than deleting or shelving them — `install velocity` gone, `trend` and `eval` surviving only in a section explaining why there is no trending band and in unrelated browser-engine copy, and the Skills `Runs` column carrying its source at `:2772`. DEF-049 answered in the code with its reason at `ServicePorts.swift:388-394`, and its unverified lead **refuted** — approve is reachable only with `pending`, which both implementations stage with `error` cleared, so `Describe.swift:218` is unreachable; the plainer defect that survives is **R21**. DEF-008 closed by the orchestrator: the losing option is better at nothing here. DEF-033 unchanged. **Only DEF-057 remains**, with a third option measured — `test-campaign` alone is 8.9 MB against the submodule's 546 MB, and populating that submodule in a worktree breaks every runner sent into it. |
