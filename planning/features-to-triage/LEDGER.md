...
| M21 | The token layer, the split accent, and `DESIGN.md` | `M21-token-layer-and-design-md.md` | `spec-M21.md` | `plan-M21.md` | **Ready for AI** — planned 2026-08-22 at `planning/plans/plan-M21.md`, Standard tier. The design-of-record question is settled: `design/mcp-router-console.html`, so this item is the re-authoring of `DESIGN.md` against it rather than the fork it was triaged as. **Sequencing against M16 is decided in the plan (D6): M21 lands first, M16 second**, and nothing in the suite goes red between the two merges — the parity oracles do not intersect (`DesignTokenParityTests` reads five tables, `BreakerGeometryParityTests` reads `Breaker geometry` and nothing else), M21 leaves that table byte-identical, and M16 landing first would have to author the Signal Path's own tokens in `ColorToken` and redden two-way parity at once. Both review lanes confirmed the order independently. **The out-of-family review did not run during planning** — three zero-byte attempts, two `agy` (a permission denial, then a timeout) and one `claude-fable-5` still running at session end. It was run afterwards against the byte-identical packet and **both lanes answered AMEND**, eight questions each. Three findings were verified against the repo: `token-register.json` carries **50** `M21-*` rows, not the 45 the plan claims (five composite shadow rows are neither modelled nor declared); five tokens carry only `mock.light` so the register is no oracle for their dark values; and `LightAppearanceTests.lightIsAuthored` will redden on those five when `ColorToken` grows to 40. Both lanes independently said the 3.23:1 dark-onAccent pin must be ported rather than replaced. **Nothing is dispositioned** — the verdicts postdate the planning session and sit in the plan's review record as a work order. Seven triage assumptions. Pairs with `DEF-042` on M28's docket |
...
| R17 | A failed index leaves a recorded error for one server and nothing at all for another | `R17-a-failed-index-can-leave-no-trace.md` | — | — | **Ready to verify** — gap-fix landed (`ai/r17`). **The fix was already correct and no code changed**; the pass corrected what the item claimed. The cause now reads **sufficient, not exclusive**, citing R19, at all four sites, with the route account kept because it is what explains the asymmetry and what the confirmed prediction stood on. The Swift/node manifest read window is **declared** in `surface.tsv`, scoped to the watch save and saying so, with the other four `saveManifest` sites left to R19. Four corrections land with their measurements: `parity-oauth.sh` standalone is 19/21 not 21/21, F2 is already documented verbatim in `ToolUnion.swift` on main, criterion 1 is bounded for the staged-only route, and the newly wrong surface is filed as **R20**. Criterion 2 stays open with its reason. Two lanes broke the corrected text and ten findings were folded in, the sharpest being that R19 is **anti-correlated** with the observed partition rather than indifferent to it, which leaves the route account the only candidate for the partition while the single disappearance stays under-determined |
| R18 | A failed index drops the digest, so the next success serves a changed surface unheld | `R18-a-failed-index-drops-the-digest-that-holds-the-diff.md` | — | — | Untriaged — found by R17's review lane and **reproduced against the built router**: fail an index, then re-index a tampered surface, and it is served with `pending: False`. Predates R17. Establish whether the digest drop is deliberate before fixing — persisting the last good digest and adding an unknown state are incompatible |
| R19 | A watch fire saves a manifest snapshot taken before a concurrent write | `R19-a-stale-manifest-snapshot-clobbers-a-concurrent-write.md` | — | — | Untriaged — demonstrated against the fixed code: a held-open watch fire clobbers a row another path wrote, with no delete in the path. Same observable R17 attributes to the deleted row, so that account is sufficient rather than exclusive. Five `saveManifest` sites, no lock, where the config writer has one |
