...
| M6 | Inbox and pairing (Mac) | mac | M5 ✓ | `?only=mac&pane=inbox`, `?sheet=pair` | Opus | **In progress** `wf_6527714f-b4a` | `ai/m6` | **The eighth and last board.** When it lands `BoardRegistry.installed` is complete and the `isn't built yet` placeholder leaves the product. Briefed on the **seven tripwire assertions** that pin the installed set, the scaffolded count and the board-file list — M6 is the change that takes the scaffold count to zero, so each of the seven must be re-pointed at something that still has teeth rather than deleted |
...
| I2 | iPhone Discover and detail | ios | I1 ✓ | `?only=phone&tab=discover` | Opus | **In progress (resume)** `wf_6527714f-b4a` | `ai/i2` @ `8f28e9d` | Two runs died on 503 capacity, neither on its code; the last two commits are orchestrator rescues of in-flight work. **4,911 insertions survived** — spec (515), plan (360), mock (1,207), five `MCPRouterKit/Discover/` files, seven `Phone/Discover/` views, a 348-line test file. Orchestrator rebased it onto `main` 2026-08-15 (was **37 behind**, and its raw diff read as deleting M5/M7/M8's merged work): clean, 0 conflicts. **Tree does not build** — `make test` exit 2 on `DiscoverHonestyTests.swift:189,211` (`specimens` used as an instance member), `make lint` exit 2 on 5 violations including **three `blanket_disable_command` at file scope in `DiscoverCopy.swift`**, a previous run silencing three rules rather than splitting the file |
...
| **M11** | **Regenerate `spec-M1.md`'s command-availability inventory from the code** | M1, M3, M4 | **A live red gate on `main`**: `mac-shell.sh` exits 1 at A22, and has since M3/M4 installed `.servers`/`.skills`. Five of its seven `surfaceAbsent` rows are wrong, and `M1-acceptance.md:60` records A22 green against a 7-command count only correct while no board existed. The inventory restates by hand a rule `MenuCommand.availability(in:)` computes, which is why it rots silently. **Deliberately not fixed on 2026-08-15** — M6 takes `installed` to 8/8, which would invalidate any inventory regenerated today |
