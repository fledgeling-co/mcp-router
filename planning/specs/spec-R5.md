...
| B92 | `DELETE /servers/:name/auth` calls `clearAuth(name)` **then** `deps.pool.clearPending(name)`, then replies (`control.ts:412-416`). The pending-clear is part of the route, not only of the completion side-effect | ordered spy trace against `signout.json` |
