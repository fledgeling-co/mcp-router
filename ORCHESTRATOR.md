...
| **P6** | **The gate states a target the owner did not choose** | router | — | — | Opus | **Running** `wf_9697f470-a29` | — | Raised by P5 and deliberately left alone by it, correctly, as outside its rows. `parity-gate.sh` prints *The cutover requires $total of $total* — **83 of 83** — while the owner decided **82 of 83** at `bec9d18`, with `fixture-registry-search` a standing exclusion. **This is not the forbidden edit**: the arithmetic diff is zero and the coverage fraction is untouched, exactly as with D1's withholding branch and G1's undispatched lane. The gate must **name the excluded row in its own output** rather than silently lower a denominator, because deleting the row would shrink the denominator and make coverage RISE, which is the trap the row was kept in the census to avoid | · **CONFIRMED LIVE on the merged tree**: the gate printed *The cutover requires 83 of 83. It has 79.* Also carrying a possible second defect for P6 to reconcile rather than fix — the gate says **4 rows proven by suite only, not by wire comparison** while `D-g1-e` says **eight** rows have never been shown able to fail. If those should agree, the disagreement is the finding |
...
| **D-p5-b** | The lifeline gateway is degraded and runners are unprotected | open, infrastructure | `8857` has **no listener** and `8787` is **bound with two listeners while lifeline reports it not answering** — a bound socket that is not serving, the same class as a healthz that lies. Probes succeed unprotected, so runs work but get **no automatic retry**. Not touched: it is the owner's multi-account proxy |
...
| **I5** | **Prove the phone↔Mac pairing round trip, and stop there** | ios | M6 ✓ I1 ✓ | — | Opus | **Running** `wf_0a97ed5f-bfe` | — | `D-m6-a` split out and made its own item, which both judges asked for independently. It has never been proven end to end and **everything else here is downstream of it**. The item ENDS at the proof: an unproven transport is the finding, and it is not licensed to change any privilege. Grok added `D-m6-b` envelope versioning as part of the same proof |
| **I6** | **Make Mac approval fast, without moving the boundary** | mac | M6 ✓ | — | Opus | **Running** `wf_0a97ed5f-bfe` | — | **This is what the owner actually asked for, separated from the thing that was blocked.** The questionnaire option he answered carries its own consequence text — *"it would want a second confirmation on the Mac, which is most of the benefit gone"* — and the panel found that **only holds if confirming means walking to the laptop**. A notification plus the menu-bar inbox band, which is already registered as `D-m6-d`, is couch-install with the boundary intact. Grok: *"faster Mac consent, not unattended install"*; fable proposed the same shape as a staged install with an undo window. **The false binary is the finding** |
...
| D-g1-e | **Eight parity rows have never been shown able to fail** | new item | The row count is honest and the reddening is not proven for those eight. Directly weakens the coverage target, which is now **82 of 83** |
