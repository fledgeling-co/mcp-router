...
| R7 | The router's thesis is unmet for every harness but Claude Code | router | R3 ✓ | — | Opus | **Needs More Work** — gap-fix 2 | `ai/r7` `60d042e` | **Verified 2026-08-21 at `effect-witness`; verdict Needs More Work (gap-fix 2 queued, `R7-gapfix-2.md`).** Every gate reproduces and every mutation arm bites, but the verifier found F1 closed the ROUTE and not the PROPERTY, one level above where anyone was looking: **R7 reads `~/.gemini/settings.json`; `agy` reads `~/.gemini/config/mcp_config.json`, and the key there is neither `url` nor `httpUrl` but `serverUrl`.** Confirmed independently by the orchestrator — 18 servers all-stdio vs 20 with `serverUrl` x6; the router entry is `npx -y mcp-remote ...` in the stale file and `serverUrl: http://127.0.0.1:8879/mcp` in the live one; `diolog-admin` and `diolog-tasks` exist only in the live one; `url`/`httpUrl` appear zero times in it; `serverUrl` and `mcp_config` appear zero times in this repo. `.migrated` dates the move to 14 Aug. Settled at `effect-witness`: during a live `agy` session seven stdio MCP children spawned and ZERO processes referenced `127.0.0.1:8879` — the shim R7 reports does not exist, and R7 proposes a migration the user already performed. Three more blockers reproduced: rule 3's header overclaims and five appliers walk it (a wrapped call, chained `appendingPathComponent`, `fopen` in the seam, `/bin/sh -c` redirect in the seam, a block-comment line); a `curl` health-check entry reads `wired-shim entries=0`; a decoy `url` beside a real `httpUrl` silently erases a duplicate. Three bundle numbers corrected: 29 `ok` lines not 27; the Gemini-key mutation CRASHES the bundle at `HarnessDialectTests.swift:163` (signal 5, unguarded `duplicates[0]`) rather than reddening 24 of 38; `D-r7-m` understates — five appliers walk through, three inside the seam, none split across files. Panel: codex 2,401 B/4 findings and agy 3,362 B/7 delivered; **grok was a lane failure at this prompt size** — 361 B of narration in 15 minutes reporting the 64 KB inline prompt truncated mid-diff. New follow-ups `D-r7-r`...`D-r7-w`. |
...
| G3 | `make test` is not deterministically green | harness | — | — | Opus | **Needs More Work** — gap-fix | `ai/g3` `4c0f920` | **Verified 2026-08-21 at `metamorphic`; verdict Needs More Work (gap-fix queued, `G3-gapfix.md`).** The item does what it was filed for and the verifier says so — six clean `make test` runs at 1-min load 8.4 to 28.6, lint 0/493, parity 358/358, `acceptance-r6` `examined=6 failures=0`, five mutations red and restored, tree restored bit-identical. **The `release()`/`armedReap()` race is structurally closed** — both are synchronous actor methods so there is no suspension point between them, and it would not reopen naturally in 80,797 rounds at a 1 ms window under 48 spinners; widening the gap to 40 ms reddens both call sites, which proves the atomicity is load-bearing. `D-g3-a`/`D-g3-b` are now empirically confirmed rather than argued — both directions of window loss forced, both exit 0. **Blocking: the fix trades a 150 ms false red for a ten-minute nameless one.** `awaitReap`/`awaitSessionEnded` await an unstructured `Task` with no deadline, so the effective bound is the pool's own armed window and P6 configures that at 600,000 ms. The assigned mutation measured `601.184 s`, and even then the one issue named `PoolReapingTests.swift:98` while the `idleMilliseconds == 25` claim at `:87` passed silently; killed at 150 s it exits 142 with no test name. The runner met this mechanism inside the item — the 63.968 s P4 incident — and closed it locally in P4 by equalising both windows, leaving P6, the headline test, exposed by design. Remedy is already in the PR: bound both awaits the way `waitUntil` is bounded. Four bundle claims refuted, all doc-level (`D-g3-j`): five accessors not three, no `Sendable` added and the same paragraph later says one was removed, 960 ms not 950, and `D-g3-a` cites pre-fix line numbers while `D-g3-b` cites post-fix. New follow-ups `D-g3-g`...`D-g3-j`. The session's unattributed red stays unattributed — six runs between load 8.4 and 28.6 say nothing about the load-548 incident. |
