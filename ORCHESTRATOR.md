...
| M23 | The mock-to-SwiftUI conversion contract | mac | — | `design/mcp-router-console.html` | Opus | **Ready to verify** — 6th gap-fix | `ai/m23` `2442033` | **Delivered 2026-08-21, ready to verify (6th gap-fix; `ai/m23` `2442033`, 2 commits).** **The enumeration is done and it is argued from the code rather than from search effort**: the byte string the gate greps for exists **once** in the engine, as the module constant at line 115; that constant is read at exactly **two** places, both `emit(REPORT_MARKER + ...)`; and nothing reaches a module global under another name because the engine contains no `getattr(`, `vars(` or `__dict__`. **Case 60 re-derives all three from the syntax tree on every run**, so the enumeration is a check rather than prose — which is how `D-m23-aw` is closed. Two emission sites, the second reached through five callers, six route classes in all, each with its assertion and owning case tabulated. The verifier's trace was reproduced with a `sitecustomize.py` on `PYTHONPATH` **rather than by editing the engine** — 145 engine runs, 10 with `--report`, eight emissions, five at S1 and three at S2. **Beyond the brief's four: route R1 was executed by nothing at all**, and **`D-m23-at` is also mis-stated** — it says three of five call sites are executed by no case and only one is; registered as `D-m23-az` to correct at merge. R5 is unreachable with a report path and **that is checkable rather than prose**: with `--report` given, `gate()` either returns 3 without raising or reaches the report block, after which `run.report_written` is true before anything downstream can raise — arm 7 confirms it by making R5 reachable and redding case 43. All four instances watched red: `normpath` on the S1 marker now reds **case 51 alone**; S2's `emit` moved into R3's call site reds 32, 60 and 61; S1's `emit` as a bare `print` reds 44 and 60; and loosening the gate's grep reds 60, 64 and 65. **Eleven arms in total, each restored from a `cp` backup with sha256 verified, and five were invisible to the shipped suite.** `D-m23-ar` corrected: case 44's two invocations **do** drive stdout-dead/stderr-open and `emit`'s fallback fires on both because it flushes explicitly — driven and unasserted, not undriven. **The harness is adopted** and the false declaration is gone: the gate script's decision needs a symlinked script, the `swift` stub `build()` already writes and a twelve-line `MeasureDump`, and runs in about a second; six cases drive all three branches, both emission sites, both non-zero verdicts and the pipe status. **Before this pass every line of that block had zero coverage, so any edit confined to it was green by construction — which is what both lanes independently exploited.** Gates: A 1 at 132 with the ledger sha unchanged, B 3 with a ledger written, selftest **0 twice at 67 cases** (was 59), lint 0, `make test` 0 twice at 1615/202, reconciler 0 across A-K; no other number moved. Engine and gate script byte-identical to the base throughout. Lanes **read-only from `/tmp` against a write-protected copy** per the standing rule, worktree clean after every gate: gemini 6,031 B with two live findings and three correct refutations; fable 8,348 B with two live, seven refutations, and **it declined to file one it could not construct a delta for**. All four lane findings are now red. New follow-ups `D-m23-ax`...`D-m23-az`. |
