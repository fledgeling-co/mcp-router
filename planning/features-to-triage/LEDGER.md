...
| R7 | The router's thesis is unmet for every harness but Claude Code | `R7-harness-reconciliation.md` | — | — | **Needs More Work** — gap-fix 2 queued (`R7-gapfix-2.md`); verified at `effect-witness`. F1 closed the route, not the property: R7 reads `~/.gemini/settings.json` while `agy` reads `~/.gemini/config/mcp_config.json`, whose key is `serverUrl` — neither `url` nor `httpUrl`. Witnessed: a live `agy` session spawned seven stdio MCP children and zero processes touching `127.0.0.1:8879`, so the shim R7 reports does not exist and its proposed migration was already done on 14 Aug. Three further blockers — rule 3's header overclaims with five appliers walking it, a `curl` health entry reading `wired-shim`, a decoy `url` erasing a real duplicate. Grok was a lane failure at this prompt size. Follow-ups `D-r7-r`…`D-r7-w` |
...
| G3 | `make test` is not deterministically green | `G3-make-test-is-not-deterministic.md` | — | — | **Ready to verify — gap-fix complete** (`ai/g3` `906e795`, 4 commits on `d544968`). Property established: **a regression in this class produces a named red inside the CI bound, not a timeout.** The fix contains no duration — it asserts what the arming chose and awaits the event, and the gap-fix bounds both awaits through `awaitEvent` under `waitUntil`'s ten-second breaker, which names the condition at the caller's source location. The bound is asserted rather than documented: a scanner requires every call site to sit lexically inside an `awaitEvent`, held by ten controls five each way. The assigned mutation reds at exit 2 in **10.461 s** naming its own condition, where the verifier had measured 601.184 s; the unnamed route (the swap the other way) reds in **3.916 s** on the resolved-integer claim, because the `#require` throws before the await is reached. Panel caught the first scan rule and, more usefully, corrected the fix's reasoning — a cancellation-aware task group does work, and this repo already contains one (`AuthorizationURLBox`), deferred as `D-g3-k`. `make test` 0 twice at 1584 tests in 198 suites; lint 0 over 494 files; parity 358/358; `acceptance-r6` `examined=6 failures=0`; three pool suites alone 0.305 s. 11 of 14 fixed sleeps removed; `D-g3-j` closed, 10 follow-ups `D-g3-a`…`D-g3-i` and `D-g3-k` open) |
...
| M23 | The mock-to-SwiftUI conversion contract | `M23-mock-to-swiftui-contract.md` | — | — | **Needs More Work** (3rd) — gap-fix 3 queued (`M23-gapfix-3.md`); verified at `effect-witness`. Everything reproduces and the ledger rebuilds byte-for-byte, but the same mechanism produced a third verdict: the property is wider than the site each finding named. The exception boundary misses `main()`'s console loop, which runs *before* the report write; the claimant test went into breadth and not copy; and the structure layer counts nodes enumerated rather than comparisons corroborated. `D-m23-s`'s deferral reason is refuted — filtering copy moves the population and not the pinned count. B3's invisible-codepoint class is closed by construction (0 of 4,174) and no new check is vacuous. Follow-ups `D-m23-x`…`D-m23-ab` |
