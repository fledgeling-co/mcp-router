...
struct CLIIndexWriteDeniedTests {
    ...
    func aWritableHomeStillPrintsOK() throws {
        let (root, home) = try IndexCLIHarness.makeHome()
        defer { try? FileManager.default.removeItem(at: root) }

        let run = try IndexCLIHarness.runIndex(home: home)
    ...
    func aFailedUpstreamWhoseFailureRowIsAlsoLostSaysSo() throws {
        let (root, home) = try IndexCLIHarness.makeHome(withABrokenServer: true)
        defer {
            try? FileManager.default.setAttributes([.posixPermissions: 0o700], ofItemAtPath: home.path)
            try? FileManager.default.removeItem(at: root)
        }
    ...
    func anOlderRowIsNotClaimedMissingFromTheCount() throws {
        ...
        let seeded = try IndexCLIHarness.runIndex(home: home)
    ...
    func aHeldSurfaceReportsChangesRatherThanTools() throws {
        ...
        let first = try IndexCLIHarness.runIndex(home: home, tools: 1)
        ...
        let run = try IndexCLIHarness.runIndex(home: home, tools: 2)
...
/// The end-to-end harness: the fixture server, the router home, and the run itself.
///
/// At file scope rather than nested in the suite so that the cases above read as cases. It is
/// `enum` with only static members because there is nothing here to instantiate.
private enum IndexCLIHarness {
    ...
    enum Failure: Error { case noBinary(String) }
    ...
    static func makeHome(withABrokenServer: Bool = false) throws -> (root: URL, home: URL) {
    ...
    static func runIndex(home: URL, tools: Int = 1) throws -> Run {
        let process = Process()
