...
| G3 | `make test` is not deterministically green | `G3-make-test-is-not-deterministic.md` | — | — | **Ready to verify** — gap-fix 3 complete (`ai/g3` `94ddd73`). All three residue shapes fixed with one-token controls, each fix taken one level up and each closing a shape nobody had named. The directional claim now rests on a count: of twelve measured shapes, eight fail toward a miss and four toward a red, so the layer fails both ways unpredictably. Family C stated open at 38 rather than closed. The panel caught two regressions the fix itself introduced. **Two of four `make test` runs went red in files this item does not touch** — `make test` is still not deterministically green, which is this item's own title, and the residue belongs to `D-g3-c` |
...
| M23 | The mock-to-SwiftUI conversion contract | `M23-mock-to-swiftui-contract.md` | — | — | **Ready to verify** — 6th gap-fix complete (`ai/m23` `e387951`, engine and gate script byte-identical). The marker's emission sites are **enumerated** rather than armed one at a time: two sites, five callers on the second, six route classes, completeness argued from the engine's single spelling of the literal and its lack of any dynamic reach into globals — and **checked against the syntax tree on every run**, so a third site reds the suite instead of waiting for a seventh verification. A trace of all 145 engine runs reproduced the verifier's 5+3 exactly and found R1 executed by nothing. Four instances closed, each watched red. **The gate script's console decision — the marker's only consumer, and a block with zero coverage — is now driven hermetically in about a second**, which disproves the prose declaration that said it could not be. Eleven arms red and restored, five of them invisible to the shipped suite. Follow-ups `D-m23-ax`…`D-m23-az` Chain carried from `ai/m23`: **Ready to verify** — 5th gap-fix complete (`ai/m23` `a523c41`, engine byte-identical). BL-1's arm reds case 43 alone. Case 48's identity was **measured** decorative, then restructured to lead the chain so deleting it now turns the case green reporting `3 + 1 = 3`. Two further holes of the same class found by lanes asked to break rather than review, both closed with the case count unmoved. **A review lane edited the artifact under test mid-gate** — the finding was genuine and the method was not; a new standing rule follows. Follow-ups `D-m23-ap`…`D-m23-as` Chain carried from `ai/m23`: **Ready to verify** — 4th gap-fix complete (`ai/m23` `0bad4a6`). Case 51 armed after reproducing the finding, red alone under the mutation and restored. `D-m23-ag`'s premise was measurably unreachable — block buffering means the print loop never reaches a write — and the Google lane named the way through, so case 44 now runs the dead pipe both buffered and unbuffered, where the unbuffered ledger holds the obituary. Case 48 rewritten to a partition identity rather than a fixture fact. The sweep of the other 56 is five mechanical passes **whose instrument was validated against the pre-arm suite**, and its blind spot is stated: the nine hand-written blocks, which include the two cases this pass was about. Gates unmoved Chain carried from `ai/m23`: **Ready to verify** — 3rd gap-fix complete (`ai/m23` `17e90e8`). The method changed rather than the targets: BL-1's enumeration is a partition of the process into three regions rather than a list of frames, and its widest member is outside `main()` entirely — a print to a dead pipe exits 120 during the shutdown flush, measured 3/3. BL-2's enumeration is checked by an AST walk rather than argued. BL-3 widened with its reason. The panel found two of three enumerations still incomplete; ten findings taken, six overruled. Selftest 47→59 cases. The negative control worked and **the unanswerable probe did not** — grok named it unprompted. One arm passed on first attempt and was recorded, not re-run Previously: **Ready to verify** — 2nd gap-fix complete (`ai/m23` `775a0d7`, engine `4b7d719`). All five blockers closed as properties with an unnamed route each: the exception boundary is now the class with ^C outside it, a floor of `0` is refused as not a floor, the layer-wide property replaces the per-layer list, invisibles cover `Mn Me Mc` plus eleven categories, and claimants are filtered rather than counted. `D-m23-l` corrected — unvouched reads `unclassified`, moving exactly two rows. The review carried a negative control and an unanswerable probe this time, and all three lanes behaved as the control predicts; three of their eleven findings were wrong and are named as such. Selftest 47 cases exit 0; `make test` 0 twice; none of the sixteen new checks moves a real-surface number. Follow-ups `D-m23-s`…`D-m23-w` Review record `M23-gapfix-2-review.md`: 11 further findings of the same class, 9 closed and 2 deferred. Previously **Needs More Work** (2nd) (`ai/m23` `f74ca96`; rung `metamorphic`; 4 blocking, see `M23-gapfix-2.md`; all three first-pass criteria reproduce, but each closed the ROUTE its finding named rather than the property — three independent instruments converged on the same new set; all three blockers closed with red-green mutation arms; 6 new follow-ups `D-m23-f`…`D-m23-k`; original verdict `ai/m23` `5d388fb`; `metamorphic`; 3 blocking, see `M23-gapfix.md`) |
...
| R16 | Adoption reads global scope only, so a project-scoped server is invisible to it | `R16-adoption-cannot-see-project-scoped-servers.md` | — | — | Untriaged — raised by the owner 2026-08-21 from a live failure. `import`/`watch` read the top-level `mcpServers` and never `projects`, so project-scoped servers can never be adopted; the router's own config type already carries a `projects` field it never populates. Not a missing read alone: one name bound to three different commands in three projects must not be flattened to one. Pairs with `D-r7-i`, its mirror image |
