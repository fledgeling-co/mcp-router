...
| R17 | A failed index leaves a recorded error for one server and nothing at all for another | `R17-a-failed-index-can-leave-no-trace.md` | — | — | **Ready to verify** — gap-fix complete (`ai/r17` `aaee8b9`). No behaviour changed; the diff over both watcher files is comment-only. **The route account came out stronger**: a single disappearance is under-determined, but the 13-of-14 partition is not, because an R19-only world predicts the inverse of what was measured. BL-2 declared with its bound, and the orchestrator's *five save sites* corrected to **three**. `R20` filed. One `make test` red named rather than rolled away |
