...
| M23 | The mock-to-SwiftUI conversion contract | mac | — | `design/mcp-router-console.html` | Opus | **Ready to verify** — 7th gap-fix | `ai/m23` `8a755fa` | **Delivered 2026-08-21, ready to verify (7th gap-fix; `ai/m23` `8a755fa`, 3 commits).** **BL-1 closed: R5 is reachable with a report path.** `main()` flushes stdout *after* `gate()` returns, so with the reader gone `BrokenPipeError` escapes into the boundary with `report_written` False and `report_path` set, the obituary is written a second time, and the ledger a reader opens says `gate: BrokenPipeError` where the first write said `manifest: no artifact at …` — the true cause replaced by a downstream symptom. Reproduced 3/3 on the shipped engine, no mutation; `PYTHONUNBUFFERED=1` does not reach it, which is case 44's discriminator. **Case 68 was watched red on the shipped engine** — selftest exit 1, 68 cases, 67 ok, **case 68 alone**, reporting `markers 2 buffered / 1 unbuffered` with the buffered ledger naming `BrokenPipeError` and the unbuffered one naming the cause — and green after the fix. **The engine is no longer byte-identical, and that is a deliberate departure from the brief's scope line.** The acceptance asks that the ledger name the original cause and the marker be emitted once; both are properties of the engine's behaviour, and `claude-fable-5` established that no selftest-only mechanism can produce them, because the overwrite happens inside `main()`'s handler driven only by argv and the state of fds 1 and 2. The change is six lines in `main()`, catching the post-verdict flush one frame before the boundary and saying nothing about what is on disk; engine `0ad79d1b…` → `9f58e96d…`, gate script byte-identical at `2229940e…`. **The first patch shape was broken twice by the lane and both breaks reproduced here**: on an unwritable ledger it claimed the file recorded this run while the file was an earlier run's clean table, and with no `--report` it claimed a ledger at `None`. **`D-m23-ba`: case 60's guard widened.** The spelling count now comes from the syntax tree with `BinOp` folding, implicit concatenation and bytes; the escape list carries eleven more spellings including `globals(`, `sys.modules` and `os.write(`; a seven-character fragment check catches run-time assembly (`%`, `.format`, `.join`, split bytes), seven being measured as the shortest bound the file admits; and the engine must import only stdlib and nothing relative. **Nine evasions red**, three of them `gemini-3.7-flash-high`'s, and **the residual is stated and measured** — an assembly from constants of six characters or fewer stays green. **`D-m23-bb`: five arms invisible is nine** — arms 1-5 and 8-11 each ran 59 ok with 0 FAIL, only 6 and 7 were visible, and the evidence table carried one fact under two wordings. **`D-m23-bc`: 145 counts python3 processes, not engine runs** — re-traced with a `sitecustomize.py` over the same 59-case suite: **145 processes, 52 the engine**, 10 with `--report`, 0 `python3 -c`; the correction that said 53 counted a `python3 -c` the trace does not find, and the over-count is 2.8x. The suite as it stands runs 173 processes, 61 the engine. **`D-m23-bd`: gate B's recipe is recorded** in the selftest header beside A and C — a direct engine invocation against the real manifest and a missing dumps dir, exit 3 with the obituary written, hermetic and no MEASURE build. What pass 2 ran under that letter is not recoverable and the file says so. Gates: **A 1 at 132 findings** with the ledger and all four dumps byte-identical either side, **B 3** with a ledger written, **selftest 0 twice at 68 cases**, **lint 0**, **`make test` 0 twice at 1615/202**, **reconciler 0** across A-K. Lanes read-only from `/tmp` against `chmod 444` copies, worktree clean after every gate: fable 5,206 B, gemini 3,251 B on a second run after headless auto-denied a tool call. Evidence `planning/evidence/M23-gapfix-7.md`. |
