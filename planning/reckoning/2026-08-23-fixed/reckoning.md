...
155 piece(s) of work remain — 49 product, 16 evidence, 90 decision — across 297 ledger rows. This reckoning speaks for 95% of the campaign's designed cases and 44% of its stated requirements; the rest is not known to be done, it is simply not known. 87 brief(s) could not be tied to the registry at all; they are listed as `unjoined` and counted as decision work, rather than assumed unbuilt.
...
| Briefs joined to evidence | 19 | 106 | 17.9% | the brief could be tied to something in the registry at all. |
...
| `unjoined` | 87 | 87 | decision-work | named in a brief; the join reached nothing, so its state is unknown either way |
...
- **DEF-003** — loopbackPinned counted the pin instead of reading it
  - this defect's status is 'fixed-in-test', which is not a word this tool recognises, so it stays broken. Guessing done is the one error here that cannot be recovered from.
...
- **DEF-006** — inventory pointed Discover and Triage at someone else's pixels
  - this defect's status is 'fixed-in-campaign', which is not a word this tool recognises, so it stays broken. Guessing done is the one error here that cannot be recovered from.
...
- **DEF-011** — Cleanup's per-row actions are specified by the design and drawn by nothing
  - this defect's status is 'fixed-in-part', which is not a word this tool recognises, so it stays broken. Guessing done is the one error here that cannot be recovered from.
...
- **DEF-035** — an incremental Swift build after adding an enum case reports impossible test failures
  - this defect's status is 'mitigated-in-campaign', which is not a word this tool recognises, so it stays broken. Guessing done is the one error here that cannot be recovered from.
...
- **DEF-041** — REQ-016's declared outbound-socket effect has no provider in production source
  - this defect's status is 'open-measured', which is not a word this tool recognises, so it stays broken. Guessing done is the one error here that cannot be recovered from.
- **DEF-042** — the design of record still draws a model-graded evaluation feature the product deliberately does not have
  - this defect's status is 'open-owner', which is not a word this tool recognises, so it stays broken. Guessing done is the one error here that cannot be recovered from.
...
- **DEF-055** — vacuity-check's blind-mutation pass examines non-tests and mis-reads Swift idiom; 7 of 7 sampled findings are false positives
  - this defect's status is 'open-detector', which is not a word this tool recognises, so it stays broken. Guessing done is the one error here that cannot be recovered from.
...
- **BRIEF-M20-menubar-status-notification** — M20 — menu bar, status item, and the notification banner
  - the join could not tie this brief to anything in the registry. That is not evidence that it was never built — it is the inferential step of this pipeline returning nothing, and a person has to read the brief and rule
  - nearest the join considered: REQ-001 (0.05), REQ-012 (0.05), REQ-023 (0.03)
- **BRIEF-M21-token-layer-and-design-md** — M21 — the token layer, the split accent, and reconciling `DESIGN.md`
  - the join could not tie this brief to anything in the registry. That is not evidence that it was never built — it is the inferential step of this pipeline returning nothing, and a person has to read the brief and rule
  - nearest the join considered: REQ-001 (0.04), REQ-019 (0.04), REQ-024 (0.03)
- **BRIEF-M22-harnesses-and-insights-boards** — M22 — the Harnesses and Insights boards
  - the join could not tie this brief to anything in the registry. That is not evidence that it was never built — it is the inferential step of this pipeline returning nothing, and a person has to read the brief and rule
  - nearest the join considered: REQ-001 (0.07), REQ-019 (0.06), REQ-011 (0.05)
- **BRIEF-M23-gapfix** — M23 gap-fix — three rows that would let a future surface reach exit 0
  - the join could not tie this brief to anything in the registry. That is not evidence that it was never built — it is the inferential step of this pipeline returning nothing, and a person has to read the brief and rule
  - nearest the join considered: REQ-019 (0.06), REQ-001 (0.04), REQ-013 (0.03)
- _…and 47 more in ledger.json_
