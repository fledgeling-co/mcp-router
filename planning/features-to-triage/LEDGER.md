...
| G3 | `make test` is not deterministically green | `G3-make-test-is-not-deterministic.md` | — | — | **Needs More Work** — gap-fix 2 queued (`G3-gapfix-2.md`); verified at `metamorphic`. The blocking finding is closed: the assigned mutation reds in 14.2 s naming its own condition and the unwrapped control exits 142 with no summary line. What blocks is the scanner meant to keep it that way — 22 planted call sites broke it five ways in both directions, including a trailing comment naming `awaitEvent(` satisfying the check for it. `D-g3-j`'s fourth correction did not land while its row says it did, and **acceptance criterion 3 was vacuous as written — the orchestrator's own**. Gemini's CRITICAL reopened as half-right: 3 of 5 bounded sites have no mutation power. Follow-ups `D-g3-l`…`D-g3-v` |
