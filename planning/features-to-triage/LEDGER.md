...
| G3 | `make test` is not deterministically green | `G3-make-test-is-not-deterministic.md` | — | — | **Needs More Work** (3rd) — verified at `metamorphic`. Every gate and the assigned mutation re-measured and held, the raw-string miss was real rather than self-inflicted, and criterion 3's deletion is honest. Blocked on four: the runner's correction of the previous verifier does not reproduce (both sites red 4 of 4 under *heavier* load); the completeness claim is right about where the approximation is and wrong about which way it fails, with two measured misses and one false fire; the population is three models not two, and the third has no closed grammar; and five register ids occur twice on the branch. Follow-ups `D-g3-aa`…`D-g3-ag` |
