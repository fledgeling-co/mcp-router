...
| M23 | The mock-to-SwiftUI conversion contract | mac | — | `design/mcp-router-console.html` | Opus | **Needs More Work** (6th) — gap-fix 6 | `ai/m23` `efa7a91` | **Verified 2026-08-21 at `effect-witness`; verdict Needs More Work (6th; gap-fix 6 queued, `M23-gapfix-6.md`).** **The restoration verifies independently** — the worktree was pristine at `a523c41` before the verifier ran anything, `git diff HEAD` empty, no untracked files, no mutation marker under `scripts/` or `app/`, engine byte-identical to `17e90e8` on sha256 — which matters because a lane mutated this tree last pass. BL-1's arm reproduces at 58 ok, case 43 alone; the `D-m23-ag` correction reproduces at 57 ok / 2 FAIL with case 50 green. **Case 48's restructure survives the attack and its floors carry load too** — deleting the exclusion entirely yields `3 + 0 = 3` which **the identity accepts**, and only `excl >= 1` reds it, so the floors are load-bearing beyond what the restructure claimed. Gates all reproduce. **The mechanism is finally named: five passes have each armed the marker on the routes they knew about, against a route set nobody had enumerated.** The verifier enumerated it — **a full marker trace records eight emissions, five from `gate()`'s post-write site and three from `write_unmeasured_report`, which has five callers, four of which lose the marker under mutation and two of those already executed**. Four measured instances, three one assertion away on existing fixtures: `normpath` on the marker leaves all 59 green because `D-m23-ap`'s closure pinned two spellings on which `normpath` is the identity (the arm is one character — spelling the existing relative path as `./rel-ledger.md`); moving the marker out of `write_unmeasured_report` leaves all 59 green; and **`D-m23-ar` is mis-stated** — case 44's two invocations **do** drive stdout-dead/stderr-open and `emit`'s fallback fires on both, and case 44 discards them with `2>/dev/null`. **The orchestrator recorded that row from the runner's report without testing it.** **The finding worth more than the four: `mock-fidelity-gate.sh` lines 128-141, the marker's only consumer, is reached by no case, and the selftest's stated reason for that gap is measurably false.** It claims covering it needs the MEASURE build and four rendered dumps, three minutes and not hermetic; the verifier built the harness that disproves it — a symlinked gate script, the `swift` stub `build()` already writes, and a twelve-line `MeasureDump` stub — reaching the gate's decision in **about a second**, and on a fixture it reproduces `D-m23-ap` by printing `NO ledger was written by this run (exit 0)` over a table it had just written. **That is the finding the previous pass could only obtain by letting a review lane mutate the tree under test.** Preserved at `/tmp/m23v6/keep/clean/`. The general form is `D-m23-aw` and is this item's own defect one level up: **nothing keeps the suite's declared blind spots true, and a bound that is wrong reads exactly like one that is right** — the declarations are the last unchecked assertions in the file. Lanes run **read-only from `/tmp` per the new standing rule**, worktree confirmed unmutated throughout; gemini 2,828 B with one break reproduced, one **refuted by measurement**, one a documented design disagreement. Follow-ups `D-m23-at`...`D-m23-aw`. |
