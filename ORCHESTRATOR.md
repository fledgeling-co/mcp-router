...
| R14 | A client's Authenticate action succeeds, and says which upstreams still need authorising | router | — | — | Opus | **Ready to verify** | `ai/r14` `76cbb53` | **Delivered 2026-08-21, ready to verify (`ai/r14` `76cbb53`, 7 commits).** Both R15 and R14 built in **both implementations**. R15: `RequestAuthority.guarding` wraps the Swift ladder, `hostRefusal` sits at the top of the TypeScript handler, `MCPEndpoint` keeps its own check as defence in depth reading the *same* list rather than a second copy, `/mcp`'s 403 body verified byte-identical against the reference (97 bytes, envelope order `jsonrpc, error, id`), `[::1]` present, and one deliberate ordering change in both — a `/mcp` POST both wrongly addressed and unparseable now answers 403 rather than 500. R14: protected-resource and AS metadata plus the RFC 9728 suffixed form, DCR, `/authorize` with a consent interstitial, `/token` with `authorization_code` and a **genuinely validated** `refresh_token`, stateless off one HMAC key at `~/.claude/mcp-router/auth/issuer.key` (0600 in a 0700 dir) with an idempotent self-signed `client_id`, and `instructions` on `initialize` carrying the same report. **The four-state report reads tool count, not `auth.authorized`** — which the orchestrator's measurement showed is `true` for four of the five silent upstreams — and `mobbin`'s row carries **no command**. Gates: `make test` 0 twice at 1595/199, lint 0, parity 358/358, `acceptance-r6` clean, `parity-manifest-check` 0 at 92 rows, and a **new `parity-authserver.sh` lane at 94 checks across both routers**. **The panel found four real defects and one was an auth bypass the runner had introduced**: grok redeemed a *consent ticket* at `/token` as though it were an authorization code and got a working token — both blobs shared four fields, the code blob had no type tag, and the missing nonce degraded to burning the empty string; every blob now names its type, reproduced red at 200 and green at 400. Both lanes independently found `Origin: null` allowed — reachable from a sandboxed iframe or a `data:` document, and the only control standing on `/register` against a `text/plain` JSON-CSRF body. Also found: `POST /authorize` needed no proof the consent page rendered (now a signed 10-minute ticket bound to client, redirect and challenge), and `/register` parsed JSON without requiring the content type (now 415, restoring the preflight as a second control). **The new lane caught a defect on its own first run**: the Swift 302 carried neither `content-length` nor chunking, so a browser following it would stall — the `location` header is readable immediately, which is why header-only assertions passed against a response that never completed. **Two corrections to the orchestrator's brief.** `make lint` was **not** blocked at 2: `.gitignore` documents that a worktree symlinks `node_modules` and `dist` to the main checkout, and with those links the `tools` guard passes — so `D-r7-af`'s *false dependency* reading is wrong, it is a missing worktree symlink. And this branch has **four** lint steps, not six; the six-step version is on `ai/r7`. **The R7 lane could not run** — it exists only on unmerged `ai/r7`, and running that copy would exercise a sibling's code from a live runner's worktree, so it was refused rather than faked. Open: `PARITY_CUTOVER_TARGET` is 82 against a census now at 92 and **is the owner's to move**; acceptance #10 (`tools/list_changed` on an upstream authorising) is not implemented since stateless HTTP has no stream to push to; #9 (no browser tab per reconnect) is mitigated by 365-day tokens, working refresh and an idempotent `client_id` but **not verified against a real GUI client**; the per-upstream authorise button is not built; and `div-r14-redirect-host` is a declared divergence asserted in both directions across fifteen shapes. Idle was 0.0% throughout, so gate timings are not representative. |
| R15 | The Host check guards `/mcp` and nothing else | router | — | — | Opus | **Ready to verify** | `ai/r14` `76cbb53` | **Built and delivered inside `ai/r14` `76cbb53`, ready to verify with it.** The authority check moves ahead of the dispatch ladder in both implementations, so every route inherits it — proven across all eleven routes by the same arm. `/mcp`'s 403 body is byte-identical to the transport's own; every other route gets the ordinary `Invalid Host header` envelope. `[::1]` is in the allowed set. The live exposure this was filed for — `/health`, `/status`, `/servers` and `/usage` answering 200 to a foreign Host — is closed, and the orchestrator confirmed 403 on a running build of the branch before it was merged anywhere. |
