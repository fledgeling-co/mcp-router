...
| D-g3-i | `waitUntil` skips a final condition check when its poll overshoots | new item | `PoolTestSupport.swift` tests `now < deadline` before `condition()`, so a sleep waking past the mark reports a timeout without re-checking. Needs the condition to land in the last 2 ms of a 10 s wait. `awaitEvent` inherits it, being built on the same poll |
