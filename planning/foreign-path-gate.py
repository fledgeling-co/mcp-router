...
WITHDRAW = re.compile(
    r"(?:\(|\||\*\*|\*|—|–|-->)\s*"
    r"(?:now\s+|is\s+|are\s+|was\s+|were\s+|it\s+is\s+|this\s+file\s+is\s+|both\s+|all\s+)?"
    r"(?:gone"
    r"|unrecoverable"
    r"|not\s+recover(?:ed|able)"
    r"|no\s+longer\s+(?:exists?|present|on\s+disk)"
...
def main():
    ...
    for path in files:
        ...
        for h in hits:
            if file_kind(path) == "RECORD" and not is_pointer(h["token"]):
                extensionless_in_records += 1
    ...
    print("  files holding a machine-durable absolute path        %5d   (counted, never blocked)"
          % len(machine_files))
    print("  extensionless scratch tokens in records              %5d   (the discriminator's hole)"
