...
| M23 | The mock-to-SwiftUI conversion contract | mac | — | `design/mcp-router-console.html` | Opus | **Needs More Work** (7th) — gap-fix 7 | `ai/m23` `d6861e9` | **Verified 2026-08-21 at `effect-witness`; verdict Needs More Work (7th; gap-fix 7 queued).** **All eleven arms reproduce exactly**, every ok-count and every red set; **the trace reproduces on every load-bearing number**, re-taken by the verifier with its own `sitecustomize.py` and the engine sha-verified after; and **the zero-coverage claim is true** — per-line tracing shows the pre-pass suite invoked the gate script once and every line of 128-141 had zero coverage, which is why two lanes exploited it. **BL-1: R5 is reachable with a report path, and the engine documents the frame that falsifies the argument.** The table's completeness reasoning is sound about `gate()`'s interior and **silent about `main()`**, which runs `sys.stdout.flush()` *after* `gate()` returns: with stdout's reader gone, `BrokenPipeError` escapes into the handler with `report_written` False and `report_path` set, **R5 executes with a real path**, the obituary is written a second time, and the ledger a reader opens says `gate: BrokenPipeError` where the first write said `manifest: no artifact at ...` — **the true cause replaced by a downstream symptom, in the artifact whose subject is honest reporting**. Deterministic 3/3, no mutation. The engine's own comment above that line reads *"The last thing that can raise, and it raises after every boundary inside `gate()` has been left"* — **the completeness argument was derived from this file and did not read it.** Narrow: through the gate script stdout goes to `tee`, which holds the pipe open, so `make mock-fidelity` cannot reach it. **Three of our own numbers are wrong and one listed gate cannot be run.** `D-m23-bb`: *five arms invisible* is **nine** — a1-a5 and a8-a11 all ran 59 ok against the pre-pass suite, and **the evidence file's own table says so**, so the summary sentence, the ORCHESTRATOR row and the LEDGER row contradict the table beneath them; the error **understates** the pass. `D-m23-bc`: *145 engine runs* counts every `python3` process — the engine ran **53** times, 10 with `--report`, an over-count of 2.7x, and `D-m23-az` repeats it. `D-m23-bd`: **gate B is in every brief's standing gate list and its recipe exists nowhere** — named once at `M23-gapfix-2.md:121` and carried through four briefs with no surface, command or fixture; the verifier constructed an equivalent and says plainly that is not reproducing it. `D-m23-ba`: case 60's enumeration is evadable three ways, **all a new emitter** — a split literal byte-identical to the constant, `globals()[...]` (absent from the escape list), and a sibling module — each leaving all 67 green, while what it *does* catch was measured; the engine today contains none of those spellings and imports only stdlib, **so the enumeration is sound now and it is the guard that is one-directional**. Both lanes independently found two of the three and no fourth class. Follow-ups `D-m23-ba`...`D-m23-bf`. |
