...
// The template comes from the built reference, not from a copy of it. An inlined duplicate was
// what this vector carried when P9 first put it on the regen path, and the consequence was
// measured: a colour changed in `dist/auth.js` left `make parity-regen` green, so a vector whose
// whole job is to notice the reference drifting could not notice the reference drifting. The
// require is the assertion — if `PAGE` stops being exported, the generator throws rather than
// silently regenerating a stale expectation.
const authModule = require(join(distDir, 'auth.js'));
const PAGE = authModule.PAGE;
if (typeof PAGE !== 'function') {
  throw new TypeError(
    `auth-pages: dist/auth.js does not export PAGE (got ${typeof PAGE}). The vector must be ` +
    `generated from the reference's own template literal, never from a copy of it.`
  );
}
