...
| R7 | The router's thesis is unmet for every harness but Claude Code | router | R3 ✓ | — | Opus | **Ready to verify** — gap-fix 2 delivered | `ai/r7` | **Second gap-fix delivered 2026-08-21; ready to re-verify.** The verdict was that pass 1 closed a ROUTE and not the PROPERTY, so each fix here is argued as a property and each acceptance names a route nobody had. **B1 — R7 was reading a file the harness had stopped reading.** `ClientConfigs` now resolves a client's config from an ordered candidate list, first-that-exists, and only Gemini has more than one: `~/.gemini/config/mcp_config.json` ahead of `~/.gemini/settings.json`. `serverUrl` joins the dialect — the key `agy` WRITES, against pass 1's `httpUrl`, which is a struct tag appearing in the live file zero times. The unnamed route is this machine's live state, both files present and disagreeing, and it is a unit test and a lane pass; a pre-migration install is a second pass, because a straight swap answers this machine and breaks the one behind it. Measured after: Gemini reads `wired via HTTP`, 12 duplicates over **19** entries, no shim replacement proposed — and 19 plus the router is exactly `agy mcp list`'s twenty rows, with the seven non-duplicates named. **B2 — the gate's header claimed more than it checked, and all five walk-throughs now exit 1**, so the fix was the gate rather than the header, and the header was narrowed anyway to say what each rule reads. The comment stripper is a block-comment state machine instead of a line-shape guess, which closed BOTH directions: a write sharing a line with a block comment was blanked, and prose inside a block whose lines lack a leading `*` was read as code. Rule 3 got its own wider vocabulary because the seam is eight files — C stdio, a subprocess redirect, and the unnamed route, **mutation without a write**: a symlink, a hard link or a mode change replaces a harness config through calls carrying no write token, and `D-r7-v` says the lane's byte digest cannot see the last of those either. Selftest 13 cases to **22**. **B3 — a route is evidence of reaching this router's MCP ENDPOINT, not of mentioning its address.** `isThisRouter` now requires `MCPEndpoint.path`, and a `command` beats an endpoint key unless an explicit HTTP `type` says otherwise. The unnamed route was the same defect one axis along: the brief named a `curl` at `/health` reading `wired-shim`, and a plain `url` at `/health` read `wired-http` for the identical reason. Both directions asserted, so `/mcp/` and a query string still read as routes. **B4 — two spellings that disagree are reported, never resolved by precedence.** `HarnessDialect.resolve` returns a conflict that lands in `unparsed`; the unnamed route is a pair with no standard key in it at all, `httpUrl` against `serverUrl`, where precedence is not even wearing the shape of a rule. The agreeing case and the reversed decoy are both asserted, and `D-r7-p`'s note is corrected in this register. Gates: swiftformat 0/509 · swiftlint --strict 0/502 · `make test` 1642 tests, **four runs, three green**; run 2 lost `CallbackLifecycleTests.swift:238`, a loopback bind race that is 0 of 8 in isolation and touches nothing in this diff, registered as `D-r7-x` rather than re-run away from · parity **358/358 at floor** · the R7 lane `pass` at **55** checks over ten passes · write-gate selftest **22 cases** · the six lint steps individually green. `make lint` blocked at the `tools` guard in a worktree with no `node_modules`, recorded as blocked. `ServerParser`, `UpstreamHash` and `SelfReference` untouched. **Merge note:** `D-r7-r`…`D-r7-w` were written on `main` after this branch's base and are absent here, so the register takes the union again; they stay deferred and untouched either way. **Third round, after out-of-family review of this pass's own diff (2026-08-21):** codex (`gpt-5.6-sol`, high) and agy (`gemini-3.7-flash-high`) delivered, grok returned nothing at 16.5 KB and was substituted with `claude-fable-5` (high). Nine findings — six taken, two overruled on measurements recorded in `planning/evidence/R7-acceptance.md`, one deferred as `D-r7-y`. The two that matter: a stdio entry carrying a stale endpoint key was being digested as an HTTP upstream, losing its own duplicate; and the write gate's comment reader opened a block on a `/*` inside a Swift string and blanked every line after it, so an applier under one reported clean — the vacuity the gate exists to refuse, arriving through the gate's own stripper. Selftest 22 → 27 cases, acceptance lane 55 → 59 checks, `swift test` 1648 twice. Commit `edab81e`. |
| R7-C1 | The Harnesses board and the `GET /harnesses` route behind it | mac | R7, M22 | — | Opus | Deferred | — | R7 ships the engine and a CLI verb only. A control route diverges from `src/control.ts` and owes a parity row; the surface that draws it is M22, untriaged. |
...
| D-r7-y | A name duplicate is settled before an entry's endpoints are read, so a conflict in one is never reported | new item | Found by the Google lane on the 2nd gap-fix's own diff. `HarnessReconciliation.compare` matches on name and `continue`s before calling `HarnessDialect.resolve`, so an entry whose two spellings disagree AND whose name matches an upstream is counted as a `.name` duplicate and never appears in `unparsed`. Deferred rather than fixed because the property B4 protects still holds — the entry IS counted, so nothing is silently zero — and the strongest available evidence is the name match. What is lost is a second finding about an entry already reported, which is a completeness gap rather than a wrong answer |
