...
| G3 | `make test` is not deterministically green | `G3-make-test-is-not-deterministic.md` | — | — | **Ready to verify** — gap-fix complete (`ai/g3` `906e795`). The unbounded awaits now run through `awaitEvent` under a ten-second breaker that names the condition, and the bound is asserted by a scanner requiring every call site to sit inside one. The assigned mutation reds in **10.5 s** naming its own condition, against 601 s before; the unnamed route reds in 3.9 s on the resolved-integer claim. Panel caught the first scan rule and, more usefully, corrected the fix's reasoning — a cancellation-aware task group does work, and this repo already contains one. Follow-up `D-g3-k`; `D-g3-j` closed |
