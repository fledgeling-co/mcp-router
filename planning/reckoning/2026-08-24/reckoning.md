...
162 piece(s) of work remain — 49 product, 16 evidence, 97 decision — across 304 ledger rows. This reckoning speaks for 81/85 (95%) of the campaign's designed cases and 11/25 (44%) of its stated requirements; the rest is not known to be done, it is simply not known. 9 input(s) could not be classified — defect status 'fixed-in-campaign' on 15 row(s); defect status 'fixed-in-part' on 1 row(s); defect status 'fixed-in-test' on 3 row(s); defect status 'fixed-upstream' on 1 row(s) — and each is named with its row count rather than counted as work or as done. 92 brief(s) could not be tied to the registry at all; they are listed as `unjoined` and counted as decision work, rather than assumed unbuilt.
...
| Briefs joined to evidence | 21 | 113 | 18.6% | the brief could be tied to something in the registry at all. |
...
| `unjoined` | 92 | 92 | decision-work | named in a brief; the join reached nothing, so its state is unknown either way |
| `broken` | 48 | 51 | product-work | measured, and the answer was no |
...
| `verified-done` | 0 | 134 | none | not remaining; kept so the denominator is honest |
...
- **DEF-003** — loopbackPinned counted the pin instead of reading it
  - this defect's status is 'fixed-in-test', which is not a word this tool classifies. It stays broken because guessing done is the one error here that cannot be recovered from, and the word is reported as a finding with its row count because that placement is this tool's decision rather than the registry's.
- **DEF-004** — the no-install guarantee read only half of where the buttons are stated
  - this defect's status is 'fixed-in-test', which is not a word this tool classifies. It stays broken because guessing done is the one error here that cannot be recovered from, and the word is reported as a finding with its row count because that placement is this tool's decision rather than the registry's.
- **DEF-005** — iOS copy assertions ran against an empty accessibility tree
  - this defect's status is 'fixed-in-test', which is not a word this tool classifies. It stays broken because guessing done is the one error here that cannot be recovered from, and the word is reported as a finding with its row count because that placement is this tool's decision rather than the registry's.
- **DEF-006** — inventory pointed Discover and Triage at someone else's pixels
  - this defect's status is 'fixed-in-campaign', which is not a word this tool classifies. It stays broken because guessing done is the one error here that cannot be recovered from, and the word is reported as a finding with its row count because that placement is this tool's decision rather than the registry's.
- **DEF-008** — iOS tab buttons carry the icon name as their accessibility identifier, and it cannot be replaced
  - a defect is a measured negative result — the unit of product work behind every failing case that cites it
- **DEF-010** — the campaign's own capture step filed two pictures under the wrong surface
  - this defect's status is 'fixed-in-campaign', which is not a word this tool classifies. It stays broken because guessing done is the one error here that cannot be recovered from, and the word is reported as a finding with its row count because that placement is this tool's decision rather than the registry's.
- **DEF-011** — Cleanup's per-row actions are specified by the design and drawn by nothing
  - this defect's status is 'fixed-in-part', which is not a word this tool classifies. It stays broken because guessing done is the one error here that cannot be recovered from, and the word is reported as a finding with its row count because that placement is this tool's decision rather than the registry's.
- **DEF-025** — the paired-Mac settings action sits inside its notice card where the design draws it below
  - this defect's status is 'fixed-in-campaign', which is not a word this tool classifies. It stays broken because guessing done is the one error here that cannot be recovered from, and the word is reported as a finding with its row count because that placement is this tool's decision rather than the registry's.
- **DEF-028** — the never-paired settings empty state drops the design's illustration and centring
  - this defect's status is 'fixed-in-campaign', which is not a word this tool classifies. It stays broken because guessing done is the one error here that cannot be recovered from, and the word is reported as a finding with its row count because that placement is this tool's decision rather than the registry's.
- **DEF-033** — the OAuth parity lane failed once inside a full gate and has not reproduced
  - a defect is a measured negative result — the unit of product work behind every failing case that cites it
- **DEF-034** — the Cleanup board's skill half could not be drawn by any build
  - this defect's status is 'fixed-in-campaign', which is not a word this tool classifies. It stays broken because guessing done is the one error here that cannot be recovered from, and the word is reported as a finding with its row count because that placement is this tool's decision rather than the registry's.
- **DEF-035** — an incremental Swift build after adding an enum case reports impossible test failures
  - this defect's status is 'mitigated-in-campaign', which is not a word this tool classifies. It stays broken because guessing done is the one error here that cannot be recovered from, and the word is reported as a finding with its row count because that placement is this tool's decision rather than the registry's.
- **DEF-036** — the capture script recorded the pairing sheet unreachable for its own reason
  - this defect's status is 'fixed-in-campaign', which is not a word this tool classifies. It stays broken because guessing done is the one error here that cannot be recovered from, and the word is reported as a finding with its row count because that placement is this tool's decision rather than the registry's.
- **DEF-037** — the witness pair map under-reported what had been photographed
  - this defect's status is 'fixed-in-campaign', which is not a word this tool classifies. It stays broken because guessing done is the one error here that cannot be recovered from, and the word is reported as a finding with its row count because that placement is this tool's decision rather than the registry's.
- **DEF-038** — the rendered-quality gate has never been able to run, and nothing said why
  - this defect's status is 'fixed-in-campaign', which is not a word this tool classifies. It stays broken because guessing done is the one error here that cannot be recovered from, and the word is reported as a finding with its row count because that placement is this tool's decision rather than the registry's.
- **DEF-039** — two capture lanes wrote one manifest and the last one deleted the other's provenance
  - this defect's status is 'fixed-in-campaign', which is not a word this tool classifies. It stays broken because guessing done is the one error here that cannot be recovered from, and the word is reported as a finding with its row count because that placement is this tool's decision rather than the registry's.
- **DEF-040** — the blind-test pass flags three tests that do read after mutating, because the mutator's name is the observable's name
  - this defect's status is 'fixed-in-campaign', which is not a word this tool classifies. It stays broken because guessing done is the one error here that cannot be recovered from, and the word is reported as a finding with its row count because that placement is this tool's decision rather than the registry's.
- **DEF-041** — REQ-016's declared outbound-socket effect has no provider in production source
  - this defect's status is 'open-measured', which is not a word this tool classifies. It stays broken because guessing done is the one error here that cannot be recovered from, and the word is reported as a finding with its row count because that placement is this tool's decision rather than the registry's.
- **DEF-042** — the design of record still draws a model-graded evaluation feature the product deliberately does not have
  - this defect's status is 'open-owner', which is not a word this tool classifies. It stays broken because guessing done is the one error here that cannot be recovered from, and the word is reported as a finding with its row count because that placement is this tool's decision rather than the registry's.
- **DEF-043** — the sidebar foot's loopback readout is absent from the build on every board, and the child-process card lost its label
  - this defect's status is 'open-measured', which is not a word this tool classifies. It stays broken because guessing done is the one error here that cannot be recovered from, and the word is reported as a finding with its row count because that placement is this tool's decision rather than the registry's.
- **DEF-044** — a capture width narrower than the design's made real controls invisible, and a witness verdict recorded them as missing
  - this defect's status is 'fixed-in-campaign', which is not a word this tool classifies. It stays broken because guessing done is the one error here that cannot be recovered from, and the word is reported as a finding with its row count because that placement is this tool's decision rather than the registry's.
- **DEF-045** — the capture script dumped the accessibility tree before the step that changed the board, so every dump was labelled with the next board's na
  - this defect's status is 'fixed-in-campaign', which is not a word this tool classifies. It stays broken because guessing done is the one error here that cannot be recovered from, and the word is reported as a finding with its row count because that placement is this tool's decision rather than the registry's.
- **DEF-046** — the campaign's own defect ids named two different defects each, because the staging file numbered from its own length
  - this defect's status is 'fixed-in-campaign', which is not a word this tool classifies. It stays broken because guessing done is the one error here that cannot be recovered from, and the word is reported as a finding with its row count because that placement is this tool's decision rather than the registry's.
- **DEF-055** — vacuity-check's blind-mutation pass examines non-tests and mis-reads Swift idiom; 7 of 7 sampled findings are false positives
  - this defect's status is 'open-detector', which is not a word this tool classifies. It stays broken because guessing done is the one error here that cannot be recovered from, and the word is reported as a finding with its row count because that placement is this tool's decision rather than the registry's.
...
- **BRIEF-M19-gfm-viewer** — M19 — the in-app GitHub-flavoured Markdown viewer
  - the join could not tie this brief to anything in the registry. That is not evidence that it was never built — it is the inferential step of this pipeline returning nothing, and a person has to read the brief and rule
  - nearest the join considered: REQ-001 (0.05), REQ-012 (0.05), REQ-024 (0.04)
- **BRIEF-M2-activity** — M2 — Activity: the live call log
  - the join could not tie this brief to anything in the registry. That is not evidence that it was never built — it is the inferential step of this pipeline returning nothing, and a person has to read the brief and rule
  - nearest the join considered: REQ-001 (0.05), REQ-019 (0.04), DEF-056 (0.04)
- _…and 52 more in ledger.json_
...
## Inputs this reckoning could not classify (9)

Each of these was written into the registry or a brief and no rule here covers it. The rows were still placed, because the partition has to be total — but they were placed by this tool's fail-closed default rather than by anything the registry said, so the class is this tool's opinion and is reported as one. An unrecognised input fails in both directions and only over-reporting announces itself.

| Input | Value | Rows | Placed in | Which rows |
|---|---|---:|---|---|
| defect status | `fixed-in-campaign` | 15 | `broken` | DEF-006, DEF-010, DEF-025, DEF-028, DEF-031, DEF-034, DEF-036, DEF-037 |
| defect status | `fixed-in-part` | 1 | `broken` | DEF-011 |
| defect status | `fixed-in-test` | 3 | `broken` | DEF-003, DEF-004, DEF-005 |
| defect status | `fixed-upstream` | 1 | `broken` | DEF-053 |
| defect status | `mitigated-in-campaign` | 1 | `broken` | DEF-035 |
| defect status | `open-detector` | 5 | `broken` | DEF-051, DEF-052, DEF-054, DEF-055, DEF-056 |
| defect status | `open-measured` | 3 | `broken` | DEF-041, DEF-043, DEF-057 |
| defect status | `open-owner` | 2 | `broken` | DEF-042, DEF-049 |
| defect status | `superseded-by-def-055` | 1 | `broken` | DEF-048 |
