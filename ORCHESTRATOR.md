...
| M23 | The mock-to-SwiftUI conversion contract | mac | — | `design/mcp-router-console.html` | Opus | **Needs More Work** (5th) — gap-fix 5 | `ai/m23` `ec9e215` | **Verified 2026-08-21 at `effect-witness`; verdict Needs More Work (5th; gap-fix 5 queued, `M23-gapfix-5.md`).** **The fourth pass's work is sound and was re-measured in full.** All 20 hand-written cases discriminate — the verifier mutated for each rather than reading them, and lists which mutation reddens which case. Case 51's arm reproduces exactly: exit 1, 58 ok, **one FAIL, case 51 alone**, restored byte-identical. Pass A's validation claim was independently re-derived — the pre-arm coverage union never executes the report block's `except Inconclusive` arm, the post-arm does, and those four lines are the **only** difference in either direction, so the arm reached exactly the branch nothing reached. Scope respected exactly: selftest and `ORCHESTRATOR.md` only, engine and gate script byte-identical to the base. **And the declared blind spot was honest** — the verifier confirmed the bound was accurate and then found the blocker inside it, in case 43. Its words: *the disclosure was the finding.* **BL-1: the marker's delivery is unarmed, and the gate then denies its own ledger.** Move `emit(REPORT_MARKER + report_path)` after the console loop, guarded, and **zero of 59 cases move**. Driven against case 43's own fixture with the engine as a separate process and the ledger read back off the filesystem: exit 3, `write_report` **succeeded** with this run's table on disk and the planted stale content gone, the marker on **neither** stream, so the gate's literal grep fails and it prints `NO ledger was written by this run (exit 3)` **over this run's real table**. The suite pins the marker's presence on success and its absence on a failed write, and nothing about its delivery when a *later* print raises — the one property the engine's docstring claims for it. The fix is one grep on a fixture that already exists. The verifier stated the counter-weight rather than burying it: this is the under-claim direction the engine calls safe, but that allowance is scoped to both streams being gone and here stderr is available. Found by asking the Google lane to **break** rather than review; its concrete placement was inert and moving it to the placement its own reasoning described is the break. Gates post-merge: A exit 1 at 132 with the ledger unmoved, B exit 3 with a ledger written, selftest 0 at 59 twice, lint 0, `make test` 0 twice at **1615/202** (up from 1603/200 — the merge brought R14/R15's tests), reconciler 0 across A-K. **`D-m23-an`: the register is now false about the tree** — `D-m23-ag` says cases 44 and 50 pass no `--report` and stay green under the ordering revert; case 44 now passes `--report` and goes red alongside 43, measured. That row arrived on main after this branch's base, so no runner could have marked it. New follow-ups `D-m23-aj`...`D-m23-ao`. |
