...
| **D-i3-g** | **The commit button says "Send" on a surface where nothing sends** | a copy decision, with the owner | A11 specifies `Send {n} to Mac` exactly and went through the spec gate, so it was not rewritten here — but the Phase D critic is right that the *code* was scrupulously renamed away from the verb (`canQueue`, never `canSend`, with `ConnectionState` carrying a comment explaining why) while the user-facing string was not. The hint beneath already reads "Queues them for review on {mac}" with the narrowing verbatim, so the button and its own hint use different verbs for the same act. Related and checkable: `queuePromisesNothing` forbids nine paraphrases of an automatic transfer (`"waiting for"`, `"to collect"`, `"on its way"`, …) and does not forbid the bare verb the rule is about, and it scans `QueueCopy` only — so the button that makes the promise is outside the guard's reach. Changing the copy and widening the guard are one change and belong to whoever owns the copy. |
| **D-i3-h** | **Queued and Dismissed are the intersection with the current results page, and the Dismissed empty-state copy claims otherwise** | R3 (a registry lookup by id), or a copy change | `TriageBuckets.resolve` iterates `results`, so both decided buckets show only what the *current* 30-item page also contains. That follows directly from A7 (`Undecided = results − queued − dismissed`) and the sets are complete on disk — but dismiss something while searching "github", then search "weather", and the Dismissed segment reads 0 while its empty state says *"Anything you turn down stays here, so a decision made on a train is still readable at your desk"*, with no control to restore it. `DismissedCapability` stores `displayName` expressly so a row is readable without a second registry search, and that field is never used for this. Two honest resolutions — render the decided buckets from the stores directly using the stored names, or change the copy to scope its claim to these results — and both are spec-level, so neither was taken unilaterally inside the item. |
