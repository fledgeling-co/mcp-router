...
| R17 | A failed index leaves a recorded error for one server and nothing at all for another | `R17-a-failed-index-can-leave-no-trace.md` | — | — | **Needs More Work** — gap-fix queued (`R17-gapfix.md`). **The fix is correct and needs no code change**; both blockers are about the claim. The route prediction held exactly and the owner's live machine corroborates it — the partition is perfect. The gate red is measured to `main`, not the branch. But a second mechanism produces the same observable after the fix (filed **R19**), and the two implementations diverge on the very property this change makes load-bearing, invisibly to a sequential lane |
