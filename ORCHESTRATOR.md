...
| **P7** | `control-auth-post-http` needs a real OAuth client | router | R5 ✓ | — | Opus | **Ready for AI** | — | Registered 2026-08-20 from D-p1-a. One of the two rows between the gate and 82 of 83. Discovery, dynamic registration, PKCE, callback on :8880. The only `AuthTransport` conformer is a test fake, so the 405 is real; the vendored SDK emits `state` unconditionally while `extractCode` hard-guards on it, so no SDK configuration reaches the reference's byte string |
| **P8** | Make `install-launchd-watch`'s `reran` attributable | parity | R4 ✓ | — | Opus | **Ready for AI** | — | Registered 2026-08-20 from D-p1-e. The second of the two. `reran` went spuriously green 2 of 6 trials against a decoy `WatchPaths`, byte-identical to a genuine re-run, so the gate would record this row green about one run in three with a watcher that never re-ran. Everything P5 built is kept; the term becomes a stamped stimulus the watcher must observe and report |
| **X1** | The iOS accessibility-tree harness is red | mac/ios | — | — | Orchestrator + runner | **In Progress — 19 → 2** | main (uncommitted) | Baseline 2026-08-20: `make all` exit 2 at `test-ios`, **19 failing cases**, all of the shape *rendered nothing*; macOS green at 1468 tests / 178 suites. Two instrument defects found by measuring: (1) a fixed 50ms settle pass is load- and OS-dependent — replaced with a non-asserting deadline poll, because asserting on an empty tree broke four passing tests while fixing four; (2) **`ObjectIdentifier` is unique only among live objects and the walker retained nothing**, so a released element vended by `accessibilityElement(at:)` had its address reused, collided in `seen`, and its whole branch silently returned `[]` — an address-reuse race, which is why failures moved between runs and files. 19 → 9 → 2, zero regressions at each step (`comm` both directions over the sorted failing-test-name sets). The last 2 are `DiscoverSurfaceIOSTests` and **were red at the baseline**; the built-in diagnostic reports `descendants=5 containers=0 elemCounts=[]` for `ScrollView { QueueCommitBar }` — no content views realised at all — while `ScrollView { plate }` through the same harness now passes. Handed to a runner as `X1` |
...
| R7 | The router's thesis is unmet for every harness but Claude Code | router | R3 ✓ | — | Opus | **In progress** (dispatched 2026-08-21) | `ai/r7` | Not a config-writing chore. The README's headline — 190 processes and ~12 GB down to 0 at rest — is true for exactly one harness; Gemini reaches the router through an `npx -y mcp-remote` shim **and** keeps 17 direct stdio entries, ten of which duplicate the router's own upstreams. Scope is stating, per harness, which of *not wired · via HTTP · via a stdio shim · wired but carrying duplicates*, and naming the overlaps. Writing each harness's config dialect is out. **The runner reads the user's real harness configs and writes none of them** — every red-green is fixture-driven, and any eventual write sits behind a seam with nothing calling it. No matched research doc exists; the brief's own transport question was never answered in this repo. |
...
| M27 | The sidebar foot's loopback readout and the child-process label | mac | M1 ✓ | `design/mocks/prototype.html` | Opus | **Verified — ready to merge** | `ai/m27` `26337b8` | **Verified 2026-08-21 at `raster-visual`.** The verifier re-ran `mac-shell.sh` against the live app and, in the same iteration as each AX assertion, took a `screencapture -l<CGWindowID>` of that window — then opened the PNGs and read `127.0.0.1:8971` and `Child processes  1 of 4` off the pixels. Two mutations red (`.combine`→`.ignore`, and deleting the line). codex/gpt-5.6-sol confirmed no runtime defect. Six follow-ups `D-m27-a`…`D-m27-f`, none blocking — but `D-m27-a` is worth reading before the next item touches A35. |
...
| D-m23-f | The gate poisons its own build directory between runs | M23 | `layer_tokens` runs `swift test` in `app/` — the same SwiftPM `.build` the gate built `MeasureDump` into, and without `MCP_ROUTER_MEASURE=1`. So every run leaves `MCPRouterUI` compiled without `MEASURE`, and the next run's `swift build --product MeasureDump` can reuse it and fail to link `SurfaceRecorder`. Seen twice on 21 Aug 2026, both after a `swift test` that was killed mid-compile. It exits 3, which is the right code for the wrong reason: the instrument broke its own precondition and reports it as an unmeasurable surface |
| D-m23-g | The error state's primary action is a disabled control where the mock draws an enabled one | M23 | `v-error/button/start-the-router` is answered by a node reporting role `state-action-disabled`; the mock draws it `btn primary lg` with no disabled attribute. Read `present` until this gap-fix, and a finding since. The build-side fix is M17's — this row records that the gate now says so |
| D-m23-h | `VOUCHED_CONTROLS` covers eight of the seventeen mock kinds the census can derive, and each entry is global | M23 | `jack`, `banner`, `field`, `callout`, `codeblock`, `progress`, `indicator`, `section-header` and `search-field` have no declared build role, so the first surface to pair one reads it as a finding until the pairing is vouched. Deliberate — a pairing nobody has vouched for is not a measurement — but it means M15–M22 each extend the table. And an entry carries no scope: `"card": {("table", "vstack")}` asserts the equivalence for every state, surface and path at once, so one central edit can retire a real control-kind finding everywhere. Both review lanes raised the blast radius |
| D-m23-i | The surplus quota binds on the build role, so an invention wearing an unmapped role is still exempt | M23 | The G2 fix asks the mock's census whether it names any kind the node's role may answer. A build that draws its fourth skeleton row with role `divider`, `placeholder-row` or `text` has no entry in `MOCK_KINDS_FOR_ROLE`, so the count is 0 and the node reads `covered-by-pair` again — both out-of-family lanes reached this by the same route. The mirror over-report is open too: a node whose role IS mapped, nested inside a pair, is a finding whenever the census names that kind anywhere in the state, even in an unrelated branch. Neither fires on the one filled ledger. Closing both needs a total map, which is `D-m23-h`'s work |
| D-m23-j | Glyph identity is never compared | M23 | The brief asks a pair to be audited on label, control kind and glyph. The harness records no glyph for an icon node, so the third is unread: an unlabelled icon now lands `unclassified` rather than `present`, which is honest, but a labelled control with a vouched role and the right text can still draw the wrong symbol. Named in the code comment and by both review lanes |
| D-m23-k | The token census is not checked for internal consistency | M23 | Every field of `MOCK-FIDELITY-TOKENS` is now parsed as the number it claims to be and the literals marker must name `stray=`, but nothing asserts `matched + pending == rows`, or that any of them is non-negative. A census that does not add up was not a measurement (gpt-5.6-sol, finding 6); the parse strictness is in, the arithmetic is not |
