...
| M23 | The mock-to-SwiftUI conversion contract | `M23-mock-to-swiftui-contract.md` | — | — | **Ready to verify** — 7th gap-fix complete (`ai/m23` `ea960a1`). BL-1 closed, **and it cost the engine's byte-identical property**: the acceptance asked for two behavioural properties no selftest edit can produce, so the runner put the contradiction to a lane, took the acceptance over the freeze, and served the freeze's stated reason by confining the change to one hunk in one function of the engine — 25 lines added and 2 removed, 15 of the additions comment and 10 code, 2 of those the pre-existing `flush()` calls re-indented into a new `try`, leaving 8 new physical code lines (four statement nodes by AST). *Six lines* understated it, and the commit carries a 206-line selftest change alongside; `D-m23-bm`. Case 68 red first at 67 ok alone, green after. Its first patch shape was wrong and a lane broke it twice. Four corrections re-measured — including one that corrects the verifier's own correction — gate B finally has a runnable recipe, and the enumeration guard's residual is stated and measured |
