...
| **G1** | **Stop the checks blaming the app for being out of date** | harness | M11 ✓ I3 ✓ P4 ✓ | — | Opus | **DIED — relaunch** | — | Killed by `503 no-eligible-account / over_reserve` (9 of 10 accounts at or over reserve). **A null return is a death, not a completion**, and the workflow still reported `completed`. **Nothing was lost**: verified 0 commits with the worktree still at main's old HEAD, so this is a clean re-run rather than a resume. Its scope has grown since it was written — it now owns the `OWED` mac-shell launch race, `D-p1-e` (which is the ONLY DIVERGED row on main and is costing 1 of 83 every run), `D-p4-e` and `D-p4-b` |
