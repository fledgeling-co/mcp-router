...
| G3 | `make test` is not deterministically green | harness | — | — | Opus | **Ready to verify** — gap-fix | `ai/g3` `906e795` | **Delivered 2026-08-21, ready to verify (gap-fix; `ai/g3` `906e795`, 4 commits on `d544968`).** Property established: **a regression in this class produces a named red inside the CI bound, not a timeout.** `awaitReap`/`awaitSessionEnded` now run through `awaitEvent`, under `waitUntil`'s ten-second breaker, reporting expiry as a failure naming the condition at the caller's source location. The wait is **abandoned rather than cancelled** — `await task.value` on a `Task<_, Never>` has no cancellation check, so a group around it as it stands awaits the loser and still takes ten minutes; an observer records the landing and the poll reads that record. **The bound is asserted rather than documented**: `PoolAwaitBoundTests` requires every call site in `app/Sources` and `app/Tests` to sit lexically inside an `awaitEvent`, and ten controls hold the scanner five each way. Assigned mutation: exit 2 in **10.461 s** naming *timed out after 10.0s waiting for: `own` to be reaped under the arming it just made* — against the verifier's measured 601.184 s. **The route nobody had named** (the swap the other way): exit 2 in **3.916 s** at `PoolReapingTests.swift:87`, fast because the `#require` on the resolved integer throws before the await is reached, which is the bound being on the right side. Control with P6's wrap removed, killed at 60 s: exit 142, no summary line. Gates: `make test` 0 twice (1584/198), lint 0 (494 files), parity 358/358, `acceptance-r6` `examined=6 failures=0`, three pool suites 0.305 s. **The panel found the fix carrying its own defect for the fourth time this session** — all three lanes took apart the first scan rule (counting wraps per function passes two-wraps-and-a-bare-call); grok added a `func` whose signature contains `awaitEvent(`, codex added `// awaitEvent(` as a false opener; all taken. **Grok also corrected the fix's stated reasoning, which is the more valuable finding**: a task group is not ruled out, only that shape of one — make the wait cancellation-aware and it works, and `AuthorizationURLBox` in `OAuthFlowStarter.swift` is already that construction in this repo, written for the same hang at 91 s against a 20 s budget. Corrected in source and brief; the construction deferred as `D-g3-k`. Two Gemini findings overruled, each refuted by the other two lanes and by measurement. `D-g3-i` left undone with its reason stated — registered, out of scope, and *a fix I cannot watch bite is not evidence*. `D-g3-j` closed on all four corrections. **`ORCHESTRATOR.md` will conflict on merge** — the runner edited the worktree's copy surgically rather than pulling `main` in mid-flight; take the union. |
