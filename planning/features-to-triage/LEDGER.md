...
| M23 | The mock-to-SwiftUI conversion contract | `M23-mock-to-swiftui-contract.md` | — | — | **Needs More Work** (7th) — gap-fix 7 queued (`M23-gapfix-7.md`); verified at `effect-witness`. All eleven arms and the whole trace reproduce, and the zero-coverage claim is true. **One of the six route rows is false**: `main()` flushes stdout after `gate()` returns, so a broken pipe puts R5 on a real path and the ledger names `BrokenPipeError` instead of the true cause — deterministic, no mutation, and the engine's own comment above that line says it. Three of our recorded numbers are wrong (nine arms invisible not five, 53 engine runs not 145) and gate B has been in four briefs' gate lists with no recipe anywhere |
