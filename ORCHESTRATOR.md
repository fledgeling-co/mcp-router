...
| M23 | The mock-to-SwiftUI conversion contract | mac | — | `design/mcp-router-console.html` | Opus | **Ready to verify** — 7th gap-fix | `ai/m23` `ea960a1` | **Delivered 2026-08-21, ready to verify (7th gap-fix; `ai/m23` `ea960a1`, 3 commits).** **BL-1 is closed, and closing it cost the engine's byte-identical property — a deliberate departure the runner disclosed as the first line of its report.** My brief said *do not change the engine* while its acceptance asked for two properties of the engine's **behaviour**, which no selftest edit can produce. The runner put exactly that to a lane before touching anything; fable found no mechanism and said why — the overwrite happens inside `main()`'s handler, driven only by argv and the state of fds 1 and 2, and a file outside that process cannot alter it. **It took the acceptance over the freeze and then served the freeze's stated reason — attributability — by keeping the change to one hunk in one function of the engine.** *Six lines* understates it (`D-m23-bm`). Measured off the commit: **25 lines added and 2 removed in that hunk — 15 of the additions comment and 10 code, and 2 of the 10 are the pre-existing `sys.stdout.flush()` / `sys.stderr.flush()` re-indented into a new `try`, leaving 8 new physical code lines**: `try:`, `except Exception as error:`, an `emit(…)` call spanning four lines, `hush_streams()`, `return 3`. That is **four statement nodes by AST** — `except` is a clause of the `try`, not a statement — so any "N statements" figure has to name its convention. And the hunk is one commit's engine half only: the commit carries a 206-line change to the selftest alongside it, 233 lines across two files. That is the right resolution: the freeze existed for attributability rather than as an end, and its purpose survived while its letter did not. R5 reproduced 3/3 on the shipped engine, no mutation, with the marker on stderr twice and the ledger naming `gate: BrokenPipeError`. Case 68 drives it on case 44's fixture buffered and unbuffered: **red first at 67 ok, case 68 alone**, reporting `markers 2 buffered / 1 unbuffered`; green after at 68 ok twice. **The runner's first patch shape was wrong and fable broke it twice**, both reproduced — on an unwritable ledger it claimed the file recorded this run while the file held an earlier run's clean table, and with no `--report` it claimed a ledger at `None`; the shape taken catches the flush one frame earlier and says nothing about disk contents. **The four corrections were re-measured rather than inherited, and one of them corrects the verifier**: `D-m23-bc` said 53 engine runs by counting a `python3 -c` the trace does not find — re-traced under a `sitecustomize.py` logging argv gives **145 processes, 52 the engine, 10 with `--report`, 0 `python3 -c`**, so the over-count is 2.8x and the corrected figure needed correcting. **What 53 was counted from is not recoverable** — the third correction to that one figure, `D-m23-bk` — but the only mechanism in the suite that produces it is case 46's `python3 -` stdin heredoc, handed the engine path as an argument: it `ast.parse`s the engine rather than running it, and its `sys.argv[0]` is `-`, so a command-line mention count takes it and a `sys.argv[0]` count does not. *Five arms invisible* is **nine** — the evidence column carried one fact under two wordings and the summary counted one of them; column normalised. **Gate B now has a runnable recipe** recorded beside A and C — a direct engine invocation against the real manifest and a missing dumps dir, exit 3, hermetic, no MEASURE build — and the file says plainly that what pass 2 ran under that letter **is not recoverable** rather than implying a reproduction. `D-m23-ba`: spelling count now AST-derived with `BinOp` folding, implicit concatenation and bytes; escape list widened by eleven including `globals(` and `sys.modules`; a seven-character fragment check catches run-time assembly, seven measured as the shortest bound the file admits. **Nine evasions red, and the residual is stated and measured** — constants of six characters or fewer still evade. Gates: A exit 1 at 132 with the ledger and all four dumps sha256-identical either side, B exit 3 with the obituary written, selftest 0 twice at **68 cases**, lint 0, `make test` 0 twice at 1615/202 with `D-m23-be`'s cancellation test green both runs, reconciler 0 across A-K. Lanes read-only from `/tmp` against `chmod 444` copies; worktree carried no mutation outside its own arms, every mutated copy under `/tmp/m23r7/mut/`, engine restored from a `cp` backup at each swap. |
