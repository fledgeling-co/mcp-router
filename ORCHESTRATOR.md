...
| M23 | The mock-to-SwiftUI conversion contract | mac | — | `design/mcp-router-console.html` | Opus | **Needs More Work** (8th) — gap-fix 8 | `ai/m23` `cf7e5fc` | **Verified 2026-08-21 at `effect-witness`; verdict Needs More Work (8th; gap-fix 8 queued).** **The engine change was the right call and the verifier established both horns itself**: R5 reproduces 3/3 on the unmutated shipped engine from *outside* the suite, driven only by argv and the state of fd 1, so nothing a selftest can write reaches it. **A 19-shape differential proves nothing else moved** — exit code, stdout, stderr and ledger byte-identical between the two engine shas across every invocation shape, with only the broken-stdout routes differing and exit 3 on all of them; gate A still exits 3 on a blocked layer, still writes the obituary, still reports 132. Both lane breaks that killed the first patch shape are dead against the shape that shipped. **Scope measured**: one hunk at `mock_fidelity.py:1362`, one function, one commit — 25 added lines (15 comment, 8 code, 2 re-indented) for 5 new statements, so *six lines* sits between the statement and physical counts and **understates** by two (`D-m23-bm`). Case 68 red first at 67 ok alone and green after twice; gate B's recipe ran verbatim; nine evasions red; **the seven-character bound is real and shortest** (6 collides on `'report'`, 4 on `'mock'`); `make test` 0 twice at **1619/204** and lint 521 files, both deltas being G3's merge rather than a defect. **BL-1: the `145` correction is applied in one of three places in its own file.** Line 1224 states it correctly; **line 481 and line 1434 still assert it**, and both are false on the verifier's own trace. `grep 145` over the one file in scope finds all three. **This is `D-m23-bb`'s own defect — one fact under two wordings, the summary contradicting the table — committed by the pass raised to fix it**, which is why the verifier called it rather than filing it. **`D-m23-bk`: `D-m23-bc`'s diagnosis mis-names the process** — there is no `python3 -c` in that suite; the 53rd is a `python3 -` stdin heredoc whose argv carries the engine path, so counting mentions gives 53 where `argv[0]` gives 52. The value 52 is right and the reason is not, which is the third correction to that one figure. `D-m23-bh` (a constant repaired at run time evades case 60 entirely) is **not blocking and the reason is recorded**: the file carries an explicit general concession, both lanes independently argued the same, and **both broke two of the verifier's own seven counter-examples**. Follow-ups `D-m23-bg`...`D-m23-bm`. |
